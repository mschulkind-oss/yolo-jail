...
| `nix build` usable inside the jail | works — host daemon, store read-only[^gcroot] | **absent, silent** — possible, not planned for now[^nixmac] | **absent, silent** — possible, not planned for now[^nixmac] | expected to work, never measured — the host's own `nix`, through its daemon[^munix] | fresh launch |
