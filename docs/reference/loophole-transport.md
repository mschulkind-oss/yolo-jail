...
| <a id="oq-t4"></a>[`OQ-T4`](#oq-t4) | A no-VM macOS backend does not make this moot | It has no VM, but since 2026-09-17 it starts every host service a launch enables, and its sandbox runs as a separate OS user that cannot read a `0600` endpoint file. So the separate-user grant above is called: `macosuser.BuildRunPlan` stages it for every endpoint variable the launch carries. That path is pinned by unit tests and has not run on a Mac. |
