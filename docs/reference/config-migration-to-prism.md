...
| <a id="ruling-1"></a>**Ruling 1** (the 2026-07-26 rulings, cited by that name from `configReset` and `staterender.go`) — **`yolo config reset` truncates the surface as well as removing both sidecars** | Adoption makes "no baseline" mean "adopt what is there", so a reset that only deleted sidecars would re-capture the discarded edits. It also makes reset visible immediately rather than after the next boot. Of the three ways to tell "first migration" from "the user asked to discard", it is the one that adds no state: a one-shot marker is a new sidecar with a lifecycle to get wrong, and an mtime heuristic fails on clock skew and touched files. |
