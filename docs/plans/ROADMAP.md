...
| **Capture timing — stop deferring to next boot** | a real gap, not just a smell | Capture runs on every entrypoint invocation (50× over 3 days here), but the container is `--rm` and `onTerminate` does not capture, so **the last session's edits are lost by construction**. [composed-file-permissions.md §5.3](../design/composed-file-permissions.md) costs four options: a `yolo config capture` subcommand (cheap, names the mechanism), shutdown capture in the existing `onTerminate` hook (closes the common case), an inotify watcher on the already-supervised `yolo-jaild` (the only real fix; needs debounce + a race story), or shrinking the problem via §8's class split. |
