...
- **image-shaped and device rows** (8): `.#yoloImageExtras` on `macos-user`, `iptables`/DNAT on `macos-user`,
  ...
  the [`OQ-P2`](../reference/macos-user-provisioning.md#why-it-is-this-way) GNU-userland exclusion, and vendor-installer auto-capture on `macos-user`.
...
**Ids are stable; the ORDER is not re-derived.** G34 and G35 were added by the 2026-09-16 measurement pass
([§5.1](#51-what-is-now-measured) rows 8 and 17); G34 ranks second by impact — it is a launch-breaking defect, not a
...
| G5 | **Terminate hooks are unwired everywhere except a Linux TTY, and one of the things they clean up is a credentials file** | macos-user, podman/macOS, container/macOS + podman/Linux non-TTY | Ending a `macos-user` session by closing the window or `kill`ing the launcher leaves `<stateDir>/env/<cname>.env` — root-owned 0600, holding every resolved `env_sources` value and provider credential — on disk. ⚠ This row said **Ctrl-C** until 2026-09-19; ^C is forwarded to the jail now and no longer terminates the launcher at all, so the trigger is the other two signals. The gap is unchanged — if anything it is reached more often, since the keystroke that used to run the hooks on a Linux TTY no longer runs them anywhere. Elsewhere: orphaned socats, stale endpoint files, uncaptured config edits, no timing report. On AC, a jail no yolo command can then stop. | small (~30 lines) | no |
...
| ~~G9~~ | ~~**`required_capabilities` is validated, exported, and read by nothing**~~ **FIXED 2026-09-17** | all four | The gate is built and the export is deleted: a config declaring `["web_search"]` that nothing satisfies is refused before the backend dispatch, so all four backends refuse alike. What *satisfies* was settled the narrow way — a declaration (`providers.<name>.capabilities`, an `mcp_servers.<name>.provides`) or the `code_editing`/`command_execution` baseline — leaving an agent's NATIVE capability unstateable, which is what `YOLO_ALLOW_UNMET_CAPABILITIES=1` is for and the one thing still owed here. | done | ~~yes~~ — [`OQ-CAP2`](../design/agent-auth-modes.md#12-decision-ledger) is ruled and built |
| G10 | **The `macos-user` declaration-silence set: keys accepted, dropped, and never mentioned** | macos-user/macOS | `mounts`, `network.mode`, `perf_logging`, `programs.autoprune`, `loopholes.<name>.jail_env`, the inherited user scope, `workspace_readonly`'s implicit `yolo-jail.jsonc` lock, and `MISE_ENV`'s whole `mise.jail.toml` mechanism all vanish without a line. DP-D15 ruled a fatal refusal for the directory-shaped cells only (config `mounts`, pack `mount` grants, host nvim config); the rest of this set has no ruling. `YOLO_STORE_PACKAGES` left this list on 2026-09-24: the arm now prints that the dial is ignored, because `packages:` already come from the nix store there — a notice, not a refusal, per the dial's own ruling in [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#store-delivered-packages). `required_capabilities` left this list on 2026-09-17 — not by being disclosed here, but because its gate went in above the backend dispatch, so there is no macos-user-specific silence left to break. | small each; medium as one sweep | **yes**, for the cells DP-D15 does not cover |
...
| G15 | **PARTLY FIXED 2026-09-16** (the `host-processes` half — `platforms: ["linux"]` now declared, so a Mac gets `journal`'s warning instead of a daemon that fails every call) **and 2026-09-24** (the merged-config half — `platformInertLines` runs `loopholes.ApplyConfigEnabled` before `PlatformInertNotes`, pinned on both the launch and the dry-run arm by `TestLaunchReportsAUserEnabledPlatformInertLoophole`). Still open: **`audio`'s pack `env` sets `PULSE_SERVER` on a Mac** | podman/macOS (+ macos-user for the `env` half) | A user who *enables* a Linux-only loophole on a Mac gets a clean launch and no line. Worse, `host-processes` has no `platforms` key, so on macOS the daemon **starts**, the front publishes, the witness passes, and every `yolo-ps` call fails on GNU-procps argv. And `audio`'s pack `env` half still crosses, so `PULSE_SERVER` names a socket that does not exist. | small (one line in the manifest; one resolver fix) | no |
...
| G19 | **PARTLY FIXED 2026-09-25** (the `YOLO_VERSION` half). **The `macos-user` bootstrap env is a hand-maintained two-name wire, and four things fall off it** | macos-user/macOS | `YOLO_PROFILES` is missing, so every pack *config surface* renders as if no profile were selected while the agent's own env is correct. `env_sources` do not reach the bootstrap, so `mcp_servers.requires_env` deletes servers whose variable the agent **will** have. `YOLO_PACK_ROOT` is bootstrap-only, so in-sandbox `yolo programs ls` says "run it there" to someone who is there. ~~`YOLO_VERSION` is unset, so `config.InJail()` is false and `yolo host apply` inside the sandbox is not refused — it renders into the shared sandbox home while every message says "your real home".~~ **The `YOLO_VERSION` half is addressed by [`OQ-FT13`](../design/agent-footer.md#OQ-FT13):** the launch sets it in the session env (`buildPlan`, `internal/macosuser/orchestrator.go`), so `config.InJail()` is true in the sandbox, and each reader that changes is audited in [agent-footer.md §2.2](../design/agent-footer.md#22-what-macos-users-marker-moves), UNVERIFIED on a Mac. That audit does not cover `yolo host apply`'s own wording, so whether it still says "your real home" in the sandbox is open. **Still open, READ 2026-09-25:** the other three. The bootstrap relay (`buildBootstrapEnv`, `internal/macosuser/runplan.go`) carries `YOLO_PROVIDERS` and `YOLO_USE_PROFILES` but not `YOLO_PROFILES`, carries no `env_sources` value, and is still the only env `YOLO_PACK_ROOT` is set in. | small (each is 1-15 lines) | no |
...
| G21 | **Nix inside the jail: four independent breaks — two fixed, the container-Mac one ruled not planned** | podman/Linux, podman/macOS, container/macOS, macos-user | ~~`nix build` in a jail is refused for `experimental-features`, because the image bakes no `nix.conf`~~ — **FIXED 2026-09-24** for the container image: `mkBinPathLinks` writes `/etc/nix/nix.conf` enabling `nix-command flakes` ([`integration/nixconf_test.go`](../../integration/nixconf_test.go)). On AC there is no store mount and no notice. An in-jail build's output gets **no GC root**, so a host `nix-collect-garbage` can delete the store path a running jail is executing from, silently. ~~On `macos-user`, the one backend that *requires* a host nix for every launch is the one whose sandbox cannot see it (`nix: command not found`).~~ — **FIXED 2026-09-24, and MEASURED on the `macos-user` CI job the same day** ([run 36050645052](https://github.com/mschulkind-oss/yolo-jail/actions/runs/36050645052)): the launch puts the host's own nix client (the store `bin` directory the launching user's `nix` resolves to) on the sandbox PATH and sets `NIX_REMOTE=daemon` plus an `extra-experimental-features` `NIX_CONFIG`, and says so, or says why not (no daemon socket, a client outside `/nix/store`) ([`internal/macosuser/hostnix.go`](../../internal/macosuser/hostnix.go); on hardware only by [`integration/macosusernix_test.go`](../../integration/macosusernix_test.go), which has not run yet; [reference](../reference/macos-user-nix-and-features.md#nix-inside-the-sandbox)). **Ruled 2026-09-24 for the container Macs (podman/macOS, container/macOS): possible, but not planned for now.** The route stays recorded below in case it is ever wanted. A refuter measured a fully working in-jail nix with two podman volumes + that same 3-line `nix.conf`. **Measured 2026-09-16, and the macos-user arm is smaller than it looked:** inside the shipped profile shape, `connect(2)` to the daemon socket survives the write-deny, `nix build nixpkgs#hello` returns RC=0, and an indirect gcroot IS created because the daemon writes it as root ([§5.1](#51-what-is-now-measured) rows 10-12). Nothing about confinement blocks it — the whole macos-user break was that `nix` is not on the sandbox's PATH. | small (each break) / medium (store lifecycle) | **ruled** for the container Macs (not planned for now); still open for podman/Linux: who reaps an in-jail build's output |
...
| G32 | **PARTLY FIXED 2026-09-16** (the AGENTS.md sentence now names the conventional local pack, which is the slot's real occupant). ⚠ **AMENDED 2026-09-22: the premise that the tree is inert is DISPROVED.** `~/.claude/skills` on the host is not empty space — it is where Claude Code auto-loads plugins from (the `skills-dir` sentinel, measured against 2.1.278), and it can hold a claude.ai **sync root** that a `yolo host apply` was adopting and silently forking ([`synced-skill-trees.md`](../design/synced-skill-trees.md), fenced 2026-09-22). So "should the literal path reach the jail" is no longer a question about an empty slot. Still open, as the row always said, is that question itself: **the host user-level skills tree does not exist as a yolo LAYER** | all four | A skill in `~/.claude/skills` on the host does not reach the jail. `SkillTarget.HostSource` was removed by S3 because it was set to the *destination*; the middle term of "built-in < host user-level < workspace" is gone. Two routes already reach the outcome (the conventional local pack; a filtered `packs` entry pointing at `~/.claude`), and **no OQ, ledger row or comment rules the literal path out on purpose**. | zero (document route 1) / small (~tens of lines for route 2's robustness fix) | no — but fix the AGENTS.md sentence either way |
...
| G35 | **A fractional `resources.cpus` passes validation and fails the launch on Apple Container** — **Measured 2026-09-16** | container/macOS | `"cpus": 1.5` is a valid config by `internal/config/validate.go:1560` ("must be a positive number") and Apple Container rejects it outright: `Error: The value '1.5' is invalid for '--cpus <cpus>'`, then a usage block. The user meets a backend's argument parser, not a yolo message, for a value yolo told them was fine — and the same config runs on podman, which accepts fractions. The fix is a backend-shaped validation or a launch-time round: both are small, and which one is right depends on whether rounding silently is better than refusing loudly, which is why this is a one-line ruling rather than an obvious patch. | small (~15 lines) | small ruling — round or refuse |
...
### Closed by the 2026-09-16 pass

Fixed in the same session as the measurements, each with the test that fails if it is
reverted — listed here because a tracker that only grows is one nobody trusts:

- **G24 whole.** `validatePrune` (non-object, unknown sub-key, a threshold the reader would
  ignore), a `prune` section in `config_ref.txt`, and the vacuous coverage test rewritten to
  look for a SECTION rather than a substring — it was passing because "prune" sits inside
  "autoprune".
- **G15's `host-processes` half.** `platforms: ["linux"]`, so the loophole reports inert on a
  Mac instead of publishing a daemon whose every call fails GNU-procps argv (measured: BSD
  `ps -C bash` is `ps: illegal argument: bash`, and there is no `/proc`).
- **G27's captures half.** `capturesArgs` asks `roBindsUnsupported` instead of comparing
  `rt == "container"` by hand, so the store arrives on AC ≥ `acROBindsFloor` — where `:ro` is
  measured to be enforced — and the below-floor refusal now prints its reason.
- **G28's prompt.** `opts.Stdin = os.Stdin`, plus the terminal gate that keeps a piped `y`
  from answering a prompt nobody saw.
- **G8's briefing half and G32's AGENTS.md half**, plus the guides' macOS port cells: the
  bridge paragraph no longer asserts `169.254.1.2` (wrong on two of three host stacks) and
  explains the `unknown` disposition macOS podman always reports.
...
**G21/G22 (nix and store delivery).** In-jail nix on the container Macs was ruled **possible, but not planned for now** on 2026-09-24; what follows is the recorded route if it is ever wanted, not scheduled work. The measured one first: in this jail, against
...
|---|---|---|---|
| `mounts`, `network.mode`, `perf_logging`, `programs.autoprune`, `required_capabilities`, `loopholes.<name>.jail_env`, `MISE_ENV`, the inherited user scope, `workspace_readonly`'s implicit `yolo-jail.jsonc` lock | macos-user | All are read only below the `macos-user` arm's return, or are absent from the closed `sandboxEnvPairs` / bootstrap wire lists. DP-D15 ruled a fatal refusal for the directory-shaped cells only (`mounts`, pack `mount`, host nvim config); the others have no ruling. `YOLO_STORE_PACKAGES` is no longer silent (2026-09-24) | `internal/cli/run/loopholeinert.go` — the four `noteMacosUser*Gaps` printers already on that arm cover only `devices`/`gpu`/`kvm`, the port keys, directory `host_files`, and skills/briefings |
...
| An **enabled** Linux-only loophole — **the report half FIXED 2026-09-24** | podman/macOS | `platformInertLines` resolved each loophole through `LoadLoophole` — the manifest only, no config — so `Enabled` was the `default_enabled: false` default and `PlatformInertNotes` skipped it. It now runs `loopholes.ApplyConfigEnabled` (built on `ConfigEnabledOverride`) first. Still open: the pack's `env` half sets `PULSE_SERVER` | the `audio` pack's `env` contribution |
...
| 11 | macos-user: does an in-sandbox `nix build` therefore work? | **YES, RC=0.** `nix build nixpkgs#hello` inside `sandbox-exec`, substituted from cache.nixos.org. G21's macos-user arm is a **delivery** problem (`nix` is not on the sandbox's PATH), not a confinement one — built 2026-09-24 on that finding, unrun on a Mac yet. |
...
| 15 | Apple Container: does it pass devices at all? | **NO — verified against hardware.** `container run --help` has no `--device`, no CDI, no device selector of any kind. This is the direct verification the surviving [§4](#4-permanent-limits) row and the `[^acdev]` footnote both asked for, and it was the one asserted-but-unverified half of that row. |
| 16 | Apple Container: is the jail's own cgroup filesystem writable, so the boot could cap processes itself? | **YES.** `cgroup.controllers` reads `cpuset cpu io memory hugetlb pids`, and writing `+pids` to `cgroup.subtree_control` succeeded. So G27's `pids_limit` sub-row is *not built rather than impossible* with a named mechanism: the entrypoint can write the cap the flag cannot pass. |
| 17 | Apple Container: does it accept the fractional `--cpus` yolo's validator permits? | **NO, and it fails the launch.** `--cpus 2` runs (`nproc` → 2); `--cpus 1.5` dies as `Error: The value '1.5' is invalid for '--cpus <cpus>'` while `internal/config/validate.go:1560` accepts any positive number. See G35. |
