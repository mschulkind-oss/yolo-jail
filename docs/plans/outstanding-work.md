...
| **OQ-T9** | **One transport or two?** The security case for keeping the Unix socket was **withdrawn** — `SO_PEERCRED` cannot tell the jail from a same-user host process, and that set is the *intended* boundary | [`loophole-transport.md`](../design/loophole-transport.md) §7.4 | **unify on `loopback-tls`** — two models drift; migration is only two consumers. Changes if separate-user `guest` goes near-term |
