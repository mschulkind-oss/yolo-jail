...
| **OQ-LP2/3/6/8/9** | **The five rulings the loophole-packaging design needs** (row **LP1**). **LP2** — do the `loopholes` block's host-exec keys (`command`, `env` in both shapes, `doctor_cmd`, and `enabled` for daemon-bearing loopholes) become **user-scope-only**? It is a breaking change for everyone who followed `docs/guides/loopholes.md:88`, and a scope error refuses the whole launch, so it needs the warn-then-error migration the doc specifies. **LP3** — a `file://` pack runs a host daemon with no prompt, ever; leave the origin model coherent, or special-case this one kind? **LP6** — is a capability namespace still the right shape now that it applies to **one** auto-activating bundled loophole? **LP8** — an approval anchored to a claim STRING is content-blind (the argv never has to change), so an exec claim is anchored to the commit; the cost is that a `?ref=main` pack re-prompts on every commit. Commit-anchoring now, or a module-dir digest? **LP9** — does LP2's scope error downgrade in-jail the way `agents` does? **LP10 (NEW, from your review)** — retire the hand-placed user loophole dir, now that a `file://` pack subsumes it? It is the one channel that starts a host daemon with **no selection step**, and retiring it forces `loopholes enable/disable` off its one special case. **LP11 (NEW, from your review)** — do bundled loopholes become **official packs**? `AGENTS.md` already dissolved agents into packs; a loophole registry is the world before that move. **LP12 (NEW, from your review)** — G1 removes the only way to give ONE workspace a loophole and not another, and packs were never per-workspace; so a workspace either gets it from a user-scope declaration that selects workspace paths, or it **asks** and your approval is recorded host-side (`pack install`'s existing y/N + lockfile, different requester). **Decide with LP2** — it is what G1's deprecation warning points people at. **LP13 (NEW, from your review — and it is the worst finding in the batch)** — every gate in the design governs a DECLARATION and none governs the FILE it names, so an agent that rewrites a workspace-resident daemon between launches is unnoticed by all four. `file://` is a bare prefix check with **no path constraint**, so a live-mounted workspace counts as "local" and prompts nothing, ever. Pre-existing, untouched by G1, and it makes G1 close half a hole rather than the hole. Fix = confirmation anchored to CONTENT for every origin (digest the module dir + argv-named files), which subsumes LP8's commit anchor and most of LP3. **Rule on this one first** | [`loophole-packaging-overview.md`](../design/loophole-packaging-overview.md) §8 (all seven, at reading level) + §1.1 (LP10/LP11 argued); [`loophole-packaging.md`](../design/loophole-packaging.md) §9 (the full form) | **LP2 yes** (largest single risk reduction, and independent of the kind); **LP3 leave it, but document it**; **LP6 genuinely open**; **LP8 commit-anchor now, digest later if the friction is real**; **LP9 yes, downgrade — `/workspace` is live-mounted, so in-jail `yolo` breaks identically** |
