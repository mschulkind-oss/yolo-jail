...
| **OQ-LP9** | **The one loophole-packaging decision still open** (row **LP1**), and review turned it from small to structural. **Was:** does LP2's scope error downgrade in-jail the way `agents` does? **Now:** *"for jail in jail, the outer jail is essentially 'user level' for the inner jail — we need to support this somehow."* That is the install/enable scope model applied recursively, and it is right: "user level" is whatever scope owns the machine the daemon runs on, so inside jail A it is jail A's own config, owned by jail A's agent, because jail A is the blast radius. **Load-bearing**, because the ruling that deleted the dev-friction escape hatch sends loophole development into a nested jail. **Measured in-jail:** `~/.config/yolo-jail/config.jsonc` is a **read-only** bind, so an agent cannot install by editing it — but its directory IS writable and the config declares `include_if_found: ["overrides.jsonc"]`, so a seam plausibly exists, undesigned. **Verify first:** where that directory resolves host-side — it must be the jail's own home tree, never the human's real `~/.config/yolo-jail/`. *(Everything else is settled: RULED — the scope model, LP2, LP6, LP10, LP11, LP13. DISSOLVED — LP12. FOLDED — LP3, and all but one row of LP8.)* | [`loophole-packaging-overview.md`](../design/loophole-packaging-overview.md) §8 (every decision as prose) + §4.5 (the scope model); [`loophole-packaging.md`](../design/loophole-packaging.md) §9 | **Support it** — after checking where the in-jail config dir resolves host-side. The original downgrade question dissolves: it was never about weakening an error, it was about the scope model having a root that depends on where you stand |
