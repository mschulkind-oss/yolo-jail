...
| 🟡 | **LP1** | **Packs ship loopholes — the 15th kind, `loophole`. DESIGNED 2026-08-13, not built. READ THIS ONE:** [loophole-packaging-overview.md](../design/loophole-packaging-overview.md) — system level, every decision as prose you can comment on, no line numbers. [loophole-packaging.md](../design/loophole-packaging.md) is the implementation authority it derives from. Closes **OQ-CAP2** with **(B)**, and it is the PREREQUISITE that [pack-capabilities.md](../design/pack-capabilities.md) has now been rewritten to assume — A6 shrinks to *"supersede an AUTO-ACTIVATING bundled loophole"*, which is one loophole. Fixes the gap T2's last paragraph names: a config-block daemon becomes `loopback-tls` + `publishes: "socket"` with yolo running a `svcendpoint.ServeFront` in front of it, so a third-party author needs neither Go nor a TLS stack, and `retiredTransportUnixSocket`'s last two readers go. **Three adversarial passes each refuted a load-bearing claim of the first draft**, and four findings changed the shape of the work: (1) the claim enumeration was not total and `packMayAccessHost` returns true on an EMPTY claim set, so a fetched pack shipping only `host_bind_mounts` + `host_devices` got an arbitrary absolute host path into the jail with **no prompt, ever** — wider than anything a fetched pack can do today; (2) a 15th kind is **A12-fatal to any jail on a pre-`just load` image** (`DecodeTolerant` tolerates an unknown *field*, not an unknown *kind*) — the `tier` incident for the third time, so the tolerance change is item **0** and lands before the kind; (3) "`Collisions` refuses a duplicate loophole name for free" is false twice over — it is never called at launch and it skips single-pack groups, so name exclusivity needs a fourth bespoke pre-flight; (4) `{loophole_dir}` is substituted in exactly ONE field, so the design's own example manifest would exec a literal placeholder. Also measured in-jail: **a `:ro` bind-mounted AF_UNIX socket is fully connectable and bidirectional**, which kills both the security rationale for the `readonly: false` refusal and the audio example's claimed cost. **Item 1 of the landing order is independent of the kind and should ship regardless:** the `loopholes` config block executes an arbitrary host command with no prompt, no lockfile and no origin check from an **agent-editable workspace** file — and its `env` override reaches a *first-party* daemon's spawn environment (`LD_PRELOAD` into the broker). **REVIEWED 2026-08-13** — your five comments on the overview added two design changes (the framework owns the wire, so a pack-shipped loophole may only say `publishes: "socket"`; and a manifest must declare its supported platforms, sharing one message with the inert-on-backend report), one withdrawal (the front cannot be the crossing audit log — a loophole's protocol can be anything, so connection-level is the ceiling), and **two new questions about how many channels there should be at all**. **RULED 2026-08-13: nearly all of it.** Organizing decision — **install is USER-scope, enable is EITHER scope** (overview §4.5). Then **LP2** (install-shaped keys user-scope-only, and the migration is a FATAL error naming the file plus an OFFER to install at user level — not warn-then-error), **LP6** (build the capability system), **LP10** (retire the user loopholes dir), **LP11** (bundled become official packs, **`audio` ships IN this batch**), and **LP13 ruled AGAINST hashing** — *"if you can edit user-level files, you have all the perms already"*, so the user-scope edit IS the confirmation and all that survives is a PLACEMENT rule (installed content may not resolve inside the mounted workspace or a jail-home tree). LP12 dissolved, LP3 folded, LP8 all but closed. **LP9's design is now settled too (your three-part split):** nested jails need the scope model to RECURSE (the outer jail is the inner jail's user level), which the "develop loopholes in a jail" ruling depends on; measured, the in-jail user config is a **read-only** bind, so it does not work today | new capability — design done | **your call** (OQ-LP2 next — LP12 is what LP2's deprecation warning has to point at; LP10/LP11 are a direction, not a prerequisite) |
...
| **OQ-LP9** | **The one loophole-packaging decision still open** (row **LP1**), and review turned it from small to structural. **Was:** does LP2's scope error downgrade in-jail the way `agents` does? **Now:** *"for jail in jail, the outer jail is essentially 'user level' for the inner jail — we need to support this somehow."* That is the install/enable scope model applied recursively, and it is right: "user level" is whatever scope owns the machine the daemon runs on, so inside jail A it is jail A's own config, owned by jail A's agent, because jail A is the blast radius. **Load-bearing**, because the ruling that deleted the dev-friction escape hatch sends loophole development into a nested jail. **The blocking check is DONE:** the in-jail `config.jsonc` exists *for nested jails* by design (`userConfigMountArgs`), is a single-file `:ro` bind of your real config, and its directory is the jail's own home — so writing beside it cannot reach the host, structurally. **DESIGN SETTLED by your three-part split** (supersedes both of my proposals — the one-big-snapshot AND `config.local.jsonc`): (1) a generated **preflight file** carrying only what in-jail `yolo check`/`loopholes list`/`pack` evaluate meaningfully, header comment saying so — kills the false-error class; (2) a **nested-launch input** written ONLY where nesting is possible, marked as existing for that reason — non-nesting setups get nothing, by construction; (3) **`--user-layer <file>`**, a CLI arg layering a file at user-level precedence — explicit privilege futzing, safe everywhere by gate-placement Test 1, and it is how an in-jail agent installs a loophole for nested development. Both files render from the one `config dump` computation; census survives as two per-consumer manifests with a drift test. Launch-frozen; `config drift` covers staleness. Testing: golden renders + one integration case per false-error class; the full nested matrix stays a by-hand per-release check. The rejected alternative — riding your `include_if_found: ["overrides.jsonc"]` line — mixes purposes and works by accident. Concept doc: [`gate-placement-principle.md`](../design/gate-placement-principle.md) (NEW — "put the gate where the authority changes"; scope names are roles that rebind as you nest). *(Everything else is settled: RULED — the scope model, LP2, LP6, LP10, LP11, LP13. DISSOLVED — LP12. FOLDED — LP3, and all but one row of LP8.)* | [`loophole-packaging-overview.md`](../design/loophole-packaging-overview.md) §8 (every decision as prose) + §4.5 (the scope model); [`loophole-packaging.md`](../design/loophole-packaging.md) §9 | **Build the three parts as specified** — nothing left to decide; last thing between the design and the "develop loopholes in a nested jail" ruling being real |
