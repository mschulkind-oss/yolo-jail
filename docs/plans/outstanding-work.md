...
| **OQ-LP9** | **The one loophole-packaging decision still open** (row **LP1**), and review turned it from small to structural. **Was:** does LP2's scope error downgrade in-jail the way `agents` does? **Now:** *"for jail in jail, the outer jail is essentially 'user level' for the inner jail — we need to support this somehow."* That is the install/enable scope model applied recursively, and it is right: "user level" is whatever scope owns the machine the daemon runs on, so inside jail A it is jail A's own config, owned by jail A's agent, because jail A is the blast radius. **Load-bearing**, because the ruling that deleted the dev-friction escape hatch sends loophole development into a nested jail. **The blocking check is DONE:** the in-jail `config.jsonc` exists *for nested jails* by design (`userConfigMountArgs`), is a single-file `:ro` bind of your real config, and its directory is the jail's own home — so writing beside it cannot reach the host, structurally. **Proposal on the table (revised on your follow-up — the raw bind is the wrong method):** the inherited file becomes a **composed, per-key-FILTERED snapshot** (the raw bind carries keys whose meaning dies at the boundary — `cache_relocations`/`host_files` — and drops `include_if_found` files, so it already filters by accident); a census classifies every key *transfers / stops / host-only*, with a drift test. Plus `config.local.jsonc`, a jail-owned writable sibling mirroring the shipped `yolo-jail.local.jsonc` shape; recursion is the same composition applied again (each level filters its effective config into the child's inherited file, so every depth sees one `:ro` inherited file + one writable own file). Snapshot is launch-frozen — the jail's normal contract, with `config drift` already covering staleness. The rejected alternative — riding your `include_if_found: ["overrides.jsonc"]` line — mixes purposes and works by accident. Concept doc: [`gate-placement-principle.md`](../design/gate-placement-principle.md) (NEW — "put the gate where the authority changes"; scope names are roles that rebind as you nest). *(Everything else is settled: RULED — the scope model, LP2, LP6, LP10, LP11, LP13. DISSOLVED — LP12. FOLDED — LP3, and all but one row of LP8.)* | [`loophole-packaging-overview.md`](../design/loophole-packaging-overview.md) §8 (every decision as prose) + §4.5 (the scope model); [`loophole-packaging.md`](../design/loophole-packaging.md) §9 | **Build `config.local.jsonc`** — small, last thing between the design and the "develop loopholes in a nested jail" ruling being real. The original downgrade question dissolves: it was never about weakening an error, it was about the scope model having a root that depends on where you stand |
