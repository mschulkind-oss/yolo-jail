...
| **B1b** | **Credential-injecting proxy for git** — host injects the token after egress, jail holds nothing, no human. The tier [boundary-broker.md §5](../design/boundary-broker.md) was missing; it is what the motivating GitHub case actually wants, and unlike B2 it is **not** blocked on N3 | new capability | nothing |
