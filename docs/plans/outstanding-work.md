...
| **OQ-LP13/2/6/9/11** | **The five rulings the loophole-packaging design still needs** (row **LP1**). **LP13 — take this one first.** Every gate in the design governs a DECLARATION and none governs the FILE it names, so an agent that rewrites a workspace-resident daemon between launches is unnoticed by all four; `file://` is a bare prefix check with **no path constraint**, so a live-mounted workspace counts as "local" and prompts nothing, ever. Pre-existing and untouched by G1. Fix = confirmation anchored to CONTENT at install, every origin (digest the module dir + argv-named files). The SCOPE half is already settled by your install/enable ruling; what is open is what install checks. **LP2** — do the config block's INSTALL-shaped keys (`command`, `doctor_cmd`, and `env` in **both** shapes, since `env` reaches a first-party daemon's spawn environment) become user-scope-only? `enabled` and `jail_env` stay at either scope per your ruling. Breaking for everyone who followed `docs/guides/loopholes.md:88`, and a scope error refuses the whole launch, so it needs warn-then-error. **LP6** — is a capability namespace still the right shape now that it applies to **one** auto-activating bundled loophole? **LP9** — does LP2's scope error downgrade in-jail the way `agents` does? **LP11** — do bundled loopholes become **official packs**? `AGENTS.md` already dissolved agents into packs; a loophole registry is the world before that move. *(RULED: the install/enable scope model, and LP10 retire-the-home-dir. DISSOLVED: LP12. FOLDED INTO LP13: LP3, and probably LP8.)* | [`loophole-packaging-overview.md`](../design/loophole-packaging-overview.md) §8 (each as prose) + §4.5 (the scope model) + §1.1 (LP10/LP11 argued); [`loophole-packaging.md`](../design/loophole-packaging.md) §9 (the full form) | **LP13 yes, and before the kind ships**; **LP2 yes** for the install-shaped keys; **LP6 genuinely open**; **LP9 yes, downgrade — `/workspace` is live-mounted, so in-jail `yolo` breaks identically**; **LP11 yes in principle, not yet — `audio` first, the broker last** |
