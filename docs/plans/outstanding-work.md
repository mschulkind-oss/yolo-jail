...
|---|---|---|---|
| **OQ-CAP2** | **Should packs be able to ship loopholes?** Review says yes and it may be the real fix — supersession (A6) exists only because loopholes are not *selected* the way packs are. Today the only third-party path is the `loopholes` block in `yolo-jail.jsonc`, **pinned to the retired `unix-socket` transport** because `internal/hostservice` is not importable outside the module. Cost: no pack kind causes **host-side code execution** today, so this is packs crossing from configuring the jail to running code on your machine | [`pack-capabilities.md`](../design/pack-capabilities.md) §10 | **decide this BEFORE building A6** — building a workaround for a gap you have decided to close is how workarounds become permanent |
