...
| E8 | **Nightly-macOS builder arch mismatch — HALF FIXED 2026-08-02, one blocker left.** `TestExtraPackageLibFarm` and `TestDevPackageLinksRuntimeLib` fail on the nightly with `image platform (linux/arm64) does not match the expected platform (linux/amd64)`. **The framing in the original entry was wrong**: the Intel runner is not a platform-support choice, it is a CI capability constraint — `nightly-macos.yml:44` says Intel runners are the only hosted ones with nested virt, and the job tests the macOS *code paths*, not Intel as a target. So it had a technical answer, not a product one. **DONE:** `publish.yml` now pushes per-arch builder tags plus a multi-arch index for `:latest`/`:<version>` (commit `7cc54a0`), so both arches resolve with no Go change. **STILL BLOCKED:** `containerbuilder.BuildersLine` (`internal/containerbuilder/containerbuilder.go:105`) hardcodes `aarch64-linux` in its format string — *"System is fixed to aarch64-linux (the arch a Mac needs)"*. Verified. So the x86_64 nightly will now pull the right image but be told the builder serves aarch64-linux, and an `x86_64-linux` build will not be offloaded to it. Needs to become arch-aware, plus its two byte-exact tests. Note `publish.yml` is tag-triggered, so even the landed half does not reach GHCR until the next release | open — one Go change |
