...
| E8 | **Nightly-macOS builder arch mismatch — needs a maintainer decision, not a fix.** `TestExtraPackageLibFarm` and `TestDevPackageLinksRuntimeLib` fail on the nightly with `image platform (linux/arm64) does not match the expected platform (linux/amd64)`. Cause is a deliberate publishing choice colliding with a runner choice: `publish.yml` pushes the builder image **`aarch64-linux` only**, commented *"it's the arch Apple Silicon runs; an x86_64 mac (rare, and Podman-only) can build locally or use QEMU"* — but `nightly-macos.yml` runs `macos-26-intel` (x86_64), so the builder pulls an arm64 image and the lib farm never materializes. Two coherent answers: publish a multi-arch builder, or drop these two tests from the Intel nightly (or move the nightly to Apple Silicon). Left open **deliberately** — silently changing either the publish matrix or the runner is a product decision, and the failure is honest as-is | open — maintainer call |
