...
audit, the writer-class split), [its `host_files` section](../reference/composed-file-permissions.md#how-a-host_files-entry-becomes-a-surface) (the shipped
...
| 1.5 | **Fix `config-ref`'s `reset`-re-seeds-`once` promise** — it documents behavior that cannot happen. Either implement it or remove the claim | defect (docs lie) | small | `internal/cli/config_ref.txt`; this is exactly what the `host_files` rule "nothing validates and then half-works" ([composed-file-permissions.md](../reference/composed-file-permissions.md#how-a-host_files-entry-becomes-a-surface)) exists to prevent |
...
| 2.2b | **De-compose the credential surfaces** — move `copilot/config` onto the `writeClaudeJSON` read-modify-write pattern so tokens leave the capture path entirely | defect fix | ~20 lines per surface; precedent was `writeClaudeJSON`, since deleted |
...
| `host-file-staging.md` | **retired 2026-09-26** — graduated into [composed-file-permissions.md](../reference/composed-file-permissions.md#how-a-host_files-entry-becomes-a-surface), now the authority on `host_files` in/out; `git log -- docs/plans/host-file-staging.md` |
