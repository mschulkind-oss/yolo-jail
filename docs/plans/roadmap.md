...
| | 💬 **AND: the broker on macos-user** — blocked by [`OQ-BP-5`](../design/backend-parity.md#OQ-BP-5) | same ruling, and it needs **no reachability measurement**: `sharesLauncherNetns` already returns true for it, the jail is a native process so `127.0.0.1` is the host loopback, and Seatbelt is `(allow default)`. ⚠ Its blocker is the **credential boundary**: `svcendpoint.Publish` writes the endpoint file 0600 and `DialLocal` rests on *"the same uid that published it"* — but the jail runs as `_yolojail`, a different uid, with no mount to reshape. Widen the mode, add a group, or copy per jail: that is a decision about a bearer token, not a hoist | `loopholeinert.go:69`, `svcendpoint/endpointfile.go:114` |
...
| 10 | Build one OpenAI subscription credential service for Codex, Pi, and opted-in host Codex | copied or merely symlinked credentials do not serialize single-use refresh; Codex has a refresh URL override, while Pi should consume access tokens without owning refresh | [`openai-auth-broker.md`](../design/openai-auth-broker.md) · [`openai-subscription-auth.md`](../research/openai-subscription-auth.md) |
