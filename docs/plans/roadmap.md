...
| | **Loopholes on macos-user** — the general framework | it needs **no reachability measurement**: `sharesLauncherNetns` already returns true for it, the jail is a native process so `127.0.0.1` is the host loopback, and Seatbelt is `(allow default)`. The credential boundary that blocked it is **answered in code**: `macosuser.EndpointGrantCommands` grants the 0600 endpoint file to the sandbox uid with two `chmod +a` ACEs, staged by `BuildRunPlan`, and `openai-auth-broker` already starts on this arm through `startOpenAIAuthDisclosed`. ⚠ Never executed on hardware. What is left is generalising the lifecycle — `startLoopholesDisclosed` is reachable only from `runContainer`, above which this arm returns, so every other pack loophole is still disclosed as inert | `loopholeinert.go:86`, `internal/macosuser/runplan.go:375` |
...
| 12 | Compute MCP delivery from authentication-source capabilities | settled 2026-09-16: resolve the selected source per agent render; suppress a server only when that source already declares its `provides` job. Codex, Z.AI, Claude, and agy declare `web_search`; a target using Kilo therefore receives Tavily when its key is present | [`kilo-tavily-profile-gating.md`](../research/kilo-tavily-profile-gating.md) |
