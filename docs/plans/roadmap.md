...
| 23 | How a `jail_daemon` runs on `macos-user` — argv resolution with no image, and whether it is confined | [`jail-daemon-on-macos-user-plan.md`](../design/jail-daemon-on-macos-user-plan.md) · in-review | 2 | [`OQ-DP8`](../design/declaration-parity.md#OQ-DP8) · [`OQ-DP9`](../design/declaration-parity.md#OQ-DP9) | **build** — the plan's steps 3–4, which gate 📦 [row 2](#-ready) and route (a) of [row 45](#-needs-you) |
...
| 2 | Start `jail_daemon` on `macos-user` | Steps 1–2 (the one composer and the by-name decline) shipped 2026-09-18. Step 5 is buildable cold: [`loophole-transport.md`](../reference/loophole-transport.md)'s [`OQ-T4`](../reference/loophole-transport.md#oq-t4) still says the no-VM backend starts no host services. Steps 3–4 wait on 💬 [row 23](#-needs-you) | [`jail-daemon-on-macos-user-plan.md`](../design/jail-daemon-on-macos-user-plan.md) |
