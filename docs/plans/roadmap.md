...
| 8 | Bedrock's schema marker, names, region, env gate, what a gate keys on — and every agent reaching every Bedrock model | [`bedrock-plumbing.md`](../design/bedrock-plumbing.md) · in-review | 14 | [`OQ-BR4`](../design/bedrock-plumbing.md#OQ-BR4) + [`OQ-BR8`](../design/bedrock-plumbing.md#OQ-BR8), ruled as one | **defect, both directions** — a profile gate fires for the wrong agent, and a second profile over `bedrock` silently drops every name-gated fact. Gates key on the profile NAME; derives key on the resolved provider. Then **build**: [`OQ-BR9`](../design/bedrock-plumbing.md#OQ-BR9)–[`OQ-BR15`](../design/bedrock-plumbing.md#OQ-BR15) (BR11 ruled: Claude gets native and everything profiles) shape the all-models direction (one provider per endpoint family, the bridge signing with SigV4, a `models` kind for a company pack, picker rendering, currency); rule BR9 and BR10 first |
