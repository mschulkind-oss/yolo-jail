...
| | Rule | Releases | Cost |
|---|---|---|---|
| **1** | [`OQ-3`](../design/broker-ca-and-nested-hosts.md#7-open-questions) — a `[SKIP]` level for `yolo check` | **Defect, live today.** `internal/cli/check/sections_loopholes.go:23` calls `r.ok` on a section it skipped, and `reporter.go:83` counts it as a pass — ten sites | one ruling, ~10 lines |
...
| 2 | `compat` facts become PROVIDER-DECLARED, so pi serves any local endpoint without yolo picking a vendor | **Ruled 2026-09-18, and it is neither option the row used to offer.** "Every loopback URL" assumes every local server is llama.cpp-shaped and silently downgrades one that is not — `docs/research/local-model-endpoints.md` names a working local provider (`bedrock-mantle`) that would lose capability. "Only llamacpp's provider" hardcodes a vendor. The ruling is **declare, don't detect**: the compat facts are SERVICE facts, so the provider states them and each agent's derive translates them, which is exactly [`OQ-CS4`](../reference/providers.md#why-its-this-way)'s shape (the provider declares the knobs; the consumer decides what each means). **It needs no new mechanism** — `packs/llamacpp` already declares `options` with defaults, and `packs/pi/derive.lua` already reads `prov.options.<name>` with a profile fallback, so this is new option names plus the block that emits them. ollama, vLLM or a user's own entry declare their own; a user whose llama.cpp build supports more overrides the default. ⚠ The five flag values and the `maxTokensField: "max_tokens"` spelling come from [`local-model-endpoints.md`](../research/local-model-endpoints.md)'s pi section, NOT from this row — an earlier cell misspelled four of them and omitted a fifth, and pi ignores unknown keys silently | `packs/pi/derive.lua`, `packs/llamacpp/pack.json` |
