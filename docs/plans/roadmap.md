...
**126 live questions across 35 docs**, plus one 🔒 that command cannot see.
...
| 27 | How a pack declares that it ADAPTS one wire protocol to another | [`protocol-resolution.md`](../design/protocol-resolution.md) · in-review | 1 | [`OQ-PR1`](../design/protocol-resolution.md#OQ-PR1) | **build** — and the ruled half is already buildable without it (see 📦). Four decisions landed in review 2026-09-18: a pack declares the protocols its program speaks, a declared adapter resolves an otherwise-unusable pairing automatically, the adapter's address is configurable, and the single-protocol `base_url` shorthand is deleted with no transition. [`OQ-PR1`](../design/protocol-resolution.md#OQ-PR1) decides the VOCABULARY the rest hangs off — a new contribution kind, or a field on the `service` kind `wire-bridge` already uses — and review round two sharpened the criterion that decides it: **a third-party adapter pack must be first-class**, so the shape is whichever makes someone else's pack resolve exactly as `wire-bridge` does — `rg -n 'wire-bridge' internal/` returning nothing is the done-condition. The two smaller questions were ruled in that round: a provider declaring no endpoints stays legal, and an absent adapter REFUSES and names the pack rather than being auto-joined |
