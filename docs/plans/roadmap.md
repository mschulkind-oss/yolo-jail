...
| 11 | Build one OpenAI subscription credential service for Codex, Pi, and opted-in host Codex | copied or merely symlinked credentials do not serialize single-use refresh; Codex has a refresh URL override, while Pi should consume access tokens without owning refresh | [`openai-auth-broker.md`](../design/openai-auth-broker.md) · [`openai-subscription-auth.md`](../research/openai-subscription-auth.md) |
