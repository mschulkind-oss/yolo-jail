...
| 8 | Bedrock's schema marker, names, region, env gate, what a gate keys on — every agent reaching every Bedrock model, on `bedrock-runtime` only, with AgentCore web search on every Bedrock profile | [`bedrock-plumbing.md`](../design/bedrock-plumbing.md) · in-review | 17 | [`OQ-BR4`](../design/bedrock-plumbing.md#OQ-BR4) + [`OQ-BR8`](../design/bedrock-plumbing.md#OQ-BR8), ruled as one | **defect, both directions** — a profile gate fires for the wrong agent, and a second profile over `bedrock` silently drops every name-gated fact. Gates key on the profile NAME; derives key on the resolved provider. Then **build**: [`OQ-BR9`](../design/bedrock-plumbing.md#OQ-BR9)–[`OQ-BR15`](../design/bedrock-plumbing.md#OQ-BR15) (BR11 ruled: Claude gets native and everything profiles; BR10 answered by the whole-matrix requirement: the bridge signs) shape the all-models direction (one runtime provider holding every model family, the bridge signing with SigV4, a `models` kind for a company pack, picker rendering, currency); rule BR9 first. [`OQ-BR16`](../design/bedrock-plumbing.md#OQ-BR16)–[`OQ-BR18`](../design/bedrock-plumbing.md#OQ-BR18) ruled 2026-09-24: the Claude subscription rides the same bridge, with opt-in per-model failover to Bedrock. Ruled 2026-09-25 ([DIR-BR3/BR4](../design/bedrock-plumbing.md#decision-ledger)): **mantle is skipped** — a documented manual recipe, no `-mantle` profile, and [`OQ-BR7`](../design/bedrock-plumbing.md#decision-ledger) answered (no `endpoint_family` field) — and **search comes from an AgentCore gateway** as an MCP preset, on by default on Bedrock profiles; [`OQ-BR19`](../design/bedrock-plumbing.md#OQ-BR19)–[`OQ-BR23`](../design/bedrock-plumbing.md#OQ-BR23) are how (the URL's home, no gateway creation, the signing proxy's shape, a pack-contributed provider-gated preset, Tavily precedence) |
...
| 13 | Write the agent footer design: yolo facts in every agent's status line — the provider and profile first, then other useful facts, and Bedrock cost later | The maintainer asked for it 2026-09-25 ("an easy way to add the provider name to the claude code footer, and possibly some other useful things … bedrock costs there if possible … spread to all agents possible"); writing the design needs no ruling. What is known: Claude Code runs a `statusLine` command from settings.json and hands it session JSON (the `statusLine`, `total_cost_usd` and `display_name` strings are in the installed 2.1.282 binary); the jail already carries the configured profiles and their providers in `YOLO_PROFILES` and the selection in `YOLO_USE_PROFILES`; nothing yolo ships sets a `statusLine` today. For the design to settle: which other agents have a footer or status hook, how a pack contributes one without clobbering a user's own `statusLine`, and where a Bedrock cost comes from (the agent's own estimate, token usage × price, or AWS billing, which lags). It would carry the bridge's failover state too, once [`OQ-BR17`](../design/bedrock-plumbing.md#OQ-BR17) is built | — (no doc yet) |
