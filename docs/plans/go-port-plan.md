...
| 8 | **macos-user delegation — DELETED 2026-07-18** | Was: Go `run` dispatch keeping `runtime == macos-user → exec python -m src.cli run …` until Stage 16b ported those modules. Now REMOVED — `internal/runcmd.Run` routes `runtime==macos-user` to `internal/macosuser.RunMacosUser` (front door injects `RealDeps(runProxy, materialize)` where materialize wraps `darwinpkg.Materialize`), and the four `macos-*` commands dispatch natively via `internal/macosuser`. No Python delegation on any macOS path. | St. 12, 16 → REMOVED St. 16b ✓ |
| 11 | **In-jail CLI-impl forward** (`yolo` transparency) | `yolo run` forwards `YOLO_IMPL=go` + `YOLO_GO_BIN_DIR=/opt/yolo-jail/dist-go/linux-<arch>` + `YOLO_PYTHON=/home/agent/.yolo-shims/_yolo_python` + `PYTHONPATH=/opt/yolo-jail` into the container ONLY when `YOLO_IMPL=go` on the host. Inside, the `~/.yolo-shims/yolo` shim → Python `main()` → re-execs the Go binary, so the in-jail `yolo` is Go — same name, agent can't tell (byte-identical container argv, parity fixture `go_impl_forward`). The entrypoint also generates a `~/.yolo-shims/yolo-go` shim (Go gate baked in) + a `_yolo_python` single-executable uv shim for the Go binary's delegate-to-Python path. Host side: `just install-go` installs a standalone `yolo-go` command (Python `yolo` untouched, no env flip). Reversible: no gate ⇒ in-jail `yolo` stays Python. Mirrored in `internal/runcmd/assemble.go` + `internal/entrypoint/scripts.go`. | St. 16 |
...
| 10 | Go entrypoint (dual image) | 3 | **landed** (generators-lib byte-verified vs live Python earlier; NOW orchestration/boot + image-wiring: `cmd/yolo-entrypoint` + `internal/entrypoint.Main` reproduce main()'s exact ordering + perf labels; the 11 side-effecting helpers ported — timezone/ld-cache, git/jj, DNAT/socat/supervisor-spawn, cgroup-delegate probe, _EXPORT_RE user-env hydration, exec_bash syscall.Exec, deferred mise-uninstall + claude-plugins subprocesses; flake `/bin/yolo-entrypoint` branches on `YOLO_ENTRYPOINT_IMPL=go` (default python, dev-override→dist-go); nested-jail verified BOTH arms boot + tools resolve + Go/Python output trees byte-identical after HOME-normalization; user-env round-trip parity test vs live Python; CLI `-e YOLO_ENTRYPOINT_IMPL` forward landed as seam #10 in run_cmd.py — set it on the host and `yolo run` forwards it into the jail); **human-gated: ONE `just load && just install` for the dual image**. **Post-freeze source drift (2026-07-18): pi host-settings merge.** Python gained a host→jail `settings.json` merge for the **pi** agent mirroring claude's: host `~/.pi/agent/settings.json` → `/ctx/host-pi/settings.json` (`:ro`, staged `ctx-host-pi/`), gated on `"pi" in agents` + the new `host_pi_files` config key (default `["settings.json"]`), `-e YOLO_HOST_PI_FILES=<json list>` — but **no** claude-style script auto-discovery (pi has no hooks/statusLine). `configure_pi()` now runs the SAME agent-agnostic three-way `_sync_host_settings`/`_sync_settings_level` merge claude uses, against a new snapshot `PI_HOST_SETTINGS_SNAPSHOT_PATH = PI_DIR / "yolo-host-synced-settings.json"`, THEN force-sets `defaultProjectTrust="always"` (jail-managed key always wins). The Go port must reproduce this: `internal/runcmd` host-mount (mirror `hostclaude.go` → a `hostPiFileArgs`, minus `appendSettingsScripts`), `internal/config` `host_pi_files` validation + `KNOWN_TOP_LEVEL_CONFIG_KEYS` (byte-exact "expected a list of strings" / "must be a filename, not a path"), and `internal/entrypoint.ConfigurePi` — which **currently writes only `defaultProjectTrust`** (agent_configs.go:159) and must gain the merge REUSING the existing `syncHostSettings` the Go claude port already has (claude.go:152), plus a `loadHostPiSettings` mirroring `loadHostClaudeSettings` and the new snapshot file. **Go side LANDED 2026-07-18**: `internal/entrypoint.ConfigurePi` now runs the merge via the existing `syncHostSettings` + new `loadHostPiSettings`/`hostPiFiles` + `PiHostSettingsSnapshotPath`, force-setting `defaultProjectTrust` after; the entrypoint tree-parity test is green (Python + Go write the pi snapshot identically). The `internal/runcmd` host-mount + `internal/config` validation Go ports remain future tasks (the mount only affects the argv when host `~/.pi/agent/settings.json` exists). | [stage-10](../implementation/go-port-stage-10.md) |
...
| 12 | Front door + slices | 2 | **landed** (cmd/yolo dispatch + seam #1 delegation; native slices gated behind `YOLO_IMPL=go`, default Python. Wired native: **run, check, doctor, ps** [internal/pscmd — reuses runtime ps/ls parsers + table renderer + tracking prune + stuck detector; byte-parity plain-echo output; ledger D11 for the shallow runtime-detect narrowing]. **2026-07-18: the FULL native command surface is now wired** — prune, init, init-user-config, config-ref, loopholes, broker, builder, and all four macos-* command bodies dispatch natively under the gate; **`runtime==macos-user` run dispatch is wired (seam #8 deleted)** via internal/macosuser + darwinpkg.Materialize. `YOLO_GO_DISABLE=<cmd,…>` per-subcommand rollback valve honored. Nothing delegates to Python except unknown sub-subcommands (defensive fall-through).) | — |
...
| 16 | run native (5 sub-phases) | 6–8 | **LANDED — all 5 sub-phases** (internal/runcmd + image.AutoLoadImage). (1) probes: `_resolve_repo_root` incl. the installed-wheel staging w/ the FROZEN rename-aside-never-delete invariant, config load+validate+preset-null+approval prompt, `_runtime` pick, identity env. (2) network/storage/image: socat lifecycle (identical argv), storage ensure, `auto_load_image` ported into internal/image (sentinel LRU, cache materialize, reload reasons, macOS build-offload narrowed w/ diagnosis). (3) argv assembly: the ordered container argv byte-IDENTICAL to live Python over a 10-fixture matrix (hermetic argv oracle) + a static golden. (4) lifecycle+locks: attach/raced-attach, orphan reap polarity, blocking flock + post-lock re-check, stale removal, `_retire_jail_made_venv`, in-process cgroup-delegate (goroutine, lazy cgroup, reuses internal/cgd), broker+relay ensure, FROZEN stop_loopholes guard stack (flock→running-check→relay-kill-before-rmtree), `yolo-user-env.sh` writer (Go-writer/Python-reader round-trip), `final_internal_cmd` byte-golden. (5) e2e: **nested-jail verified BOTH arms boot; container argv BYTE-IDENTICAL (219 elts); cross-impl attach both directions; 3-way flock race elects one starter; orphan reaping on dead owner PID.** Wired behind **YOLO_IMPL=go** gate (frontdoor gatedNativeSubcommands += run; default delegates to Python). macos-user = delegation seam (not ported). 5 e2e fixes: isatty ioctl, runRun arg parse, in-proc cgd, no-timeout Exec, raw debug dump. **⚠ 2026-07-18 RE-AUDIT: NOT host-default-safe — see [go-port-audit-2026-07-18.md](../implementation/go-port-audit-2026-07-18.md). Enable ONLY via `scripts/go-front-door.sh` (four env vars); bare/partial `YOLO_IMPL=go` on a host can silently drop bundled loopholes (OAuth-broker CA/audio) and hijack the repo root, and native `run` drops the banner + jail indicator + storage-migration; native `ps` is destructive on macOS/Apple Container (D11 unsound). Do NOT make YOLO_IMPL=go a default until §A/§B of the report clear.** | [stage-16](../implementation/go-port-stage-16.md) |
| 16b | macOS backend port (as-is) | 3–4 | **package-landed** (internal/macosuser: all pure producers — SBPL, dscl/dseditgroup argv, inheriting-ACL ACEs, `sudo … env -i … sandbox-exec` launch argv, the generated Python bootstrap [repr + nested json.dumps], login rc files — + RunPlan/plan_invariants + orchestrator RunMacosUser [frozen ordering: dry-run before gates, cheap preconds before nix build, plan after gates] + the 4 macos-* command bodies + _refuse_if_root; internal/buildercmd: builder.py VM-lifecycle + builder_cmd.py {setup,start,stop,status} reusing internal/builder pure generators. **Dry-run artifact dump byte-identical vs live Python across a 5-fixture matrix**; builder setup-state parity across 7 fixtures; `GOOS=darwin go build` green. Bootstrap-vs-entrypoint DECISION: emits the SAME Python bootstrap (keeps Python entrypoint alive on macOS → repointing at the Go entrypoint is a documented follow-up, so St.17 entrypoint deletion is NOT auto-unblocked by 16b; the rest of the Python subtree IS released). **Front-door wiring LANDED 2026-07-18: run dispatch `runtime==macos-user` + seam #8 deletion + macos-*/builder handlers all wired in cmd/yolo/native.go; `darwinpkg.Materialize` (the streaming native aarch64-darwin nix build — the last impure half of darwin_packages.py) ported so `RealDeps` has a real materialize adapter. `GOOS=darwin` build green; dry-run artifact parity holds.** As-is broken behaviors reproduced: docstring-claimed-but-unchecked sudo/interp preconditions, stub-python fallback argv, no-GC-root darwin store, unverified first-boot + path_helper fix.) | [stage-16b](../implementation/go-port-stage-16b.md) |
...
<!-- changelog -->
- [2026-07-18] **Full native command surface + macOS run dispatch landed; seam #8 deleted.** Wired native dispatch for every remaining command body (prune/init/config-ref/loopholes/broker/builder/macos-*) and the `runtime==macos-user` run branch; ported `darwinpkg.Materialize` (streaming native aarch64-darwin nix build — the last impure half of darwin_packages.py). Added the `YOLO_GO_DISABLE` per-subcommand rollback valve. Closed the pi host-file port (`hostPiFileArgs` + `host_pi_files` config key/validation — the KNOWN_TOP_LEVEL_CONFIG_KEYS drift the re-audit flagged). Cleared all 2026-07-18 re-audit OPEN items: FIXED — AC 2-of-4 `_ac_materialize` (user-env.sh + briefings), absolute `ca_cert` join, tree-timeout stderr bytes, 26 config-schema constants now drift-covered; LEDGERED — D15 stdin-EOF (decided semantics), D16 terminator wire bytes, D17 malformed-200. Each fix has a regression test; `GOOS=darwin` + full suite + vet + staticcheck green.
- [2026-07-18] Recorded post-freeze Python source drift in the Stage 10 row: the pi agent gained host→jail `settings.json` merging (host_pi_files / YOLO_HOST_PI_FILES / /ctx/host-pi / PI_HOST_SETTINGS_SNAPSHOT_PATH) mirroring claude. **Go side now fully landed** (entrypoint ConfigurePi merge earlier; `internal/runcmd` hostPiFileArgs + `internal/config` host_pi_files key/validation 2026-07-18).
