...
<!-- changelog -->
- [2026-07-18] Recorded post-freeze Python source drift in the Stage 10 row: the pi agent gained host→jail `settings.json` merging (host_pi_files / YOLO_HOST_PI_FILES / /ctx/host-pi / PI_HOST_SETTINGS_SNAPSHOT_PATH) mirroring claude; Go `ConfigurePi` + `internal/runcmd` + `internal/config` must add it in a future port task (not implemented on the Go side).
