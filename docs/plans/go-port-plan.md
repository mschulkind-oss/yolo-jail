...
| 15 | check native | 2 | **diagnostics core landed** (internal/checkdiag: nix build-failure classifier, dry-run will-build tri-state parse + offending drvs, /etc/nix builders-config parse, creds-freshness duration fmt, Linux-builder remedy — byte-goldened); **section ordering / control-flow / badges pending** | — |
| 16 | run native (5 sub-phases) | 6–8 | **briefing gate + socat argv landed** (internal/agentsmd: byte-exact AGENTS.md/CLAUDE.md generation across net/ports/loophole/resource permutations, hardlink-break inode-preserving write, yolo-source-tree probe; internal/network socat argv); **probes → lifecycle → argv-assembly → e2e pending** | — |
