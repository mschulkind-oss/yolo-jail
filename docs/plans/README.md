...
|---|---|---|
| [roadmap.md](roadmap.md) | **THE forward plan.** Everything still to do and nothing else — grouped by state (💬 needs you / 📦 ready / 🔒 waiting / 🧊 icebox), counted from its own contents, and citing every open question by ID in the design doc that holds it. As of 2026-09-02 (full-corpus audit): **thirteen** 💬 rows — four left that day without a ruling, answered by the tree or by measurement — and the 📦 queue holds **five** items, two of them audit-minted no-ruling work (the check warning channel, and five small repairs that had been sitting inside 💬 rows disguised as questions). | **Start here for "what is left?"** |
...
| [../design/provider-catalog-and-selection.md](../design/provider-catalog-and-selection.md) | Splits the knot the two above left tangled: **catalog** (the agent's directory of providers it *could* use) and **selection** (which one it *does* use) are two features, and one table drives both — measured in a live jail, `-p zai` changes the behaviour of one agent in four. Also dissolves disable-without-deleting. | **DECIDED 2026-09-01** (ledger, §10; a tenth question was withdrawn as never having been a design question) and **built** 2026-09-02 — §3's empty pi row was filled from source (`070a3574`), which is what unblocked its plan's step 5, and selection landed for all four agents. §8's own order has one residue: step 4, option C's explicit disable, is still unbuilt. |
| [../design/provider-table-fidelity-plan.md](../design/provider-table-fidelity-plan.md) | Implementation plan for [`provider-table-fidelity.md`](../design/provider-table-fidelity.md)'s defect list: the `wire_api` dialect translation, the composer's composition bugs, the `--profile` split, the census. Written against `578c7e5f`. | ✅ **SHIPPED 2026-09-02**, every step of its build order: the regression test first (`cee9c1fc`), the pi/opencode source verification that gated it (`070a3574`), the canonical vocabulary (`0f04632d`), the composer slice (`5d8bd1fe`, `868b610f`, `ce5ad699`), `--timing` (`886a9191`), the census (`67f87f36`), and D10/D11 (`7fa624ba`). |
| [../design/provider-catalog-and-selection-plan.md](../design/provider-catalog-and-selection-plan.md) | Sibling plan for [`provider-catalog-and-selection.md`](../design/provider-catalog-and-selection.md): env-emitting derives, deleting the `env_shape` placeholder vocabulary, the `pack_profiles` → `use_profiles` rename, the `options` block. | ✅ **SHIPPED 2026-09-02**, step 1's research included (`070a3574`): `use_profiles` (`43d24e9e`), the env derives with `env_shape` deleted (`f55f2109`, `3144fbed`), the codex/pi/opencode selection keys (`58437e8b`, `6d1d7c54`), and `options` + the `kind: "profile"` shrink (`8aceeb0f`, `714e987e`). |
