...
| [../design/base-home-legacy-state.md](../design/base-home-legacy-state.md) | The machine-wide base home is the union of every shipped pack's workspace-scope state dir, pre-created as mountpoints and never cleaned, so it accumulates per-workspace runtime state (transcripts, session stores) that every jail can read and that the launch seeds back into each workspace. A host-only, separate-marker, fail-closed migration MOVES the runtime bytes to a host-only archive, plus a narrowed seed and the invariant that a base state dir is a mountpoint whose steady state is empty. | **DESIGN, partly built** — detection shipped 2026-09-21, and so did a launch refusal that departs from the design (the user runs a printed `mv`; no apply step). R1–R4 pre-ruled; eight questions, routed as [roadmap row 36](roadmap.md#-needs-you). |
...
| [../design/sso-backed-bedrock.md](../design/sso-backed-bedrock.md) · [plan](../design/sso-backed-bedrock-plan.md) | Bedrock from a host `aws sso login`, served into the jail by `packs/aws-auth` over the container-credentials endpoint. [`OQ-SSO7`](../design/sso-backed-bedrock.md#13-decision-ledger) rules three Bedrock credentials supported, SSO primary. | **DECIDED, partly built** — plan steps 1–4 and 6 built and wired end to end in code; steps 5, 7, 8 unbuilt; nothing has run on a real host. |
