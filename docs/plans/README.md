...
|---|---|---|
| [internal-rollout-macos-user.md](internal-rollout-macos-user.md) | **The scope statement for announcing yolo internally to Mac-using engineers on `macos-user`** — fifteen user-visible promises with their measured status, the five rulings they are derived from, the deferred remainder by name, and seven questions for the maintainer. It is the FILTER over [setup-support-gaps.md](setup-support-gaps.md), not a second copy of it: which of that grid's gaps a colleague meets in week one. | **Start here for "what is between us and announcing it?"** — DESIGN, seven questions open |
