...
| [pack-declaration-reform-plan.md](pack-declaration-reform-plan.md) | Implementation plan for [../design/pack-declaration-reform.md](../design/pack-declaration-reform.md): replace the 59-key manifest with `contributes[]` (one kind per contribution, a checkable footprint, the one-writer rule), pull MCP to a shared canonical type with per-agent `derive` projections (no op DSL), and record per-key provenance. Five phases, format rewrite last. | **Plan** — OQ1 + OQ2 decided 2026-07-29; Phase 0 (kind registry) is start-ready. |
