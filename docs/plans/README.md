...
| Doc | What it is | Status |
|---|---|---|
| [cli-color-audit.md](cli-color-audit.md) | Make `prune`/`builder`/`macos-*` render rich markup to ANSI instead of stripping it; consolidate the duplicated printers. | ✅ **DONE** — every item is landed, the last on 2026-09-26: `run/console.go` consolidated onto `internal/richtext` (`67454a8`), and the TTY probe unified onto `internal/tty` (`b76b2ba`, with the two private copies that commit missed in `prune` and `ttyproxy` removed 2026-09-26, and every color decision routed through the `NO_COLOR` gate). |
...
| [../design/provider-credential-scope.md](../design/provider-credential-scope.md) | Split out of provider-switching on 2026-09-22: which provider **credentials** and profile-gated env reach an agent, and how a value reaches only the agent that selected it. Today every hydrated `env_sources` key and every satisfied pack `env` gate reach every agent. | **DESIGN, 2026-09-26 — the gate is BUILT** (`6b3eadae`, reviewed and fixed the same day), on all three vehicles, and observed once in a nested podman launch; [`OQ-CN7`](../design/provider-credential-scope.md#OQ-CN7)–[`OQ-CN9`](../design/provider-credential-scope.md#OQ-CN9), raised in review, are open. |
