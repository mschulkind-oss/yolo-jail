...
| [../design/pack-briefing-defaults.md](../design/pack-briefing-defaults.md) | Why a pack's root `AGENTS.md` ships to the wrong readers: it is also the repository's own agent instructions, declaring one narrow briefing silently switches its delivery off, and a manifest cannot spell broadcast at all. The repository keeps `AGENTS.md`; shipped prose moves to a yolo-only location (a `briefing/` directory), silence means broadcast in a manifest, and every declaration is additive and per-file. Carries a prior-art appendix across seven agent ecosystems. | **BUILT 2026-09-23** — every question ruled and built; MEASURED in a nested jail. [Ready row 9](roadmap.md#-ready) is struck; a graduation candidate for `system-doc`. Companion sketch: [../design/pack-briefing-defaults-plan.md](../design/pack-briefing-defaults-plan.md). |
