...
|---|---|---|
| [stage-e-parked-items.md](stage-e-parked-items.md) | The design work deliberately NOT built (E1–E5), each with why it is parked and what unparking costs. Also links the host-nix-environment study as potential future work, and records the two former Stage E defects so a stale pointer does not re-open them. | **Reference** — parked by decision, not omission. |
