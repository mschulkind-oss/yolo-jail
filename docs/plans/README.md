...
| [../design/pi-git-extension-caching.md](../design/pi-git-extension-caching.md) · [plan](../design/pi-git-extension-caching-plan.md) | Pi's git extension checkouts move into a machine-scoped `.pi-shared-git` store beside `.pi-shared-npm`, so a repository already on the machine is fetched, not re-cloned per workspace. Sharing adds a first-install race (two jails cloning one checkout, pi deleting the other's on failure), closed by a content-keyed `due_on_change` trigger on pi's pre-launch refresh. | **DESIGN** 2026-09-25 — built provisionally on the leanings of [`OQ-1`](../design/pi-git-extension-caching.md#OQ-1)–[`OQ-3`](../design/pi-git-extension-caching.md#OQ-3); those and [`OQ-4`](../design/pi-git-extension-caching.md#OQ-4) (an update under a live session elsewhere) are open. Nothing has run against a real git extension. |
...
| [../design/wire-bridge-gateway.md](../design/wire-bridge-gateway.md) | Split out of bedrock-plumbing 2026-09-25: the wire bridge as the jail's model gateway — SigV4 signing, routing claude's everything profile by model id, a sign-only chat-completions route, the subscription arm with opt-in failover, and the new all-traffic mode that enforces a model allowlist and routes per agent. | **DECIDED, 2026-09-25, nothing built.** Parts 1–4 ruled 2026-09-24 ([`OQ-BR10`](../design/wire-bridge-gateway.md#OQ-BR10), [`OQ-BR16`](../design/wire-bridge-gateway.md#OQ-BR16)–[`OQ-BR18`](../design/wire-bridge-gateway.md#OQ-BR18)); part 5, the 2026-09-25 direction [DIR-WG1](../design/wire-bridge-gateway.md#DIR-WG1), had its questions [`OQ-WG1`](../design/wire-bridge-gateway.md#OQ-WG1)–[`OQ-WG5`](../design/wire-bridge-gateway.md#OQ-WG5) ruled the same day ([ledger](../design/wire-bridge-gateway.md#decision-ledger)). Nothing is open; part 1 (the signer) is ready to build. |
...
| [../design/provider-switching.md](../design/provider-switching.md) | Split out of bedrock-plumbing: a deselected profile leaves codex, pi and opencode asking a new endpoint for the old provider's model, and the fix is one new selection rule — clear the id yolo wrote, keep the one the user wrote. Its alias and shipped-id questions moved to model-lists-and-pickers 2026-09-25. | **DECIDED, 2026-09-25** — nothing open. [`OQ-PSW2`](../design/provider-switching.md#decision-ledger) (the deselect clear) and [`OQ-SW1`](../design/provider-switching.md#OQ-SW1) (`-p` outranks a host-config value) are built; [`OQ-PSW4`](../design/provider-switching.md#OQ-PSW4) (silent unless `--verbose`) waits on `--verbose` reaching the jail. |
