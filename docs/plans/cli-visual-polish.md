...
| 5 | **`NO_COLOR` crosses into the jail**, beside `TERM` and `COLORTERM`: as `-e NO_COLOR=<value>` on a container launch and on an attach's `exec`, and in the session env file on macos-user. It crosses only when non-empty. **An attach decides for its own session.** When the attaching `yolo` has no `NO_COLOR` but the running container's environment shows a non-empty one, the `exec` passes `-e NO_COLOR=`. | The in-jail `yolo`, the entrypoint and the prompt read it there. A running container keeps its launch environment, so the attach carries it too. Without the empty pair, a jail launched with `NO_COLOR` would stay colorless on every later attach while the host half of that attach colored, so one invocation would read two environments (decision 3). The value is empty because the convention counts empty as unset and `podman exec` cannot remove a variable. A jail launched without `NO_COLOR` never gets the empty pair, and neither does an attach whose inspect returned nothing, which proves nothing. The consequence is deliberate: an agent CLI that honors the convention also sees it, which is what setting it asks. |
...
| 8 | **Enforced by an inventory and by driven tests**, not by review. `TestEveryColorEmitterIsInventoried` (in `internal/tty`) lists every file whose string literals hold an ANSI escape, together with the gate that governs it, and fails on an unlisted one. `TestNoPrivateTerminalProbe` fails on a terminal probe outside `internal/tty`. Each gate has a test that drives it with and without `NO_COLOR`, and so does each place a command decides color: `TestEveryCommandHonorsNoColor` runs package `cli`'s entry points, and `TestEveryConfigVerbTakesTheOneColorDecision` covers the `yolo config` verbs. The unset run is the control, so the veto cannot pass on output that never colored. | Two private copies of the probe, `prune`'s and ttyproxy's, survived [the color audit](cli-color-audit.md)'s record that the probe was unified. A gate's own test pins only the gate. A review on 2026-09-26 found six `yolo config` verbs and the set-wide `yolo host-daemon status` report each deciding color at a call site no test drove, so the verbs now share one decision and the report has its own test. The inventory reads string literals, so it cannot see color that another program draws (decision 9). |
| 9 | **The terminal jail indicator's red honors it.** The red kitty tab and the red tmux pane border that `SetupJailIndicator` sets around a launch take `tty.NoColor` over the process environment. Under `NO_COLOR` the tab keeps its `🔒 JAIL <project>` title and the pane its border line and text. Kitty's restore then skips its color reset. The tmux style options are still captured and restored. | kitty or tmux draws the indicator, so yolo writes no ANSI escape and the inventory cannot see it. The red is still color yolo adds, so it goes through the gate. The words carry the jail signal and the red only repeats it, so withholding the red loses nothing. A reset to `none` would clear a tab color the user chose. Restoring the tmux styles still heals a red border that an earlier colored launch left behind. |
