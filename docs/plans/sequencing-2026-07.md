...
`host_files` **shipped** 2026-07-25; its in/out rule now lives in
[composed-file-permissions.md](../reference/composed-file-permissions.md#how-a-host_files-entry-becomes-a-surface) (the planning doc was retired 2026-09-26; `git log -- docs/plans/host-file-staging.md`). The items
...
`docs/design/agent-auth-modes.md`) so a future reader can defend or redesign it:
...
- **Bedrock creds do not reach a macos-user jail.** The worked example in
  agent-credentials.md [§3](../reference/agent-credentials.md#env_sources--the-sanctioned-secret-channel) rides the `/ctx/host-claude` mount, which does not exist
  there, so the surface fails open to defaults. The doc notes the fail-open in its [§5](../reference/agent-credentials.md#per-backend-differences)
  table but never connects it to [§3](../reference/agent-credentials.md#env_sources--the-sanctioned-secret-channel).
...
for `Inputs.Workspace`." That is circular — `internal/` is this repo's choice, not
...
answered in [../reference/what-yolo-is.md](../reference/what-yolo-is.md).
...
builder, see linux-builder-lifecycle.md (`linux-builder-lifecycle.md`, archived 2026-09-09 — the removal is DONE; the mechanism is in [`macos-linux-builder-explained.md`](../research/macos-linux-builder-explained.md))),
**and nix-ld**
...
| `nix-ld-dynamic-linking.md` (retired 2026-09-26; the mechanism is in [`mise-node-dynamic-linking.md`](../reference/mise-node-dynamic-linking.md), the plan is in `git log -- docs/plans/nix-ld-dynamic-linking.md`) | Replace the `LD_LIBRARY_PATH` whack-a-mole with nix-ld; closes the custom-`mcp_servers` startup gap. | jail-side — **DONE 2026-07-22** (Variant A: custom `nix-ld.overrideAttrs`, `DEFAULT_NIX_LD` baked; MCP-wrapper exports removed; `yolo check` tripwire added). The host-gated `env -i` acceptance matrix it waited on was overtaken by shipping: ci.yml's `integration` job runs the podman suite on both arches, and only the scrubbed-environment tripwire, which runs in-jail under `yolo check`, has no recorded aarch64 run. |
...
| `linux-builder-lifecycle.md` (archived 2026-09-09; git has it) (`linux-builder-lifecycle.md`, archived 2026-09-09 — the removal is DONE; the mechanism is in [`macos-linux-builder-explained.md`](../research/macos-linux-builder-explained.md)) | Decision record: the two builder mechanisms (VM vs container), why the **VM builder was removed** and the container builder is the sole builder, plus the KEYS-bug diagnosis kept as evidence + a manual unblock. | jail-side (macOS-runtime-gated) — **DONE 2026-07-23**: `internal/builder` + the `yolo builder` commands deleted; `yolo check` Image Build + the run-path failure remedy rewired onto the container builder |
...
| [../reference/what-yolo-is.md](../reference/what-yolo-is.md) | Boundaries: which subsystems are separable from the sandbox ("could you use the config engine without a jail?" — yes for the engine, no for the built-in surface data), and how pack-shipped *logic* would build/ship/cache. Recommends Lua → subprocess, never Go plugins or compile-at-first-run. | conceptual — decision input for item 5 |
| `three-decisions.md` (archived 2026-09-09; superseded by `docs/reference/pack-system.md`) (`three-decisions.md`, archived 2026-09-09 — superseded by [`pack-system.md`](../reference/pack-system.md)) | The three open pack decisions in depth under the all-packs / no-agent-by-default target: where composition runs, projections as data vs code, and what agents-as-packs means for the stateful boot writers. Finds every surface already routes through 2 engine mechanisms; proposes 3. | conceptual — **read before any pack work** |
| [../reference/third-party-pack-logic.md](../reference/third-party-pack-logic.md) | How a third-party agent pack ships *logic*: declarative projections by default plus a subprocess "projector" over one-shot JSON. Retracts "can't be Go" — `goSrc` forbids linking, not a separate program. Build/source tiers reuse the already-shipped `{name, version, url, hash}` spec. | conceptual — the answer to item 5's logic question |
| **[BACKLOG.md](BACKLOG.md)** | **The single ordered implementable list for the whole composed-config + packs cluster** (stages A–E), with a pointer per item to its reasoning doc. | **plan — start here for this cluster** |
| `module-consolidation-and-cleanup.md` (retired 2026-09-26; `git log -- docs/plans/module-consolidation-and-cleanup.md`) | Collapse the parity-era `internal/*` split; drop parity machinery; the OSS-hygiene remnants. | **DONE 2026-07-21 except the `open-source-project` skill audit**, handed to [`roadmap.md`](roadmap.md) on 2026-09-26. Package-merge declined, recorded in [`retired-decisions.md`](retired-decisions.md#no-package-consolidation-and-the-py-names-stay). The maintainer's project-dossier update lives outside this repo and leaves this tree's tracking. |
...
parity in a nested jail. **mise IS also ported** (`ConfigureMisePrism` renders it
statefully; `yolo config render mise` works — verified 2026-07-25; an earlier
version of this paragraph claimed otherwise). Still **not** manifest-modeled and
**not** reachable via `yolo config render`: **MCP, LSP and identity**
(`config render mcp|lsp|identity` → "no surfaces", verified); they run through
bespoke generators — their ports remain.
