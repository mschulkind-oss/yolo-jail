...
- **Bedrock creds do not reach a macos-user jail.** The worked example in
  agent-credentials.md [§3](../reference/agent-credentials.md#3-worked-example--aws-bedrock-a-jail-local-credential-done-right) rides the `/ctx/host-claude` mount, which does not exist
  there, so the surface fails open to defaults. The doc notes the fail-open in its [§5](../reference/agent-credentials.md#5-per-backend--per-os-differences)
  table but never connects it to [§3](../reference/agent-credentials.md#3-worked-example--aws-bedrock-a-jail-local-credential-done-right).
