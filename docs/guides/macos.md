...
| `lsp_servers` | **NOT installed — MEASURED FALSE on hardware 2026-09-12.** The stage does exec the generated script, and the script installs from `$YOLO_LSP_NPM_INSTALL` / `$YOLO_LSP_GO_INSTALL`, whose **only producer is the container's podman `-e` lines** (`internal/cli/run/assemble.go:866-867`). This backend sets neither, so the script finds an empty list and **exits 0 having installed nothing**. Whether to wire the variables or restore the retired warning is an open ruling — see [the handoff](../plans/handoff-macos-user-open-threads.md#1-lsp_servers-installs-nothing-on-this-backend--confirmed-and-three-published-claims-ride-on-it) | `macosuser/provision.go` → `ProvisionArgv` |
