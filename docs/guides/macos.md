...
| `lsp_servers` | **installed**, since 2026-09-12: the bootstrap generates the installer script here too and the confined provisioning stage execs it before the agent. The warning that said otherwise was retired with the gap. ⚠ NOT MEASURED on hardware — see the runbook | `macosuser/provision.go` → `ProvisionArgv` |
