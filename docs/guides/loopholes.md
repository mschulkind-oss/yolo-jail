...
- `cgroup-delegate` — the last built-in: yolo's own in-process daemon, with no manifest and no config key. It is queued to become a manifest loophole too (OQ-A6), and when it does, `paths.BuiltinLoopholeNames` goes with it.
...
| **reserved names refused** | a shadowed loophole name means a daemon nobody audited running under a name the user trusts — and it would be *half* a loophole: yolo's own daemon runs while the manifest's binds, devices and `jail_env` still cross into the jail. The set is **composed**, not listed: the bundled loophole directory names, plus `claude-oauth-broker` from the broker's own constant, plus `cgroup-delegate`, the last built-in service name with no manifest at all. **A name LEAVES the set when its manifest leaves `bundled_loopholes/`** — that is how `host-processes` and `audio` came to be shippable by packs under their own names. **A name that was never a directory does NOT leave by itself**: `journal` was a constant in `paths.go`, so shipping the `journal` pack meant deleting the reservation by hand in the same commit, and `claude-oauth-broker` still has that shape | pick another name (the loophole's name is its directory basename, so rename the directory). Refused at launch, fatally, naming both sides — the same pre-flight that refuses two packs claiming one name |
