...
| **Use `nix` inside the jail** | Yes, through your host's nix daemon.[^cap-nix] | No. It is possible, but not planned for now.[^cap-nix-mac] | No. It is possible, but not planned for now.[^cap-nix-mac] | Should work: the sandbox uses your Mac's own `nix`, through its nix daemon.[^cap-nix-mu] |
