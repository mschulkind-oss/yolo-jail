...
**Related:** [jail-version-predictability.md](jail-version-predictability.md) — the host↔jail mise version skew uncovered here (2026.6.11 vs 2026.2.1) and the plan for pinning/updating jail tool versions
**Decision surface:** [jail-state-separation-design.md](jail-state-separation-design.md) — options F + F+ from this doc, bundled with per-side venvs, as the likely path; open questions consolidated there
...
[venv-per-side-design.md](venv-per-side-design.md) S2a.)*
...
- \+ Also insulates jails from the **host's mise version** (see
  [jail-version-predictability.md](jail-version-predictability.md)) — the
...
- **Workspace-resident venvs stop crossing the boundary.** A `.venv` in
  ...
  [venv-per-side-design.md](venv-per-side-design.md); recommendation:
...
[jail-state-separation-design.md](jail-state-separation-design.md); new
...
[venv-per-side-design.md](venv-per-side-design.md).
