...
| loopholes / host services | ✅ | 🟡 (OpenAI auth only) | 🟡 **exactly ONE starts here** (2026-09-16). The macos-user arm of `run.Run` starts the **openai-auth broker** when that loophole is active and **refuses the launch** if it does not come up (`startOpenAIAuth`); the sandbox uid is granted the endpoint by ACL, `macosuser.EndpointGrantCommands` called from `BuildRunPlan`'s stage commands. Every OTHER pack-shipped loophole is inert and is DISCLOSED as inert (`notePackLoopholesInert` over `withoutOpenAIAuthPack`) — `startLoopholesDisclosed` lives inside `runContainer` and this arm returns above it, deliberately: that wrapper exists to make disclosure inseparable from the SPAWN, and nothing else spawns here. ⚠ This cell said "none start here" and called `EndpointGrantCommands` a built-but-uncalled primitive with zero production call sites; both stopped being true when the broker was wired to this arm. Track L part 1 — the general framework on this backend — is still NOT STARTED; the broker is a one-off, and its spawn is the one host execution on this arm that prints no host-exec disclosure before it. |
