...
> [!WARNING]
> **Dated postscript (2026-08-23): `internal/agents` no longer exists, and
> neither does the agent registry.** Every citation in this doc to
> `internal/agents/agents.go` (§Layer 3 at `:76-154`, §Layer 4 at `:57-62`, the
> Evidence index) points at a package that was renamed to `internal/jailcontent`
> and then emptied of exactly this content. **Agents are packs.** An agent's
> install spec is now declared in its own `packs/<name>/pack.json` —
> e.g. Claude's native installer is `{"url": "https://claude.ai/install.sh",
> "via": "installer"}` at `packs/claude/pack.json:9-10`, which is the same URL
> this doc records, now sourced from a manifest instead of a Go literal. The
> mechanism the doc describes (lazy install on first use via a launcher in
> `~/.yolo-launchers`, generated by `GenerateAgentLaunchers`,
> `internal/entrypoint/shims.go`) is unchanged — only where the spec is *read
> from* moved.
>
> Consequently the quick-reference row *"Add/enable a coding agent (copilot,
