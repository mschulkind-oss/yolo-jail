...
| Podman inside a *nested* jail | ✅ | ✅ | ✅ | `/dev/fuse`+`/dev/net/tun`+SYS_ADMIN+MKNOD+NET_ADMIN+NET_RAW passed to jails at every depth; bridge networking works (`--network=host` no longer required) |
...
| `--add-host` DNS overrides | ✅ | ✅ | ❌ | AC: not supported; loopholes declaring `intercepts` are skipped |
...
| | | | | |
| **Image Building** | | | | |
| `nix build .#ociImage` | ✅ | ✅¹ | ✅¹ | ¹Builds from binary cache by default; a non-cached package is built by offloading to an ephemeral nix+sshd container on the active runtime, torn down after — no standalone Linux builder VM |
