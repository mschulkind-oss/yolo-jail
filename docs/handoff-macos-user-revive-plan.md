...
**Reads with:** [macos-no-vm-direction.md](macos-no-vm-direction.md) (`## Decision`),
[macos-nix-shell-backend-proposal.md](macos-nix-shell-backend-proposal.md)
(`## Decisions (settled)`).
...
- **change:** in `macos_setup`, add a readiness probe: `nix` on PATH + `flake.lock`
  ...
  every dangling `docs/…` string to `docs/macos-no-vm-direction.md` (or this
...
- **kind:** docs
- **files:** `docs/macos-no-vm-direction.md`
