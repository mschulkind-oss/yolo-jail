...
```jsonc
{
  // Optional: extra nix packages to include in this project's jail image.
  // Names must match nixpkgs attributes.
  ...
  // Optional: extra host paths mounted read-only for additional context.
  // "mounts": ["~/code/other-repo", "~/code/shared-lib:/ctx/shared-lib"],
  "network": {
...
3.  **Fail-Loudly Shims**: If an agent tries to use a blocked tool (like `grep`), it receives an error message explaining *why* and optionally suggesting alternatives.
