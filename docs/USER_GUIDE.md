...
- **Locked-memory limit:** yolo adds `--ulimit memlock=-1` whenever GPU passthrough is active. AMD's KFD pins a ~13 MB queue ring buffer to create a command queue, and the rootless default `RLIMIT_MEMLOCK` (often 8 MB) is a kernel-hard cap that can't be raised from inside the jail — without this, `AMDKFD_IOC_CREATE_QUEUE` fails with `EINVAL` and ROCr segfaults. (NVIDIA pinned host memory benefits identically; the jail's `--memory` cap still bounds total RAM.)
...
- **GPU enumerates and `hipMalloc` works, but any kernel launch segfaults (trace shows `AMDKFD_IOC_CREATE_QUEUE … EINVAL`):** the container's locked-memory limit is too small for KFD to pin the queue ring buffer. yolo sets `--ulimit memlock=-1` automatically when GPU is enabled; if you see this, you're likely on an older yolo (update it) or running a hand-rolled `podman run` without the flag — add `--ulimit memlock=-1`.
