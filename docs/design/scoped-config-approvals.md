...
Re-verified 2026-09-09 (previously 2026-09-02; the two line-anchors had drifted and are now named
by symbol): `CheckConfigChanges` takes workspace-only config at both call sites (`internal/cli/run/run.go`
builds `wsCfg` from `LoadWorkspaceConfig` for the macos-user arm and the fresh-launch arm, both
reaching it through `checkConfigChanges` in `internal/cli/run/preflight.go`), the legacy-marker
branch and `retireLegacyWorkspaceSnapshot` are deleted, a fresh non-empty workspace prompts
(`internal/config/snapshot.go`), and `yolo check --accept-config-changes` is wired
(`AcceptConfigChanges`, `internal/cli/check/checkcmd.go` → `internal/cli/check/check.go`). The `TestCheckConfigChanges*` suite pins every branch,
