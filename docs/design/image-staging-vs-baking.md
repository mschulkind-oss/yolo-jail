...
- **`macos-user`** — no container, no image (`internal/entrypoint/darwin.go:81`), no bind mounts. It already
  ...
  (`darwinpkg.go:1-15`). **Both halves exist since 2026-09-06**: the host reuses this package through
  `MaterializeAt` and the jail wiring is `internal/entrypoint/storepackages.go` ([§4](#4-candidates-ranked) C4).
...
### C4 — Deliver `packages:` from the mounted store instead of baking it. **Rank 4. Shape RULED 2026-08-25; AUTHORIZED and BUILT 2026-09-06.**
...
**The [OQ-1](#101-decision-ledger) ruling, both halves.** **(1) Shape settled:** C4/C5 ship as an **opt-in fast path
with the baked path retained** for the backends and launches that do not opt in. The "accept a documented
asymmetry and make store-delivery *the* mechanism" branch is no longer live. **(2) Go/no-go settled
2026-09-06** — the maintainer authorized the build on the evidence in [§1.8](#18-re-measured-after-c2--c3--this-is-11-step-5) and [§1.9](#19-re-measured-2026-09-06--what-a-go-only-rebuild-costs-podman-and-what-chooses-the-flake): the disk term is C3's, the
...
> **This paragraph is the binding shape of the shipped feature, not a proposal** — the sentence that used
> to close it ("C4 stays unbuilt until the maintainer says otherwise") was discharged on 2026-09-06.

**As built, 2026-09-06.** The opt-in is `YOLO_STORE_PACKAGES=1` — an environment variable and not a config
key, because what it selects is a property of the MACHINE and of this launch (podman, Linux, a running nix
daemon) while `packages:` is workspace-scope by ruling ([OQ-4](#101-decision-ledger)); it sits beside
`YOLO_NIX_HOST_DAEMON` and `YOLO_NO_HOST_LOOPBACK` and is read the same way. A launch that asks and is not
eligible **falls back to baking and says so**, because baking still yields a jail with every declared tool,
while refusing costs the user a working launch over a preference about where bytes live. Five pieces:

| Piece | Where |
| :--- | :--- |
| Eligibility + the plan; the fallback notice; the fatal "declared package has no build" (macos-user's A2 ruling, applied) | `internal/cli/run/storepackages.go` |
| The one line that IS R2 — an opt-in launch passes **no** `ExtraPackages` to the image build | `internal/cli/run/imageload.go` |
| The `buildEnv` realization, `internal/darwinpkg` verbatim as [§3.3](#33-the-other-three-backends-shapes) said, through the new `MaterializeAt` | `internal/darwinpkg/materialize.go` |
| The `generate_store_packages` genStep and the `/run/yolo/packages` symlink farm (`bin` → PATH, `lib/lib*.so*` → `LD_LIBRARY_PATH`, `lib/pkgconfig` → `PKG_CONFIG_PATH`) | `internal/entrypoint/storepackages.go` |
| PATH position — immediately before `/bin`, so opting in moves no precedence relation — and the collision check that now counts the farm | `internal/entrypoint/boot.go`, `launchercollision.go` |

**Two things the design did not name and the build had to settle.**

1. **The GC root could not be `darwinpkg.ProfileRootLink` unmodified.** Its leaf is FIXED, which
   `gcroot.go` argues for correctly *for a notch with at most one profile per home*. A machine running
   JAILS is the other case: several workspaces with different `packages:` lists are live at once, so a
   fixed leaf lets one launch unroot the closure another jail is still executing from — the N1 defect
   resurrected by concurrency. C4 passes a **content-keyed** out-link under `paths.PackageRootsDir()`,
   which is `image.ImageRootsDir`'s rule applied to the same problem, and passes `""` (unrooted) in-jail
   for the reason `rootImageFn` already gives for images.
2. **R5 is unchanged, not merely accepted.** A store-delivered library moves from `/lib` to
   `/run/yolo/packages/lib`; both are discoverable only through `LD_LIBRARY_PATH` (`flake.nix:728-732`
...
| R8 | **C4/C5 make the jail structurally dependent on the host nix daemon** — the socket is mounted read-write, gated on Linux by path existence alone (`shouldMountHostNix`, `internal/cli/run/hostprobes.go`). | Pre-existing, but C4 turns "convenient" into "load-bearing" — for an OPT-IN launch only, which is what keeps the dependency a choice. A deliberate decision, per [`gate-placement-principle.md`](gate-placement-principle.md) Test 2. The mount predicate and the eligibility predicate are now the same call (`Options.hostNixMounted`), so a launch cannot promise store delivery and then omit the mount. |
...
[OQ-7](#101-decision-ledger) as MOOT, [OQ-8](#101-decision-ledger), the mount lever, as shipped, and
[OQ-9](#101-decision-ledger), C4/C5's go/no-go, as called. **IDs are an API and are never renumbered**:
...
[OQ-5](#101-decision-ledger) by ID. The 2026-09-06 re-audit added two questions ([§10.2](#102-open-questions)) and two rulings: [OQ-8](#101-decision-ledger), the mount lever, and [OQ-9](#101-decision-ledger) — the go/no-go [OQ-1](#101-decision-ledger) held open, taken as a new ID precisely because [OQ-1](#101-decision-ledger)'s own text is cited from elsewhere. ([OQ-8](#101-decision-ledger) and [OQ-9](#101-decision-ledger) were written the same day in separate worktrees and both claimed the id 8; the mount lever landed first and kept it.)
...
1. ✅ **[OQ-6](#OQ-6) — ANSWERED 2026-09-08 by a successor doc; the go/no-go it withheld is now that
   ...
   **Answer (2026-09-08):**
   ...
   > confirmation arrived with the timing spans: [`perf-logging.md`](./perf-logging.md) [§7](./perf-logging.md#7-what-the-first-real-runs-measured)
...
6. ~~**C4, then C5 — only if the maintainer calls it**, on step 5's evidence.~~ **CALLED 2026-09-06
   ([OQ-9](#101-decision-ledger)), and BOTH ARE BUILT** — C4 first, C5 second on C4's mechanism and under
