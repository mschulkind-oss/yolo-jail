...
**Status:** DESIGN, 2026-08-25 — **with one question ruled and one half of [§10](#10-sequencing--what-i-would-build-in-order) step 3 shipped the same day (`be7b8591`).** [OQ-DF1](#11-open-questions) was ruled *"stream, keep zero tars"* and [`image-staging-vs-baking.md`](image-staging-vs-baking.md)'s **C3** implements it: on podman the load path writes no tar at all, so Ledger B's growth term is zero going forward. Everything else here is still design — nothing built beyond that and what [`../plans/storage-lifecycle.md`](../plans/storage-lifecycle.md) [§1](../plans/storage-lifecycle.md#1-root-the-running-images-closure--first-everything-depends-on-it)–[§4](../plans/storage-lifecycle.md#4-log--overlay--cache-lifecycle--independent-lower-priority) already shipped, and what *it* shipped made a GC *safe* without making one *happen*. **The pre-C3 backlog is untouched**, [OQ-DF2](#OQ-DF2)/DF3/DF4 are open, and Apple Container still writes a tar per store path. **One thing did change under Ledger C without being ruled here:** C2 armed `PruneOldImages` for the first time, so a dedup and a liveness veto shipped with it and `4064f720` made that veto fail SAFE — which retires the *safety* half of [OQ-DF3](#OQ-DF3) while leaving the retention number and the reach untouched ([§11](#11-open-questions) [OQ-DF3](#OQ-DF3)). The re-measurement both docs were waiting on has also been taken: [`image-staging-vs-baking.md`](image-staging-vs-baking.md) [§1.8](./image-staging-vs-baking.md#18-re-measured-after-c2--c3--this-is-11-step-5).

**The short version.** Every reclaimer yolo owns works; none of them ever runs, because all seventeen — every exported `Prune*`/`Purge*`/`Sweep*`/`Reap*` in `internal/prune` (fourteen), plus `RunNixStoreGC`, `HardlinkDuplicateFiles`, and `capture.PruneSupersededCaptures` — are reachable only from a human typing `yolo prune` (verified 2026-08-25 — one non-test call site, `internal/prune/prunecmd.go`, reached only from the CLI dispatch table at `internal/cli/dispatch.go:30`; the seventeenth was added there 2026-09-04 by [`install-capture.md`](../plans/install-capture.md) slice 5, under the same single trigger, and it is the first that does not live in `internal/prune`). So the fix is not a better sweeper — it is moving the delete to the process that made the bytes: **the load path should never leave behind what it does not need**, with `yolo prune` demoted from primary mechanism to crash-recovery backstop. The three ledgers a loaded image occupies are the store closure (rooted, bounded, correct); the cache tar (**zero-growth on podman since C3** — nothing writes one — but still **unbounded on Apple Container**, whose converters need a real file, and with the ~485 GiB pre-C3 backlog untouched on disk); and podman's own image store (**a reclaimer that now fires for TAGGED rows and still cannot see a nameless one** — C2 armed a pass that had returned one row for years, and the `<none>` class the repo-name filter structurally misses is the residual [OQ-DF3](#OQ-DF3) is about). **Two of those three sentences were rewritten on 2026-08-25** — this summary described the pre-C2/C3 system until C2 and C3 shipped the same day; [§3.2](#32-ledger-b--the-cache-tar-bounded-to-zero-on-podman-since-c3-unbounded-on-apple-container) and [§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row) are the bodies it now agrees with.
...
>
> **Its risk rows are always spelled [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) R4,** because [§9](#9-risks) below mints its own R1–R8 and the two tables do not agree: that doc's R4 is the offline safety net and its R5 is scrubbed-`LD_LIBRARY_PATH` breakage, while this doc's R4 is a late safety net for a fast leak and its R5 is budget thrash. A bare `R5` in this file is always [§9](#9-risks)'s.

**Reads with:**

- [`image-staging-vs-baking.md`](image-staging-vs-baking.md) — the [OQ-5](./image-staging-vs-baking.md#101-decision-ledger) ruling this doc executes, its [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk) measurement, and the C2/C3 cost model this doc's sequencing has to interleave with.
...
The maintainer ruled on 2026-08-25, on [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [OQ-5](./image-staging-vs-baking.md#101-decision-ledger):
...
The interesting word is **"bug"**, because the obvious reading — "disk filled up" — is not a bug, it is a consequence. Disk fills up on a machine that builds container images sixty percent of commits ([`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [§1.1](./image-staging-vs-baking.md#11-what-triggers-a-rebuild-and-how-often) — restated here, not re-measured). The bug is structural, and it is this:
...
The sharpest available evidence is not "the hint was ignored" — it is that **the exact artifacts the hint was about are still on disk, byte for byte.** [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk) measured 125 tars totalling 404.4 GiB on 2026-08-15. Measured today in the same directory:
...
All measurements below were taken by me in this development jail on 2026-08-25 against `486a13bb`, labelled **MEASURED** / **NOT MEASURED** in the manner of [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk). Every figure it restates from that doc I re-took rather than copied.

> [!IMPORTANT]
> **This is the PRE-C3 series, and it is retained as dated evidence rather than as a forecast.** C3
> shipped later the same day (2026-08-25) under the [OQ-DF1](#11-open-questions) ruling, so **the podman growth term
...
> [`image-staging-vs-baking.md`](image-staging-vs-baking.md) [§1.8](./image-staging-vs-baking.md#18-re-measured-after-c2--c3--this-is-11-step-5) as the post-C2 state. The two
...
> [!WARNING]
> **There are two image tar caches on this device and [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk) measured one of them.** Inside this jail `~/.local/share/yolo-jail` is a bind of `<ws>/.yolo/home/local/share/yolo-jail` — the **nested jail's** state dir, inside the repo working tree. The **host's** cache is mounted separately at `~/.cache`. [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk)'s numbers are the nested one. Both are measured below; the true total is **606 GiB, not 404 GiB**.

### 2.1 Levels

| Thing | 2026-07-22 | 2026-08-15 | **2026-08-25** | Label |
| :--- | :--- | :--- | :--- | :--- |
| Nested `cache/images` (the path [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk) measured) | 3 tars, 9.5 GiB | 125 tars, 404.4 GiB | **148 tars, 480.71 GiB**, mean **3.248 GiB** | MEASURED |
...
| ↳ *reconciliation:* [`image-staging-vs-baking.md`](image-staging-vs-baking.md) [§1.8](./image-staging-vs-baking.md#18-re-measured-after-c2--c3--this-is-11-step-5) counts **149** | — | — | the 149th is `de22e97910302cee.tar`, written 19:17:21 by the last pre-C3 load, between this pass and that one | MEASURED |
...
| **podman image store** | — | **NOT MEASURED** | **PRE-C2: 2 images, 6.391 GB**; 0 containers, 0 volumes. Superseded the same day — after C2 the same command returns **eight rows over seven images** ([`image-staging-vs-baking.md`](image-staging-vs-baking.md) [§1.8](./image-staging-vs-baking.md#18-re-measured-after-c2--c3--this-is-11-step-5)) | MEASURED (first time), then PRE-C2 |
...
- **Every number is from one machine — this jail.** This is the same limitation [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) records as its R7, and it applies unchanged. It is enough to establish that the defect is real and roughly how fast it bites; it is not a population estimate.
...
- **Ledger C observations do not survive a jail restart**: this container instance's podman store
  is empty (`GraphRoot=/var/lib/containers/storage`, outside the persistent home), so [§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row)'s
...
> re-stream of the same one ([§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row)'s correction). [§2](#2-measured-2026-08-25)'s measured series is the PRE-C3 series and is
...
### 3.3 Ledger C — podman's own image store. **The one with no reclaimer for a nameless row.**
...
[`image-staging-vs-baking.md`](./image-staging-vs-baking.md) R4 says C3 removes the offline safety net if taken to "never write a tar", and its mitigation is explicit: **"Keep-N, not zero."** The maintainer's ruling says *"I see no reason to keep any of this around."* Those are not the same instruction.
...
**My verdict was:** keep-zero by default, with an explicit opt-in for the disconnected case, because the measured fallback is nearly always Ledger C and never tars 2–148. It traded **the maintainer's** risk on **his** machine — a jail that will not start is a much worse day than 400 GiB of disk — and [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) R4's caution was written for a good reason, so it went to Open Questions rather than into an assertion.

> [!IMPORTANT]
> **RULED 2026-08-25 ([OQ-DF1](#11-open-questions)): *"stream, keep zero tars."*** The maintainer ruled expressly for the
...
| A4 | **Never materialize — stream the derivation into `podman load`.** | **SHIPPED for podman in `be7b8591`, 2026-08-25** ([§4.1](#41-candidate-invariants-weighed)b; [OQ-DF1](#11-open-questions) ruled the retention that goes with it). Strictly better than A3 where it applies: saves the write as well as the retention. This is [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) C3, and [§10](#10-sequencing--what-i-would-build-in-order) sequences the two together. Podman only — Apple Container keeps the file. |
...
| A8 | **Podman-store reclamation.** | **Adopted, and it is the best bytes-per-unit-effort in this doc** — because there is still no reclaimer that can see a `<none>` row ([§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row)) and the leak is the *common* case. The 3.55 GB measured here pre-C2 is now three nameless rows, and C2 additionally left four permanently-tagged images coexisting ([`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [§1.8](./image-staging-vs-baking.md#18-re-measured-after-c2--c3--this-is-11-step-5)), which is a second class the retention rule has to price. Independent of A3/A4, so it can go first. **The safety half is no longer open** — a liveness veto ships and fails safe (`4064f720`); the blast radius and the retention number are what remain. See [OQ-DF3](#OQ-DF3). |
...
- **Image *content* policy.** What nixpkgs ships into the image, and whether `fullPackages` belongs on the run path, is [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) C5 — gated on its [OQ-1](./image-staging-vs-baking.md#101-decision-ledger), whose ruling preserved the re-measurement gate. This doc reclaims copies of the image; it does not argue about what is in one.
...
| R3 | **Losing the offline fallback** — a user with no network and a failed build and an empty runtime store cannot start a jail. | [§6](#6-the-offline-safety-net-honestly) shows the window is narrower than [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) R4 assumed (Ledger C is consulted first, and is the fallback in every case where it exists). **RULED 2026-08-25 ([OQ-DF1](#11-open-questions)): keep zero, no opt-in.** The residual risk is accepted and narrow — it needs a failed build *and* an empty runtime image store *and* no usable tar from the pre-C3 backlog. `newestTars` still reads whatever is on disk, so the net change for a disconnected user is that no NEW insurance is written. |
...
**First, Ledger C — but not until [OQ-DF3](#OQ-DF3) is ruled.** Reclaiming superseded podman images needs nothing *else* from this doc: no change to the tar path, no change to rooting, no new invariant. It is also the clearest defect — a filter that structurally cannot see its target ([§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row)) — and the measured prize is immediate. Doing it first also produces the missing measurement: once untagged images are visible to yolo, the podman ledger stops being a blind spot in every future disk argument. That is exactly why **[OQ-DF3](#OQ-DF3) is the sharpest of the four** — how far into a shared podman store yolo may reach is the only thing standing between this step and code. Do not start here before it is ruled.
...
The maintainer ruled the **premise** (it is a bug) and the **goal** (minimal disk, delete without `--apply`). He did not rule the whole **mechanism** — but he ruled **[OQ-DF1](#11-open-questions)** on 2026-08-25, expressly for the C3 implementation, and that one is now closed. Three remain.

1. ✅ **[OQ-DF1](#11-open-questions) — RULED 2026-08-25: Does the offline tar fallback survive at all — keep-zero, keep-one, or keep-N-opt-in?**

   This was the sharpest tension in the doc ([§6](#6-the-offline-safety-net-honestly)) and the only place where the ruling and [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) R4 pointed in opposite directions. It blocked the default value of every retention knob, and it decided whether A3/A4 shipped as "delete" or "delete all but one". The measured facts: the fallback is only reached when the runtime image store has *also* been emptied (`internal/image/autoload.go:353-375`), Ledger C is consulted first and holds the same image, and tars 2–148 are older builds of a tree nobody is on.
   ...
   **What it settles, and what it deliberately does not.** SETTLED: the write path (C3, shipped the
   ...
   - **Pre-existing tars are not swept.** C3 stopped *creating* them; the ~485 GiB already on disk is
     [§10](#10-sequencing--what-i-would-build-in-order)'s work, and the code says so at `internal/image/autoload.go:473-475`.
   - **Apple Container still writes and retains one tar per store path**, because `skopeo copy
     ...
     consume a stream (`internal/image/autoload.go:494-521`). That is a constraint of the backend,
   ...
   - **The fallback's READER survives untouched.** `newestTars` still loads any tar that exists
     (`internal/image/autoload.go:361-375`), which is what keeps an offline start working. The
     ruling removed the writer, not the reader.
   - **[OQ-DF2](#OQ-DF2) and [OQ-DF3](#OQ-DF3) are untouched by it.** Where automatic reclamation lives, and how far into
...
2. 💬 **OQ-DF2: Where does the automatic reclamation live — the write path, the launch path, or `yolo prune`'s default?**
   ...
   [OQ-5](./image-staging-vs-baking.md#101-decision-ledger) settled that yolo *may* delete without `--apply`; it did not say which component does the deleting, and the three are materially different work with different P7 exposure. **(i) Write path** (delete-on-success inside `AutoLoadImage`): narrowest blast radius, improves P4, but only ever cleans up after *itself* — it never reclaims the 148 tars already on disk or another backend's leavings. **(ii) Launch path** (a sweep at jail start, independent of what this launch wrote): reclaims the existing backlog, but fires during other jails' builds and maximises P7. **(iii) `yolo prune` inverts its dry-run default**: simplest, fully explicit, but leaves the trigger gap that [§1](#1-the-ruling-and-what-the-bug-actually-is) identifies as the actual bug.
...
These are **not this doc's decisions** — they are [`image-staging-vs-baking.md`](./image-staging-vs-baking.md)'s, recorded here because this doc's shape depends on them. That doc's Decision Ledger is authoritative; this table is a pointer.

| ID | Ruling, 2026-08-25 | What it fixes here |
| :--- | :--- | :--- |
| `image-staging` [OQ-5](./image-staging-vs-baking.md#101-decision-ledger) | 404 GiB of cached tars is a **bug**. No reason to keep any of it. Minimal disk is the goal. yolo **may** delete cached tars without `--apply`. The shipped GC work is nowhere near enough. | The premise of this entire doc ([§1](#1-the-ruling-and-what-the-bug-actually-is)); licenses [§4.2](#42-the-shape-this-lands-on)'s aggression and rules out A1/A2 as primary. |
| `image-staging` [OQ-3](./image-staging-vs-baking.md#101-decision-ledger) | **Content-addressed tags win.** `localhost/yolo-jail:latest` is **not** a public surface and nothing may depend on it by name. The cachix caveat concerns the **nix binary cache**, a different surface from the podman image tag. | Licenses fixing the `:latest` thrash in [§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row) and makes C2 and this doc's Ledger C work share one retention decision ([§10](#10-sequencing--what-i-would-build-in-order)). |
| `image-staging` [OQ-1](./image-staging-vs-baking.md#101-decision-ledger) | If C4/C5 ship, they ship as an **opt-in fast path with the baked path retained**. The gate's *step* — `image-staging` [§11](#11-open-questions) step 5's re-measurement after C2+C3 — **was taken 2026-08-25** and is that doc's [§1.8](./image-staging-vs-baking.md#18-re-measured-after-c2--c3--this-is-11-step-5); **that discharges the step, not the gate.** The go/no-go itself is unmade and remains the maintainer's. | Sets the precedent for R6's accepted backend asymmetry, and fixes [§10](#10-sequencing--what-i-would-build-in-order)'s fifth step as the shared measurement gate. **Not** an approval of C4/C5, which are both still unbuilt. |
| `image-staging` [OQ-4](./image-staging-vs-baking.md#101-decision-ledger) | `packages:` **stays workspace-scope** — "yes, has to be." Fix the cost, not the scope. | Puts scope out of bounds ([§8](#8-what-this-does-not-cover)) and makes cost the only available lever, which is what this doc pulls. |
