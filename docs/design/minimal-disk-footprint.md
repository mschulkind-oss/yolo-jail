...
- [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#delivering-into-the-runtime) — C9, which replaced this doc's shipped delivery mechanism and made Ledger B zero on every backend ([§3.2](#32-ledger-b--the-cache-tar-now-bounded-to-zero-on-every-backend)).
...
[`../plans/storage-lifecycle.md`](../plans/storage-lifecycle.md) was written after a real incident: a host `nix-collect-garbage` reclaiming ~2.5 TiB swept a **running jail image's own store closure**, leaving 235 of 467 `/bin` symlinks dangling (`storage-lifecycle.md`). Its [§1](../plans/storage-lifecycle.md#1-root-the-running-images-closure--first-everything-depends-on-it)–[§4](../plans/storage-lifecycle.md#4-log--overlay--cache-lifecycle--independent-lower-priority) shipped 2026-07-22 and delivered exactly four things:

| § | What shipped | Evidence, checked 2026-08-25 |
| :--- | :--- | :--- |
| [§1](../plans/storage-lifecycle.md#1-root-the-running-images-closure--first-everything-depends-on-it) | A durable per-image nix GC root, `build/roots/<sha16>`, re-asserted every run | `internal/image/gcroot.go`; called from `AutoLoadImage`, `internal/image/autoload.go` |
| [§2](../plans/storage-lifecycle.md#2-auto-gc-safety-net-min-freemax-free--only-after-1) | `yolo check` **warns** when the host daemon's `min-free == 0` — yolo never edits `nix.conf` | `internal/cli/check/section_autogc.go` |
| [§3](../plans/storage-lifecycle.md#3-bounded-rooting-aware-store-gc-in-yolo-prune--after-1-and-2) | Opt-in, bounded, rooting-aware `yolo prune --nix-gc`; refuses in-jail | `internal/prune/nixgc.go`; gating at `internal/prune/prunecmd.go` |
| [§4](../plans/storage-lifecycle.md#4-log--overlay--cache-lifecycle--independent-lower-priority) | Lifecycle sweeps for derived junk: dangling out-links, orphaned agent staging, age-purged agent logs | `internal/prune/{staleoutlinks,agentstaging,agentlogs}.go` |

Read that column again with the ruling in hand. **Every one of those four made a GC *safe*. None of them made a GC *happen*.** Its [§1](../plans/storage-lifecycle.md#1-root-the-running-images-closure--first-everything-depends-on-it) creates roots (it adds a protection, not a reclaim). Its [§2](../plans/storage-lifecycle.md#2-auto-gc-safety-net-min-freemax-free--only-after-1) is explicitly detect-and-warn. Its [§3](../plans/storage-lifecycle.md#3-bounded-rooting-aware-store-gc-in-yolo-prune--after-1-and-2) is opt-in and default-OFF. Its [§4](../plans/storage-lifecycle.md#4-log--overlay--cache-lifecycle--independent-lower-priority) sweeps classes that were never the bulk.
...
> [!IMPORTANT]
> **The gap between "safe to run" and "runs" is the whole of the new work.** Everything in [§4](#4-a-budget-not-a-retention-count)–[§10](#10-sequencing--what-i-would-build-in-order) below is an attempt to close that one gap without spending the safety that storage-lifecycle's [§1](../plans/storage-lifecycle.md#1-root-the-running-images-closure--first-everything-depends-on-it)–[§4](../plans/storage-lifecycle.md#4-log--overlay--cache-lifecycle--independent-lower-priority) bought.
...
This ledger is the one part of the system that already works the way the whole system should. The ordering is right — `RegisterRoot` runs *before* `os.Remove(outLink)` (`autoload.go`), so there is no unrooted window — and reclamation is deliberately simple: `PruneOrphanImageRoots` (`internal/prune/imageroots.go`) reaps a root untouched for longer than its retention horizon (`prune.ImageRootRetention`) and nothing else. It was triple-guarded (tri-state liveness, an LRU-10 protected set, a 3600 s age floor) when this was written; [`OQ-LS1`](../reference/image-retention.md#why-its-this-way) replaced all three with the age cutoff, because losing a root costs a rebuild and never a running container.
...
**Writer: none, on any backend.** C3 removed podman's on 2026-08-25, and C9 removed the last one on 2026-09-09 ([`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#delivering-into-the-runtime)). A backend that cannot take a stream — Apple Container, and podman on macOS — gets a **temporary** archive, which the launch removes itself (`deliverViaArchive`, `internal/image/autoload.go`). Since the [delta archive](../reference/image-staging-vs-baking.md#the-delta-archive) it is tarred from a temporary layout in a per-attempt directory under the state dir's `image-delivery/`, outside the jail-mounted cache, and `PruneImageDelivery` reclaims a killed launch's leftover. Until then it was `cache/images/<key>.oci-archive.tmp` (Apple Container) or `.docker-archive.tmp` (podman on macOS), which the `*.tmp` sweep below still reclaims. Nothing writes a *retained* tar any more, so "unbounded on Apple Container" stopped being true with C9.
...
> [!NOTE]
> **`PruneImageCache`'s keep-N eviction branch is tested now.** It was the untested branch guarding the largest artifact class when this was checked on 2026-09-18 — no `imagecache_test.go` existed, and `internal/cli/run/housekeeping_test.go` pins only that `reapImageTars` is *called*. The 2026-09-20 `.tmp` fix ([§5](#5-invariants--what-must-not-break) P4) added `internal/prune/imagecache_test.go`, whose `TestPruneImageCacheTmpGraceFloor` includes a case dropping the tars past `keep` and retaining the newest, so the tail-drop now fails a test if it breaks. R7 is discharged on both halves.
...
- **(2) Post-C2, re-streaming the same store path** — minted a new image ID because the flake baked `created = "now"`, and handed the content tag to the newer build. **Extinct since C9 (2026-09-09):** nix2container takes no build-time timestamp, so `created` is a constant and re-delivering one store path yields the same image ID ([`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#what-a-copy-reports)). Kept because the pricing below rests on it.
...
>
> **Correction, MEASURED 2026-08-25:** an earlier version of this warning said an image is untagged only "when *its own* store path is rebuilt". That was too narrow at the time. **Re-streaming the SAME store path also minted a new image ID** — the flake set `created = "now"`, so the build timestamp was image content — and the new image took the content tag, leaving the previous one `<none>`. (No longer true since C9 made `created` a constant; see (2) above.) It is nearly free: [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md) [the cost model](../reference/image-staging-vs-baking.md#cost-model) measures such a pair at **91.36 kB unique each** against 3.554 GB shared, because every layer but the metadata is identical. Cheap, but it is a `<none>` row all the same, so it is in [OQ-DF3](#OQ-DF3)'s class.
...
> [!WARNING]
> **The second P4 exposure, the one an automated sweep was always going to find — found 2026-09-18, FIXED 2026-09-20 (`d89e0ee1`).** `PruneImageCache` swept **every** `*.tmp` under `cache/images` unconditionally, on the reasoning that a `.tmp` there is a leftover from a crashed materialization (`internal/prune/imagecache.go`). Since C9, an in-flight delivery on a backend that cannot stream writes exactly such a file — `cache/images/<key>.oci-archive.tmp` or `.docker-archive.tmp` (`archiveTempPath`, `internal/image/autoload.go`) — and neither existing lock closes it: the housekeeping lock is held only across the load path's inspect-and-record, and the image-copy lock is deliberately a **different** file (`internal/image/copylock.go`), which states why parking every reaper behind a multi-minute copy is worse than the race. So another launch's housekeeping slot could delete the archive this one was copying from — macOS-only in practice, because that is where `deliverViaArchive` runs. **The fix is neither of the two this warning first proposed**, a new name or the copy lock: it is an **age grace floor** (`imageCacheTmpGraceFloor`, one hour), the shape `internal/prune/sweep.go` already used for the same reason, sized against a multi-minute store write plus the load that follows it. A floor also still reaps a genuinely crashed delivery's leftover, which a suffix skip would have stranded forever. Pinned at the callee (`internal/prune/imagecache_test.go`) and at the `Run` call site (`internal/prune/prunecmd_test.go`).
...
- **Image *content* policy.** What nixpkgs ships into the image, and whether `fullPackages` belongs on the run path, is [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#store-delivered-packages) C5 — built 2026-09-06 as the opt-in fast path its [OQ-1](../reference/image-staging-vs-baking.md#why-its-this-way) ruled, with baking the default and the only path on the Macs. This doc reclaims copies of the image; it does not argue about what is in one.
- **The host's own `/nix/store` beyond yolo's roots.** This is the *larger* line item today — 659 GiB, +45 GiB/day ([§2.2](#22-rates--the-numbers-to-argue-from)), and `storage-lifecycle.md` already marks it **UNBOUNDED** — and this doc deliberately does not claim it. The lever is the host daemon's `min-free`/`max-free`, which yolo detects and warns about but **must not edit** (P5, `internal/cli/check/section_autogc.go`). It is storage-lifecycle [§2](../plans/storage-lifecycle.md#2-auto-gc-safety-net-min-freemax-free--only-after-1)'s host-gated residual and stays there. Saying otherwise would be this doc taking credit for someone else's `nix.conf`.
...
- **Per-workspace overlay growth and the `staticcheck` cache subdir**, which no reaper covers. Real, small relative to the image ledgers, and orthogonal to the mechanism argued here. (`nce` was on this line until [OQ-BF2](disk-levers-and-backfill.md#OQ-BF2) added it to `PurgeCacheByAge`'s default list; `staticcheck` was left out on purpose, as self-trimming.)
...
| R5 | **A byte budget that thrashes.** Evicting an image the next launch rebuilds costs 3.31 GiB of write plus a nix build to save 3.31 GiB of disk. The trade is wrong whenever the evicted image is one a live workspace is still on. | Never evict the current image (the per-workspace current-image pointers of [`OQ-LS3`](../reference/image-retention.md#why-its-this-way) are the existing precedent; Ledger A's LRU-10 protected set was, when this was written). Prefer delete-on-success, which is thrash-free by construction: it deletes only what has *already* been consumed. |
...
| :--- | :--- | :--- | :--- | :--- |
| [OQ-DF1](#112-open-questions) | **Stream, keep ZERO tars** — the maintainer's own words, ruling expressly for the C3 implementation, and past the leaning: no keep-N and **no opt-in retention knob**. On podman the nix stream was piped straight into `podman load` and `cache/images` stays empty on success. Three boundaries the ruling deliberately left standing: pre-existing tars are not swept, Apple Container still wrote and retained one per store path (a backend constraint, not an exemption), and the fallback's READER is untouched — the ruling removed the writer, not the reader. ⚠ Two of those are since overtaken: C9 (2026-09-09) replaced the stream with a layer-negotiating `skopeo copy` and gave Apple Container a temporary archive it removes, and the housekeeping slot's `reapImageTars` now sweeps the backlog | 2026-08-25 | [§6](#6-the-offline-safety-net-honestly), C3 in [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#delivering-into-the-runtime) | ✅ `be7b8591` |
...
1. ✅ **[OQ-DF1](#112-open-questions) — RULED 2026-08-25, and COMPACTED: does the offline tar fallback
   ...
   > exists, which is what keeps an offline start working. (This warning used to add that Apple
   > Container still writes one tar per store path; since C9 it writes a temporary `.oci-archive.tmp`
   > the launch removes, named so `newestTars` cannot match it — [§3.2](#32-ledger-b--the-cache-tar-now-bounded-to-zero-on-every-backend).)
   > A change that "finishes" DF1 by deleting the reader breaks the safety net the ruling
   > deliberately kept.
...
> [!NOTE]
> **How this doc cites that one, and the two ID spaces that do not agree.** Citations are by named section and ruling ID, **never by line number** — that doc is a system reference now, and its numbered sections and its risk table are gone. This doc's `C2`, `C3` and `C5` mean its [content-addressed image ref](../reference/image-staging-vs-baking.md#the-content-addressed-image-ref), its [streaming load](../reference/image-staging-vs-baking.md#delivering-into-the-runtime) and its lean image ([store-delivered packages](../reference/image-staging-vs-baking.md#store-delivered-packages)); `C9` is layer-aware delivery, which graduated into the same reference ([delivering into the runtime](../reference/image-staging-vs-baking.md#delivering-into-the-runtime)).
>
...
| `image-staging` [OQ-1](../reference/image-staging-vs-baking.md#why-its-this-way) | If C4/C5 ship, they ship as an **opt-in fast path with the baked path retained**. The gate's *step* — `image-staging`'s design-era step 5, the re-measurement after C2+C3 — **was taken 2026-08-25** and is that doc's [cost model](../reference/image-staging-vs-baking.md#cost-model). C4 and C5 were then built on 2026-09-06 in exactly that shape: opt-in per launch (`YOLO_STORE_PACKAGES=1`), podman on Linux only, baking retained everywhere else. | Sets the precedent for R6's accepted backend asymmetry, and fixes [§10](#10-sequencing--what-i-would-build-in-order)'s fifth step as the shared measurement gate. |
