...
**First, Ledger C — SHIPPED 2026-09-06 for its NUMBER/TRIGGER half; still gated on [OQ-DF3](#OQ-DF3)'s REACH half for the rest.** Reclaiming superseded, TAGGED podman images needed nothing *else* from this doc: no change to the tar path, no change to rooting, no new invariant — it is simply `PruneOldImages`, already-shipped and veto-protected, now called from the launch path itself (debounced) instead of only from a human's `yolo prune --apply`. See [§11](#11-open-questions) [OQ-DF3](#OQ-DF3) for the ruling and `internal/prune/autoreap.go` / `internal/cli/run/autoreapimages.go` for the code. **What this step does NOT do**: it does not make an untagged (`<none>`) image visible to yolo — the repo-name filter [§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row) describes is unchanged, so the podman ledger is still a blind spot for that class, and remains the measured prize a future step could still take. That widening is what **[OQ-DF3](#OQ-DF3)'s REACH sub-question** still gates — how far into a shared podman store yolo may go beyond images it can already name by tag. Do not widen reach before that is ruled; the number/trigger shipped here does not license it.
...
The maintainer ruled the **premise** (it is a bug) and the **goal** (minimal disk, delete without `--apply`). He did not rule the whole **mechanism** — but he ruled **[OQ-DF1](#11-open-questions)** on 2026-08-25, expressly for the C3 implementation, and that one is now closed. **[OQ-DF3](#OQ-DF3)'s NUMBER and TRIGGER halves were additionally settled and shipped 2026-09-06** (authorized, not a maintainer ruling on the record the way [OQ-DF1](#11-open-questions) was — see that entry for the distinction); its REACH half is untouched and stays open. Two and a half remain.
...
2. 💬 **OQ-DF2: Where does the automatic reclamation live — the write path, the launch path, or `yolo prune`'s default?**
   ...
   **Answer:**
   > _(empty — fill in when decided; Ledger C's own trigger is answered under [OQ-DF3](#OQ-DF3) instead, for the reason noted above)_
...
3. 🟡 **[OQ-DF3](#OQ-DF3) — NUMBER and TRIGGER halves shipped 2026-09-06; REACH half still open. How much of podman's image store may yolo reclaim — only images it can prove are its own, or dangling images generally?**
   ...
   **Answer (NUMBER + TRIGGER, 2026-09-06):**
   ...
   > [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) [§1.9](./image-staging-vs-baking.md#19-re-measured-2026-09-06--what-a-go-only-rebuild-costs-podman-and-what-chooses-the-flake)). Applying today's unchanged `keep=2` against that
   ...
   >
   > **What is deliberately NOT touched, and stays [OQ-DF3](#OQ-DF3)'s open REACH half:** whether yolo
   > may reclaim an untagged (`<none>`) image at all. `PruneOldImages`' query is still the same
   > repository-name filter [§3.3](#33-ledger-c--podmans-own-image-store-the-one-with-no-reclaimer-for-a-nameless-row) describes, which structurally cannot see a nameless row —
