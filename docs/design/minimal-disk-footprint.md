...
**Scope note.** This doc owns the *mechanism* — what gets deleted, when, by whom, and what the offline fallback becomes. The *measurement and the verdict* are [`image-staging-vs-baking.md`](image-staging-vs-baking.md) [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk)'s, and that doc's OQ-5 ruling is what this one executes.
...
- [`image-staging-vs-baking.md`](image-staging-vs-baking.md) — the OQ-5 ruling this doc executes, its [§1.6](./image-staging-vs-baking.md#16-what-it-has-actually-cost-on-disk) measurement, and the C2/C3 cost model this doc's sequencing has to interleave with.
...
The maintainer ruled on 2026-08-25, on [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) OQ-5:
...
- **Image *content* policy.** What nixpkgs ships into the image, and whether `fullPackages` belongs on the run path, is [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) C5 — gated on its OQ-1, whose ruling preserved the re-measurement gate. This doc reclaims copies of the image; it does not argue about what is in one.
...
- **`packages:` scope.** Ruled workspace-scope, emphatically, by [`image-staging-vs-baking.md`](./image-staging-vs-baking.md) OQ-4: *fix the cost, not the scope.* This doc fixes cost. It does not reopen scope.
...
| :--- | :--- | :--- |
| `image-staging` OQ-5 | 404 GiB of cached tars is a **bug**. No reason to keep any of it. Minimal disk is the goal. yolo **may** delete cached tars without `--apply`. The shipped GC work is nowhere near enough. | The premise of this entire doc ([§1](#1-the-ruling-and-what-the-bug-actually-is)); licenses [§4.2](#42-the-shape-this-lands-on)'s aggression and rules out A1/A2 as primary. |
...
| `image-staging` OQ-1 | If C4/C5 ship, they ship as an **opt-in fast path with the baked path retained**. The gate's *step* — `image-staging` [§11](#11-open-questions) step 5's re-measurement after C2+C3 — **was taken 2026-08-25** and is that doc's §1.8; **that discharges the step, not the gate.** The go/no-go itself is unmade and remains the maintainer's. | Sets the precedent for R6's accepted backend asymmetry, and fixes [§10](#10-sequencing--what-i-would-build-in-order)'s fifth step as the shared measurement gate. **Not** an approval of C4/C5, which are both still unbuilt. |
| `image-staging` OQ-4 | `packages:` **stays workspace-scope** — "yes, has to be." Fix the cost, not the scope. | Puts scope out of bounds ([§8](#8-what-this-does-not-cover)) and makes cost the only available lever, which is what this doc pulls. |
