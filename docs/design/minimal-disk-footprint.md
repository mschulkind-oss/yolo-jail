...
> [!IMPORTANT]
> **C3 IS SUPERSEDED BY C9, and this doc's A4 verdict is now history rather than the mechanism**
> (2026-09-09, [`layer-aware-image-delivery.md`](layer-aware-image-delivery.md)). "Stream the
> derivation into `podman load`" is gone: `.#ociImage` is a nix2container manifest and the launch
> DELIVERS it with a layer-negotiating `skopeo copy`, so there is no stream script, no
> `ImageLoadStdinCmd`, and no pipe. Every ruling below survives unchanged in its own terms —
> [OQ-DF1](#112-open-questions)'s "keep ZERO tars" is *more* true (Apple Container's file is now
> temporary and removed by the launch, so no backend retains one), and the fallback's READER is
> still untouched. What is stale is the MECHANISM each candidate describes, and the `file:line`
> citations into `internal/image` that named it. Do not re-derive the current pipeline from this
> doc; read the reference's
> ["Delivering into the runtime"](../reference/image-staging-vs-baking.md#delivering-into-the-runtime).
...
- [`storage-and-config.md`](../reference/storage-and-config.md) — where these bytes live, and the state/config separation the budget has to respect.
...
> sweeping — and the decline is CORRECT. What is wrong is that the ask fails at all: podman was
> plainly reachable in that run, which removed 1 image and 18 orphaned image GC roots.
> `LivePrefixSources` (`internal/prune/prefixroots.go`) returns not-known if the live set is
> unknown **or** if any single container cannot be inspected, and the message does not say which.
>
> **So the largest growing ledger has a reaper that has probably never run on a machine with live
> jails.** That is a defect, not a ruling, and it is upstream of everything this doc sequences.
...
design ([§3.3](#33-ledger-c--podmans-own-image-store-and-the-nameless-row)).
...
**P4. No reclaimer may delete an artifact an in-flight launch is between steps on.** **HONORED SINCE 2026-09-02 (`cc53b591`) — and it was VIOLATED when this invariant was written, which is why it is stated as one.** The violation, for the record: nothing locked or liveness-gated the cache tar, and the window between `fileExists(cacheFile)` and the converter that reads it was unguarded, so a concurrent `yolo prune --apply` evicting a *reused* tar inside it made the launch exit 1. Since C3 that pair exists only on the **Apple Container** arm — podman has no file between the two steps to race for — and that arm is where the guard went; the `newestTars` fallback reads tars on **every** backend, so it got its own. [§10](#10-sequencing--what-i-would-build-in-order) step 2 has the shapes and the pin.

> [!WARNING]
> **P4 is honored by RECOVERY, not by mutual exclusion, and a future reclaimer must not read it as a lock.** Nothing was added that locks the cache or asks a deleter's permission — the launch treats a tar that has vanished at use time as regenerable and re-materializes it once. So the invariant binds the *reclaimer* exactly as before: a new deleter that assumes the launch path will notice its eviction is relying on a two-pass recovery whose second miss is fatal by design (a cache directory that cannot keep a file is not a race). Note the polarity this still creates: [§4.2](#42-the-shape-this-lands-on)'s delete-on-success **improves** P4, because the process doing the deleting is the one that just finished with the file.
...
> [!IMPORTANT]
> **P7 is the bill this design comes with.** Automating a safe-in-principle sweep converts every unguarded race in [§3](#3-the-three-ledgers) from "a thing a careful human avoids" into "a thing that happens on a schedule set by other people's jails." P4's violation was the concrete instance, and **the requirement this bullet imposed was met**: P4 closed 2026-09-02, four days before [OQ-DF3](#OQ-DF3)'s automatic trigger shipped. The requirement itself is not spent — it binds the next unguarded race the same way, so anything built here still closes its race before, or in the same step as, it closes a trigger gap, never after.
...
| R2 | **A prune racing a concurrent launch** — evicting a reused tar between `fileExists` and the converter that reads it killed the launch (P4). **Since C3 that pair is Apple-Container-only**: podman streams and has no file between the two steps to race for. The cross-backend half is the `newestTars` fallback, which still reads tars on *every* backend. | **RETIRED as a live risk 2026-09-02 (`cc53b591`)** — both halves guarded, and it landed before the automation, which is what this row demanded. Delete-on-success would additionally *narrow* it by making the user the deleter, and is still unbuilt ([OQ-DF2](#OQ-DF2)). See [§10](#10-sequencing--what-i-would-build-in-order) step 2; the residual is a second miss in the same launch, which is fatal on purpose. |
...
**What shipped, and what it is NOT.** Both check-then-use windows are closed, each with the guard its own failure mode wants, and **no deletion of any kind was added** — `Materialize` is still the only writer, and [OQ-DF2](#OQ-DF2) option (i) was deliberately left unbuilt so closing P4 did not front-run the ruling that makes reclamation automatic.

- **The converter path (Apple Container only, since C3).** `loadAppleContainerFromCache` (`internal/image/autoload.go`, the `CLOSES P4` block) treats a tar that is *missing at use time* as a **recoverable condition**, not as a failure: the tar is regenerable from the store path by construction, so a lost race costs one re-materialization instead of a failed launch. Bounded at **two passes** — the one the launch planned, plus one recovery — because an unbounded retry would turn a cache directory that cannot keep a file (a full disk, a reclaimer in a tight loop) into a hang that re-streams a multi-GB tar forever. It keeps a **discriminator that matters**: if the tar is still on disk after the converter fails, that is a genuine conversion failure, not an eviction, so it stays fatal rather than duplicating a multi-GB write to hide the real fault.
...
- **Pinned from inside the seam** by `internal/image/tareviction_test.go`, four tests: the mid-launch eviction survives, a converter failure over a tar that is *still there* stays fatal, the retry is bounded (this one **hangs rather than failing** when the bound is removed, exactly as its comment predicts), and the fallback skips a tar evicted after listing.

**Third, the write path — HALF DONE, 2026-08-25.** Streaming (A4) shipped as [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#delivering-into-the-runtime)'s **C3**, under the [OQ-DF1](#112-open-questions) ruling: on podman the tar is never written. What remains of this step is the OTHER half — delete-on-success (A3) for the backend that must still write a file, and the sweep of the pre-C3 backlog, neither of which C3 touched. A3 is still gated on [OQ-DF2](#OQ-DF2). The split held: C3 owned "stop writing the tar on podman", and this doc owns "and what happens to the tars that do get written, on every backend."
...
| :--- | :--- | :--- | :--- |
| [OQ-DF1](#112-open-questions) | **Stream, keep ZERO tars** — the maintainer's own words, ruling expressly for the C3 implementation, and past the leaning: no keep-N and **no opt-in retention knob**. On podman the nix stream is piped straight into `podman load` and `cache/images` stays empty on success. Three boundaries the ruling deliberately left standing: pre-existing tars are not swept, Apple Container still writes and retains one per store path (a backend constraint, not an exemption), and the fallback's READER is untouched — the ruling removed the writer, not the reader | 2026-08-25 | [§6](#6-the-offline-safety-net-honestly), C3 in [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md#delivering-into-the-runtime) |
...
| [OQ-DF3](#OQ-DF3) (NUMBER) | **`--keep-images` stayed 2 — the count was never the defect.** The liveness veto, not the count, is what protects a live workspace's image. ⚠ **SUPERSEDED IN MECHANISM 2026-09-09** by [`OQ-LS3`](./the-load-sentinel-is-not-a-liveness-oracle.md#111-decision-ledger): the count was the wrong *unit*, so the flag is deleted rather than retuned and retention is the union of one current-image pointer per workspace with the `podman ps` veto. This row's reasoning is why the number was never worth arguing about; it is no longer a description of the code | 2026-09-06, superseded 2026-09-09 | [§11.2](#112-open-questions) [OQ-DF3](#OQ-DF3) |
