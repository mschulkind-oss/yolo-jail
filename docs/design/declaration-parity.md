...
([§9](#9-where-the-sweep-was-wrong) ends with the three that expired that way). Three sweep rows
...
> [!IMPORTANT]
> **MOST OF THIS SECTION SHIPPED ON 2026-09-13, and the rows still read as defects.** The
> evidence cells below describe the tree at `55a77e79` in the present tense, and a row is not
> live merely because it is un-struck. **[§11](#11-what-i-would-build-in-order)'s warning note
> is the authority for which rows the fix wave closed** — it names them, and this note
> deliberately does not repeat the list. Check the symbol before reusing a row: for the
> macos-user rows the arm to look for is `paths.NativeRuntimes`, which
> `run.appliedCtxMounts`, `run.briefedResourceLimits` and `run.sharesLauncherNetns` all have now.
...
| id | Declaration | What actually happens | Evidence, by symbol |
| :--- | :--- | :--- | :--- |
| **DP-B12** | the `<workspace>/yolo-jail.jsonc` `:ro` bind | A single-file bind emitted **ungated** — no `roBindsUnsupported` check, no `acMaterialize` fallback. Lowest severity left in this section: it fires only when `workspace_readonly` is set, and that key ALREADY warns by name on this backend, so the mechanism the file belongs to is disclosed even though this bind is not. | The `wsConfigFile` bind at the head of `run.workspaceReadonlyMountArgs`. **The two `/dev/null` shadows that used to head this row are fixed** — gated and disclosed, `run.assembleRunCmd` + `shadowbinds_test.go`. `TestNoHostHomeBindSurvivesOnAppleContainer` cannot see either class (it filters on sources under the user's HOME; `/dev/null` and a workspace path both fall outside); `TestNoAppleContainerBindHasANonDirectorySource` is the instrument that does, and it is a census over the ARGV because the divergence is an absent gate with no `rt ==` for a source census to find. Whether Apple Container DROPS or ERRORS on a single-file bind is unknowable from Linux — [`backend-parity.md` §4](backend-parity.md#4-the-proposal--a-backend-census-sibling-to-renderfieldset) residue 3. |
...
| **DP-B33** | `apply --sealed`'s remedy for outstanding capture keys | The refusal reads *"promote them into a pack or `yolo config reset <agent>/<surface>` to discard"* — English prose where a shipped command now exists. | `cli.applySealed`. ⚠ **The sweep said `yolo config promote` does not exist. It does** — see [§9](#9-where-the-sweep-was-wrong). Fix: [DP-L14](#6-alignable-with-the-mechanism-and-its-cost). |
...
`packload.LaunchFlagsFor(_, false)` — the GUARDED posture — really has none;
`render.Target.Fields()` really has no production caller; `cli.parseRunArgs` really has no
`--at` case; `render.refusalReasons` really is missing `service` and `blocked-tool`;
`cli.registry` really does map `"stop"`.

> [!IMPORTANT]
> **Three of that list's negatives have since EXPIRED, and every one of them expired by
> being BUILT.** They were true at `55a77e79`; the days after it are what moved them. **Re-derive
> an absence before reusing it** — a negative is the one form of evidence that a fix invalidates
> silently, which is why this sub-section exists at all.
>
