...
**The most important thing in this doc is [§3](#3-the-dependency--real-pack-vocabulary-conditionally-included)** (the dependency vocabulary) — everything
...
that opened this; its [`OQ-1`](./cerebras-pack-and-copilot-delivery.md#open-questions) is this doc's subject, its D-4 gets revisited in [§6](#6-consequences-for-packscerebras-and-one-reversed-ruling)),
...
Verified 2026-09-04 ([`cerebras-pack-and-copilot-delivery.md`](./cerebras-pack-and-copilot-delivery.md) §research, four primary sources):
...
- **Not a loophole.** Loopholes reach *out* from the jail to host capabilities the jail
  ...
  ([`security-shim.md`](./security-shim.md), [`loophole-protocol.md`](./loophole-protocol.md)). The bridge serves *inward*: it binds the
  ...
  ruling below ([§2.1](#21-the-service-kind-is-primary--loopholes-form-around-it-ruled)) makes the vocabulary say so.
...
fold-in (the [`OQ-A10`](./loophole-activation.md#decision-ledger) precedent) and *enforcement-as-refusal* (the launch errors if the
...
selection-lazy ([§3.4](#34-the-daemon-is-selection-lazy-not-config-lazy)): claude selected but riding zai (or nothing) while pi rides cerebras
...
2. **The claude derive is unchanged.** It already emits `ANTHROPIC_BASE_URL` from
   ...
   closure in [§3.1](#31-the-vocabulary) is what makes the declaration true, not composition's business.
...
licenses the *coarse* `when_bins` condition ([§3.2](#32-why-this-is-the-right-shape-and-what-it-costs)): including the bridge whenever claude
...
discards it.) Outbound: `Authorization: Bearer <the provider's key>`, read at boot ([§5](#5-lifecycle-key-channel-and-failure-behavior)).
...
- **Startup order.** Daemons start at boot, before the agent command. The bridge binds
  ...
  > the exact contradiction of [§3.4](#34-the-daemon-is-selection-lazy-not-config-lazy)'s healthy idle. The mechanism this build ships: the
...
- The `anthropic` endpoint declaration is conditional on this design shipping — until
  ...
  refusal at preflight once [§3](#3-the-dependency--real-pack-vocabulary-conditionally-included)'s rule exists; landing the rule and the declaration in the
  same commit as the daemon is the sequencing, [§8](#8-what-i-would-build-in-order)).
...
> This doc's [§3](#3-the-dependency--real-pack-vocabulary-conditionally-included) drafting claimed "pi/opencode/copilot resolve `openai` endpoints and never
...
> ([§3.4](#34-the-daemon-is-selection-lazy-not-config-lazy)'s mechanism, taken literally: the same selection table every agent honors).
...
- **No host-side bridge.** The bridge exists only in-jail; `yolo host -- claude` gets no
  ...
  walk through it).
- **No inbound authentication scheme.** The jail is the trust boundary ([§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters)). If that ever
  stops being true, the bridge grows auth before it grows anything else.
- **No streaming of reasoning.** Dropped at the bridge ([§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters)) — revisiting it is allowed
...
1. **The vocabulary, both pieces** — `needs` (packdecl schema + validation: strict
   ...
   [§2.1](#21-the-service-kind-is-primary--loopholes-form-around-it-ruled) kind: a jail or host daemon contribution, endpoint, restart policy, witness — no
...
2. **The translator as a library** (pure Go, stdlib `net/http` only — no new vendored
   ...
   request/response fixture pairs: every row of [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters)'s table, both directions, streaming
...
4. **The pack + the cerebras change, in one commit**: `packs/wire-bridge/` — the first
   `kind: service` instance, README opening with the [§2](#2-what-a-wire-bridge-is--and-is-not) not-a-loophole paragraph, census
...
| Risk | Mitigation |
| :--- | :--- |
| Translation drift: claude sends a shape the bridge mishandles (new beta blocks, tool variants) | [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters)'s table is the contract; unknown block types fail **closed** — an anthropic-shaped 400 naming the unrecognized block, never a silently-mistranslated request |
...
| The bridge becomes a de-facto gateway (scope creep) | [§7](#7-what-this-does-not-license) is the fence; a gateway is a different doc |
...
| :--- | :--- | :--- | :--- |
| WB-D1 | The bridge translates exactly one protocol pair: anthropic Messages ↔ openai-chat-completions | 2026-09-04 | [§2](#2-what-a-wire-bridge-is--and-is-not), [§7](#7-what-this-does-not-license) |
| WB-D2 | The URL is a provider fact (manifest `endpoints.anthropic.base_url`), the port lives only there, and the claude derive is untouched | 2026-09-04 | [§3](#3-the-dependency--real-pack-vocabulary-conditionally-included) |
| WB-D3 | Dependency is real manifest vocabulary — top-level `needs` with `when_bins` — auto-included at selection resolution and printed on the banner. SUPERSEDES this doc's own first draft (enforcement-as-refusal), rejected by the maintainer 2026-09-04: a mechanism the manifest cannot state is the wrong mechanism | 2026-09-04 | [§3.1](#31-the-vocabulary) |
| WB-D4 | Inbound auth: none; the jail is the boundary. Outbound key: the 0600 `yolo-user-env.sh`, read once at boot | 2026-09-04 | [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters), [§5](#5-lifecycle-key-channel-and-failure-behavior) |
| WB-D5 | Upstream reasoning content is dropped, not surfaced as thinking blocks; unknown block types fail closed with a named 400 | 2026-09-04 | [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters), [§9](#9-risks) |
| WB-D6 | `strict: true` is never sent upstream — claude's schemas contain what qwen's strict mode rejects | 2026-09-04 | [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters) |
| WB-D7 | Build in-repo, stdlib-only. Runtime axis already ruled: Go/Rust over Python (local-model-endpoints.md, 2026-08-20); Bifrost is a framework where a shim is needed; the four community `claude-code-proxy` repos are Python/dormant/wrong-problem | 2026-09-04 | [§8](#8-what-i-would-build-in-order) |
| WB-D8 | cerebras declares `context_window: "65536"` when this ships (reversing cerebras-doc D-4's reasoning for claude-only options, not its method) | 2026-09-04 | [§6](#6-consequences-for-packscerebras-and-one-reversed-ruling) |
| WB-D9 | `needs` may name only embedded official packs — selection must not become a supply-chain channel for unreviewed code | 2026-09-04 | [§3.1](#31-the-vocabulary) |
| WB-D10 | Needs resolve as a transitive closure at selection, before staging; cycles refuse the launch naming the loop; explicit user selection is joined, never overridden | 2026-09-04 | [§3.1](#31-the-vocabulary) |
| WB-D11 | User config carries no `needs` key — manifests only. A user-declared loopback anthropic endpoint with no bridge selected is their own dead URL; `yolo check` warns, never refuses | 2026-09-04 | [§3.4](#34-the-daemon-is-selection-lazy-not-config-lazy) |
| WB-D12 | The auto-inclusion prints on the launch banner and in `yolo check` — non-negotiable: a pack nobody typed must never join a launch silently | 2026-09-04 | [§3.1](#31-the-vocabulary) |
| WB-D13 | The listen port is fixed and manifest-borne — 8214, carried only in the provider's `anthropic` base_url; a collision is witness-fatal in a fresh namespace ([`OQ-3`](#decision-ledger), ruled 2026-09-04) | 2026-09-04 | [§3.1](#31-the-vocabulary), [§3.3](#33-the-endpoint-trick--unchanged) |
| WB-D14 | `count_tokens` refuses (404). No estimate, no zero-stub — measured: z.ai answers it `200 {"input_tokens":0}`, the source of claude's "0 tokens" display; a refusal falls back to claude's own estimator. Ruled: "I don't want to invent behavior" ([`OQ-4`](#decision-ledger)) | 2026-09-04 | [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters) |
| WB-D15 | Upstream reasoning default stands — no provider option, no bridge default. A request naming a thinking level translates only on a 1:1 mapping; none exists today, so v1 translates nothing ([`OQ-5`](#decision-ledger), ruled: "upstream to stay its default, but also allow setting it from within the agent when possible") | 2026-09-04 | [§4](#4-what-must-translate--the-protocol-surface-claude-actually-utters) |
| WB-D17 | copilot rides the bridge too: the need's when_bins is [claude, copilot] and the serve predicate walks all active profiles — found building step 4 (the copilot derive prefers anthropic endpoints, D-3, so the loopback URL reaches it) | 2026-09-05 | [§6](#6-consequences-for-packscerebras-and-one-reversed-ruling) warning |
| WB-D16 | `kind: service` is primary vocabulary — a jail or host daemon contribution with endpoint, restart, and witness, no grants. The bridge is its first instance; loopholes re-form as service + boundary grants (the [§2.1](#21-the-service-kind-is-primary--loopholes-form-around-it-ruled) decomposition) as a named follow-up, not a prerequisite ([`OQ-2`](#decision-ledger), ruled: "make service the primary and have loopholes form around them") | 2026-09-04 | [§2.1](#21-the-service-kind-is-primary--loopholes-form-around-it-ruled) |
