...
>    routing did not.
> 2. **R1 is now expressible TWICE, by two mechanisms built the same day, and both survive.**
>    `Orphaned []Kind` could express neither cause nor audience: with two addressed entries of one
>    kind, one matched and one not, a list of kinds can only name the kind. Two independent fixes
>    landed. `Destinations.Addressed` is per CONTRIBUTION — the audience, the source, and the
>    destinations it reached (empty when none did) — so it describes SUCCESSFUL deliveries too.
>    `Orphaned` became `[]packload.Orphan`, the kind PLUS the `agents` selector of the contribution
...
| carry | [`effectiveUseProfiles`, `assemble.go`](../../internal/cli/run/assemble.go) | `for k, v := range o.UseProfiles` — copies the map the user wrote; the value is carried as a plain string, never derived |
...
1. `ComposePackBriefings` moves into [`prepare.go`](../../internal/cli/run/prepare.go)'s `briefingDestinations` loop, taking the destination's audience as an argument.
2. `briefingStagingName` ([`briefingdest.go`](../../internal/cli/run/briefingdest.go)) is keyed by destination, and `assemble.go`'s mount must use the same key — the two spellings are already coupled by comment ([`assemble.go`](../../internal/cli/run/assemble.go)) and would now be coupled by content too.
