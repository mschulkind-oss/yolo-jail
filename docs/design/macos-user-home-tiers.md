...
| :--- | :--- | :--- |
| **machine** — credentials shared by every jail | `~/.local/share/yolo-jail/home/` (`paths.GlobalHome`), bound `:ro` as the home base, plus each pack's `scope: machine` dir bound rw from it (`internal/cli/run/assemble_parts.go`, `packload.SharedDirs` in `assemble.go`) | `/Users/_yolojail` |
| **workspace** — pack `state` dirs, agent history, composed content, installed programs | `<ws>/.yolo/home/{npm-global,local,go,yolo-bin,config}` plus each pack's `scope: workspace` dir, bind-mounted (`assemble_parts.go`, `packload.WritableDirs` in `assemble.go`) | `/Users/_yolojail` |
...
1. **Pack `state` dirs are machine-wide.** `.claude`, `.codex`, `.gemini`, `.pi`
   ...
   (`buildBootstrapEnv`, `internal/macosuser/runplan.go`), so `~/.claude/history.jsonl` is already a
...
3. **Composed content races between concurrent launches.** This is the new one.
   ...
   home on every entry (`InstallHomeOverlay`, `internal/entrypoint/darwin.go`).
   ...
   login rc files (`WriteLoginRC`, `darwin.go`) carry this workspace's
   `packages:` store paths, and `~/.config/mise/config.toml` (`ConfigureMisePrism`,
   `darwin.go`) carries this workspace's `mise_tools`. Both are written to the
...
(`attachExisting` is reached only from `runContainer`, `run.go`), no workspace flock (`acquireWorkspaceLock`'s one production caller
is inside `runContainer`, `run.go`), and `yolo stop` is a no-op here
...
- **That hook runs on macos-user**, unchanged: `RunDarwinBootstrap` calls
  `RunPackHooks` (`internal/entrypoint/darwin.go`). The link exists in
...
- **Every directory the podman argv binds from the sidecar becomes a symlink from the
  ...
  (`packload.WritableDirs`, `assemble.go`). Same location for a project's
  agent state on every backend; a symlink where podman has a bind.
- **Every pack-declared machine-scope directory stays in the account home — and is
  mirrored into the sidecar.** `packload.SharedDirs` (`assemble.go`) is the
...
- **What stays machine-wide stays because the container keeps it machine-wide too.**
  ...
  (`entrypoint.Env`'s `MiseData`, `internal/entrypoint/env.go`) — which would land it in the *per-workspace*
...
- **The login rc files** (`.zprofile`, `.zshrc`, `.bash_profile`) are read by the
  ...
  launch's store bin dirs, `WriteLoginRC`, `darwin.go`). They must stop carrying
...
| :--- | :--- | :--- |
| `internal/macosuser/macosuser.go` | `SandboxPath(home string, prefix []string)` | every PATH entry (`.yolo/bin/block`, `.local/bin`, mise shims, …) |
| `internal/macosuser/macosuser.go` | `LaunchArgv(…, workspace, user, home string, …)` | the `HOME`/`USER`/`SHELL`/`PATH` quartet handed to `env -i` |
| `internal/macosuser/seatbelt.go` | `SeatbeltProfile(workspace, sandboxHome string, readonlyRels []string)` | the writable subtree **and** the `/Users` read re-allow |
| `internal/macosuser/runplan.go` | `DarwinBootstrapArgv(stagedYolo, home string, …)` | the `HOME`/`JAIL_HOME` pair the generators write into |
| `internal/macosuser/runplan.go` | `buildBootstrapEnv(…, home string, …)` | the login-rc PATH (`YOLO_DARWIN_LOGIN_PATH`) |

⚠ The first review counted three; there are five in `BuildRunPlan`'s reach alone
(`runplan.go`), which is the difference between "pass a string at
...
these same functions (`BuildRunPlan`, `runplan.go`; `CaptureStagingHome` and
`BuildCapturePlan`, `internal/macosuser/capture.go`), and its profile already emits a post-allow
`(deny file-write* (subpath <home>))` over the whole shared home
(`SeatbeltCaptureProfile`, `internal/macosuser/seatbeltcapture.go`) — the exact shape a narrowing under
...
(`StageHomeOverlayCommands`, `internal/macosuser/macosuser.go`), and the
bootstrap copies them over the home (`InstallHomeOverlay`, `darwin.go`). Only
the *unwritable* half is missing — the copy is agent-writable, which
`noteMacosUserContentGaps` says at every launch (`loopholeinert.go`) — and that
...
tier (`packload.WritableDirs` / `packload.SharedDirs`, `internal/packload/packload.go`). The container mount assembler consumes
