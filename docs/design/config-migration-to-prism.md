...
> **Postscript, 2026-08-23 — the migration is over; [§0](#0-why-this-doc-exists--the-one-confirmed-live-bug-fixed-2026-07-22)'s "one confirmed live bug" is
> fixed and so is every surface in the [§2](#2-surface-inventory-and-stale-state-risk) inventory.** Read [§1](#1-the-migration-principle)–[§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) in their original
...
>
> 1. **[§3.1](#31-the-hazard-confirmed-against-the-engine)'s "no production caller yet wires `mergeDiff`/`last_render`/`overlay` into
...
> 2. **[§4.2](#42-git-identity-gitconfig--high) prescribes a git-kv codec prism surface for git identity. That is NOT what
>    shipped, and it is the one place this doc would send a reader down a dead path** —
>    see the retraction inline at [§4.2](#42-git-identity-gitconfig--high).
> 3. **[§3.2](#32-the-rule-key-first-migration-on-the-absence-of-last_render)'s first-migration rule was deliberately INVERTED for object surfaces on
...
>    discarding it."* **[§3.2](#32-the-rule-key-first-migration-on-the-absence-of-last_render)'s cost paragraph ("the first-migration boot drops any
>    un-captured pre-migration in-jail edit… correct and unavoidable") is therefore
>    RETRACTED** — it was neither. The full argument is
>    [composed-file-permissions.md](composed-file-permissions.md) [§4.2](composed-file-permissions.md#42-copilotconfig-can-wipe-a-live-oauth-token)/[§5.2](#52-what-it-clears).
>
> Two smaller drifts: **the gemini pack no longer exists** (`packs/` has no `gemini`
> dir, verified 2026-08-23), so [§4.5](#45-gemini--med-managed-mcp-sidecar--a-latent-security-bug)'s latent `setDefault` security bug is moot by
> deletion rather than by fix; and **[§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent)'s `yolo config migrate --rebuild` was never
> built** — `internal/cli/config.go` dispatches `render|ls|diff|reset|capture|drift|dump`
> and no `migrate`. Per [§7](#7-summary)'s own answer ("Is clear-and-rebuild needed for any surface?
...
[storage-and-config.md](storage-and-config.md) [§1.1](storage-and-config.md#11-config-ownership-principle-generated-config).
...
> ([§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22)) — the first-migration seed renders from the empty yolo layers and the stale
...
prism's capture-diff overlay ([§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) of the plan) will, on first boot, mistake that
...
1. **The pre-existing file is not authoritative.** The fresh pipeline render is.
   ...
   forever ([§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)). The migration's job is to *replace* the file with a truthful
...
3. **Convergence beats cataloguing.** Prefer a mechanism that needs no
   ...
   per-surface path ([§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)) needs no such catalogue; the narrow mise cleanup ([§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22))
   is the one bounded exception, and only because it must run *before* the first
   prism render for that surface.
4. **Clear-and-rebuild is a real, allowed tool — but the last resort** ([§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent)),
...
| **git identity** | `~/.gitconfig` | `identity.go configureGit` | none — sets keys only when env present, never unsets | **HIGH** (resolved 2026-07-23 — **off the prism**: host-composed + `:ro` mount, [§4.2](#42-git-identity-gitconfig--high) retraction) |
...
| Gemini settings | `~/.gemini/settings.json` (+ sidecar) | `agent_configs.go ConfigureGemini` | MCP reconciled via sidecar; some keys `setDefault` (see [§4.5](#45-gemini--med-managed-mcp-sidecar--a-latent-security-bug) bug) | MED |
...
- **MED** surfaces self-heal their yolo-managed slice each boot (via snapshot
  ...
  **first-migration overlay freeze** ([§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)) if the bootstrap is done naïvely. Not
...
is no `last_render` sidecar, so [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent)'s `mergeDiff(last_render, current_file)` sees
the entire existing file as an in-jail edit. [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) is the fix, and it must land
...
plan's [§5](../plans/agent-settings-composition.md#5-surviving-regeneration--the-capture-diff-overlay) loop then runs `overlay = deepMerge(overlay, delta)` with
...
into the overlay. Because the overlay outranks host/workspace ([§4](#4-per-surface-migration-specifics)) and **never
ages** ([§9](../plans/agent-settings-composition.md#9-decisions-all-settled), reset-only), that stale content is pinned forever. Even a corrective
...
not a live regression.** The fix belongs to whoever writes the [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) sidecar
persistence step.
...
- From the **second** boot onward the normal [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) loop runs against a *truthful*
...
- **Never seed `last_render` from the pre-existing on-disk file.** That is the
  trap: it re-introduces the exact hazard of [§3.1](#31-the-hazard-confirmed-against-the-engine), because the on-disk bytes are
...
loss on the one migration boot; [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) is the escape hatch for the rare surface where
...
- **Both present:** steady state; run the [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) loop.
...
Independently of first-boot, the [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) accumulation step `overlay = deepMerge(overlay,
...
The [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) bootstrap is the uniform mechanism. A few surfaces need extra care because
...
**As shipped, the [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) bootstrap alone fixed mise — no pre-render scrub was needed.**
...
unnecessary: convergence ([§1](#1-the-migration-principle) corollary 3) is achieved by the seed, not a
...
- Add a `miseFormerDefaults = {"node", "python", "go"}` set (a bounded,
  documented catalogue — this is the narrow [§1](#1-the-migration-principle)-corollary-3 exception, justified
...
point the [§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22) pre-render scrub is subsumed and can retire. Ship the scrub now;
...
> [!WARNING]
> The [§4.2](#42-git-identity-gitconfig--high) ownership boundary below ("a blanket overwrite of `~/.gitconfig` would be a
...
- Because the prism regenerates from the env each boot, a removed env var means
  ...
  has no memory of. The [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) bootstrap handles this **only for the owned keyspace**
...
- **Ownership boundary is critical here.** `~/.gitconfig` frequently contains
  ...
  *only* yolo's enumerated keys, and leaves every other key untouched. A blanket
  ...
  surface where "replace the file" ([§3.2](#32-the-rule-key-first-migration-on-the-absence-of-last_render)) must instead be "reconcile the owned
...
- **Orphaned files:** `yolo-host-synced-settings.json` (Claude `~/.claude/`, pi
  ...
  the prism never reads. **Delete them during the migration boot** ([§4.7](#47-orphan-file-cleanup-all-med-surfaces)) to avoid
  confusion and future-yolo mis-reads.
- **First-migration freeze:** handled by [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) — the empty-overlay seed means the
...
- Pi's host settings source no longer depends on any config key: `host_pi_files`
  ...
  `/ctx/host-pi/settings.json` fail-open (plan [§10.4](../plans/agent-settings-composition.md#104-decisions-settled-2026-07-23)).
...
the **orphaned sidecar** (delete on migration, [§4.7](#47-orphan-file-cleanup-all-med-surfaces)) plus the first-migration
freeze (handled by [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)).
...
render force-corrects them. Fixing this is part of the Gemini port, and the [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)
...
path ([§4](#4-per-surface-migration-specifics), born-on-prism). Verified at real nested-jail boot (sorted keys,
...
The [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) bootstrap is automatic, per-surface, and clean, and it is the default. But
...
[§5](../plans/agent-settings-composition.md#5-surviving-regeneration--the-capture-diff-overlay)/[§9](../plans/agent-settings-composition.md#9-decisions-all-settled), which is the narrower form). Its effect on an owned file is *identical* to
[§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) (drop stale, regenerate from constructions) — the difference is that it is
...
- The obsolete bespoke sidecars/snapshots of [§4.7](#47-orphan-file-cleanup-all-med-surfaces).

After clearing, the next boot sees `last_render` absent everywhere and runs the [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)
...
- **Workspace files** — anything under `/workspace`. That is the operating agent's
  and mirrors the host ([§2](../plans/agent-settings-composition.md#2-six-principles-the-line-in-the-sand) of the plan; ownership principle [§1.1](storage-and-config.md#11-config-ownership-principle-generated-config)). yolo never
...
`--yes`) before acting. The automatic [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) path is the only thing an ordinary boot
...
- The **first-migration bootstrap** ([§3.2](#32-the-rule-key-first-migration-on-the-absence-of-last_render)): the `last_render`-absent detection,
  empty-overlay seed, and skip-capture-this-boot logic. This belongs to whoever
  writes the [§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent) sidecar persistence step — it is *part of* that step, not a later
  add-on. Without it, the very first ported surface (pi) freezes its state.
- The **tombstone-preserving accumulation merge** ([§3.4](#34-the-tombstone-preserving-accumulation-fix-required-regardless)) — a correctness fix to
...
- A **fixture/regression vector** in the Phase A corpus: a first-boot input where
  ...
  the naïve diff) is wired, and is the guard against regressing [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface).

**Phase B (surface fan-out) — per-surface migration lands with each port.**
✅ **Done for the agent-config surfaces (2026-07-22).**

- **pi** (the proof-of-concept, first): exercises the [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) bootstrap + orphan
  cleanup ([§4.3](#43-claude--pi-settings--med-snapshot--first-migration-freeze), [§4.7](#47-orphan-file-cleanup-all-med-surfaces)) end-to-end. ✅
- **copilot** early: the zero-stale surface, cleanest bootstrap smoke test ([§4.6](#46-copilot--none)). ✅
- **Claude/gemini/opencode/codex**: each carries its orphan-sidecar cleanup ([§4.7](#47-orphan-file-cleanup-all-med-surfaces))
  and its pre-existing-bug fix (gemini `setDefault`→managed [§4.5](#45-gemini--med-managed-mcp-sidecar--a-latent-security-bug); codex
  table-preservation [§4.4](#44-opencode--codex--med-managed-mcp-sidecar)) as part of the port. ✅ (plus **agy**, born on the prism.)
- **mise**: ✅ **Ported (2026-07-22).** `GenerateMiseConfig` is replaced by
  `ConfigureMisePrism` (`internal/entrypoint/prism_mise.go`), composing
  `~/.config/mise/config.toml` through the engine. The [§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22) pre-render scrub is
  **subsumed and gone**: the first-migration seed renders from the (empty) yolo
  ...
  prism never owns `/workspace` files, [§5.3](#53-what-it-must-never-clear-hard-invariant)).
- **MCP/LSP siblings** (copilot `mcp-config.json`/`lsp-config.json`, agy
  `mcp_config.json`): ✅ **Ported (2026-07-23)** onto the *stateless*
  `renderSurfaceComputed` path ([§4.6](#46-copilot--none)) — pure per-boot overwrites, no sidecars,
...
- **git identity**: ✅ **Done 2026-07-23** (`c250c72`) — but **not** via the scoped
  git-kv codec [§4.2](#42-git-identity-gitconfig--high) proposed. It ships as host-composition + a `:ro` bind at git's
  default global path; see the retraction at [§4.2](#42-git-identity-gitconfig--high) and
...
- The obsolete snapshot/managed-MCP sidecars are removed on each surface's
  first-migration boot ([§4.7](#47-orphan-file-cleanup-all-med-surfaces)). ✅ mise ported (2026-07-22) — the standalone [§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22)
  ...
  stateless `renderSurfaceComputed` path ([§4.6](#46-copilot--none)). ✅ **git identity done 2026-07-23**
  by a different mechanism than [§4.2](#42-git-identity-gitconfig--high) planned (see the retraction there). **Phase C is
  closed; nothing is pending.**

The **clear-and-rebuild** command ([§5](#5-last-resort-fallback--clear-and-rebuild-allowed-gated-never-silent)) can land any time in Phase A/B as an
operator tool; it is not on the critical path — the automatic [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface) bootstrap is.
...
> now unbuildable** — `GenerateMiseConfig` is deleted and the port subsumed it ([§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22)
...
- **The first-boot bootstrap (one sentence):** on detecting an absent `last_render`
  ...
  next boot.
- **Is clear-and-rebuild needed for any surface?** **No** — the automatic [§3](#3-the-first-boot-bootstrap--the-clean-path-do-this-first-for-every-surface)
  bootstrap (plus the [§4.1](#41-mise-configmiseconfigtoml--high---resolved-by-the-port-2026-07-22) mise scrub and the [§4.2](#42-git-identity-gitconfig--high) git owned-key reconcile) cleanly
