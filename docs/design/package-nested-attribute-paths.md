...
**Reads with:** [`provisioner-sets.md`](provisioner-sets.md) ([§6](provisioner-sets.md#6-the-nix-resolver-in-depth) — how `packages:` materializes off-container; it absorbed [`noncontainer-nix-environment.md`](noncontainer-nix-environment.md) on 2026-09-11), [`image-staging-vs-baking.md`](../reference/image-staging-vs-baking.md) (the image build path), [`mise-node-dynamic-linking.md`](../reference/mise-node-dynamic-linking.md) (the `/lib` symlink farm and dlopen discovery).
