...
**Why it matters.** `yolo apply` at the jail notch prints, in as many words:
...
> — [`apply.go:171-175`](../../internal/cli/apply.go)

Run that command and every `via: npm` program a selected pack declares is still absent. The
launcher installs it on **first invocation** ([`shims.go:1256-1265`](../../internal/entrypoint/shims.go)
— *"Cold home: the FIRST install is not a poll … without this branch a fresh jail would simply have
no agent CLI at all"*). So the provisioning command provisions, exits 0, and leaves the thing you
selected the pack for uninstalled.
...
> here"* ([`shell.go:420-423`](../../internal/entrypoint/shell.go)).
...
> — [`applyhostdepgate.go:9-11`](../../internal/cli/applyhostdepgate.go)
...
- **Scoped to the declared set, not to an agent.** Core has no agent registry and cannot tell
  ...
  ([`autocapture.go:54-95`](../../internal/cli/run/autocapture.go)).
...
target command ([`provision.go:72-73`](../../internal/provision/provision.go)), and it is the only
phase that is both after the launchers exist and after `GenerateCABundle` — the TLS constraint that
sank the 2026-09-03 boot step. ⚠ It may **not** go in launcher generation: that runs host-side as a
dry run under `yolo check` ([`check/entrypoint.go:81-96`](../../internal/cli/check/entrypoint.go)),
so a generator that installed would install on an observe verb.
...
| **B. Fix the message instead of the behaviour** — stop claiming `yolo -- true` provisions | **Rejected, but it is the cheap half and should ship either way.** It removes the false statement without giving the notch a readiness act, so `yolo apply --at jail` still has nothing truthful to offer. Worth doing on day one; not an answer. |
...
| **R4.** `macos-user` has no provisioning stage of this shape. | Named, not assumed away: the backend's provisioning is its own, and [OQ-JR2](#OQ-JR2) decides whether readiness ships container-first with the gap reported on that backend. |
...
4. `yolo apply --at jail` no longer claims a provisioning that did not happen.
...
3. 💬 <a id="OQ-JR3"></a>**[OQ-JR3](#OQ-JR3): does `yolo apply --at jail` become real, or keep pointing at the launch?**
   Today it is a stub whose message is the false claim this doc opens with. Once the launch is a
   readiness act, the stub could tell the truth unchanged — *provisioning happens as part of
