...
| 4 | **npm, for servers** — MCP presets | `npm install -g` | `mcp_presets` | **drives** — bootstrap script (`BootstrapScript`, `internal/entrypoint/shell.go`) plus the evergreen refresh (`internal/entrypoint/serverrefresh.go`) | **absent, and warned**: the preset wrappers are Linux-absolute, so the bootstrap skips them and the stage installs none of the npm packages behind them (`Env.SkipMCPPresets`). The evergreen refresh has nothing to refresh here either: its set is the same preset package list, empty on this backend ([§1.1](#11-five-findings-the-table-forces), F5) | absent | receipts |
...
| 7 | **capture store** — yolo's own CAS | reflink → hardlink → copy (`capture.Materialize`, through `chain.placeFile`, in `internal/capture/materialize.go`) | derived from row 6 | **drives**, cold install only; auto-capture default on | **recording half only, and that half is MEASURED 2026-09-11** ([M4](../plans/runbooks/mac-provisioner-measurements.md#m4--does-the-capture-recording-half-work-on-hardware)): `yolo capture claude` records and stores an entry; what refuses here is AUTO-capture and `_try_materialize` (`internal/cli/run/autocapture.go`'s file comment; F5). The relocation rewrite a materialize here needs, [`install-capture.md`](../plans/install-capture.md#hand-offs--what-is-not-wired-and-the-exact-line-that-wires-it) H2, landed 2026-09-26 and is measured on Linux only; no launch on this backend reaches it until H4 | absent | capture manifest; receipt `kind:"capture"` |
...
Every cell is READ FROM CODE at `77190a2b`, except row 4, re-read on 2026-09-25: `lsp_servers` left that row then, because it only renders agent config and installs nothing on any backend ([`OQ-LSP1`](../reference/mcp-configuration.md#oq-lsp1)). **The guest column's rows 5, 6 and 7 are now also
...
**Update, 2026-09-25 (READ FROM CODE): the server half of F5 no longer arms.** The refresh set
(`serverRefreshSet`, `internal/entrypoint/serverrefresh.go`) is now the MCP preset package list
alone, since `lsp_servers` installs nothing ([`OQ-LSP1`](../reference/mcp-configuration.md#oq-lsp1)),
and that list is empty whenever `Env.SkipMCPPresets` is set, as it is on macos-user. So a guest
launcher bakes `SERVERS_ENABLED=0` and never calls `_refresh_servers`. This update does not
re-check the `_try_materialize` half.
...
| Auto-capture cannot run on macos-user | READ FROM CODE | `internal/cli/run/autocapture.go`'s file comment — an empty `CAPTURES_DIR` ([`install-capture.md`](../plans/install-capture.md#hand-offs--what-is-not-wired-and-the-exact-line-that-wires-it) H4). ⚠ This row gave a second reason, slice 6's relocation contract refusing until H2; H2 landed 2026-09-26 and retired it |
