...
wired — the `YOLO_JAIL_TIMING=1` argv pair (`YOLO_PROFILE` until D13) and the bash timers — is untouched (D4), and rides the
...
stderr and, through `YOLO_JAIL_TIMING=1`, an in-container `=== YOLO Jail Profile ===` block on top of
