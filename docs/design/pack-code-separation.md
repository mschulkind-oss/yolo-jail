...
> | :--- | :--- | :--- |
> | The broker **ships as a pack** (OQ-1, [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)) | 2026-08-19 | `packs/claude/loopholes/claude-oauth-broker/manifest.jsonc` — a contribution of the claude pack, not a pack of its own ([`broker-as-a-pack.md`](broker-as-a-pack.md) [§13](./broker-as-a-pack.md#13-what-empty-the-channel-actually-required--measured-2026-08-19)) |
> | The credential harvest is **deleted**, not generalized (OQ-3, [§5](#5-shared_credentials--generic-or-moved)) | 2026-08-18 | `internal/entrypoint/claude.go` is 86 lines with one function; the merge survives only as the comment at `:20-40` explaining why it went |
> | `internal/agents` **is renamed** (OQ-4, [§3.3](#33-internalagents-already-generic-in-body)) | 2026-08-18 | the package is `internal/jailcontent`; the registry the old name described was deleted rather than moved |
...
2. **Does the `claudeAiOauth` merge move into `internal/oauthbroker`, or does it stay as a claude-specific hook?**
   ...
   **Answer:**
   > **Obviated — nothing moves, and no `internal/claudecreds` is needed.** The other two rulings dissolved it: OQ-3 **deletes** the harvest's copy rather than relocating it ([§5](#5-shared_credentials--generic-or-moved)), and [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)'s doctor ruling **deletes** `check`'s copy by moving the grading behind the broker's existing `doctor_cmd`. What is left is one copy, in `internal/oauthbroker` — which is exactly where the original leaning wanted it, reached by deleting two things instead of building one. The broker is the only component that *must* understand the file, because it is the thing that refreshes the token.
   >
