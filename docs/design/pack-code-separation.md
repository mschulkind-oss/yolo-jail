...
> **DECIDED — rename them all out, and `AGENTS.md` is the only default.** Every claude mention goes unless it is genuinely generic — and "copilot implements claude's plugin format" is the shape of the exception, which should live in a pack anyway, not in core. So: rename the package (§6), fix `skills.go:52`, and **`DefaultBriefingFiles()` returns `["AGENTS.md"]`**. A pack whose prose lives in a differently-named file writes `from` explicitly, which is what `from` is for.
>
...
- `hostclaude.go` stops implying a claude-specific path (**shipped**: `packhostgrants.go`, not `hostfiles.go` — see §3.5).
...
1. **Should the broker become externally shippable now, or stay bundled with the seam documented?**
   ...
   **Answer:**
   ...
   >
   > **Designed out in [`broker-as-a-pack.md`](broker-as-a-pack.md) (2026-08-15), which changed the estimate:** the first half is already expressible and merely unexercised, so the real work is the relay's `jail_id` stamp and a manifest move. Two questions there — OQ-BP1 (may an official pack keep a baked daemon?) and OQ-BP2 (does the front gain a declared parse, or does attribution change shape?) — decide how large this is.
