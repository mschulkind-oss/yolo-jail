...
> read it as **the argument that produced the code**, not as a proposal. [§1](#1-verdict-up-front)–[§6](#6-internalagents--whats-actually-left) keep their original
...
> | :--- | :--- | :--- |
> | The broker **ships as a pack** ([OQ-1](#open-questions), [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)) | 2026-08-19 | `packs/claude/loopholes/claude-oauth-broker/manifest.jsonc` — a contribution of the claude pack, not a pack of its own ([`broker-as-a-pack.md`](broker-as-a-pack.md) [§13](./broker-as-a-pack.md#13-what-empty-the-channel-actually-required--measured-2026-08-19)) |
> | The credential harvest is **deleted**, not generalized ([OQ-3](#open-questions), [§5](#5-shared_credentials--generic-or-moved)) | 2026-08-18 | `internal/entrypoint/claude.go` is 86 lines with one function; the merge survives only as the comment at `:20-40` explaining why it went |
> | `internal/agents` **is renamed** ([OQ-4](#open-questions), [§3.3](#33-internalagents-already-generic-in-body)) | 2026-08-18 | the package is `internal/jailcontent`; the registry the old name described was deleted rather than moved |
> | `check`'s claude freshness grading moves behind `doctor_cmd` ([§6](#6-internalagents--whats-actually-left)) | 2026-08-18 | `internal/cli/check/sections_loopholes.go:225` records the Go-side grading as *"deleted with this"* |
>
> **The one thing this doc's [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally) got wrong is worth carrying forward**, because it is why the work
> was smaller than it looks: [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally) named a jail-side shippable binary and an expressible per-jail relay
...
**Two annotation kinds below.** `> **REVIEW —**` blocks are a second agent's review comments (tags: **AGREE** · **DISAGREE** · **GAP** · **NIT**); `> **DECIDED —**` blocks are the maintainer's rulings from that review round, and they win over both the original prose and the review. Everything else is small. Verdict on the whole: the diagnosis is right and well-evidenced, [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally) and [OQ-1](#open-questions)/[OQ-4](#open-questions) should be signed as written, and [§5](#5-shared_credentials--generic-or-moved) + [OQ-2](#open-questions) + [OQ-3](#open-questions) need the rework described below.
...
**Reads with:** [`loophole-packaging-overview.md`](loophole-packaging-overview.md) (the `loophole` kind and the framework-owned wire — the extension point this doc leans on), [`loophole-packaging.md`](loophole-packaging.md) (the detailed design, [OQ-LP10](./loophole-packaging.md#decision-ledger)/[OQ-LP11](./loophole-packaging.md#decision-ledger)/[OQ-LP14](./loophole-packaging.md#decision-ledger)), [`pack-capabilities.md`](pack-capabilities.md) (the `serves`/`supersedes` vocabulary the broker already uses), [`agent-credentials.md`](agent-credentials.md) (what crosses the boundary today).
...
> **DECIDED — the verdict after the review round.** The first clause holds; the other two do not. The hook becomes genuinely generic, but by **deleting** the merge rather than relocating it ([§5](#5-shared_credentials--generic-or-moved)) — so nothing moves into the broker's code, and [OQ-2](#open-questions) dissolves. And the broker is **not** kept bundled: the extension point gets built and the broker ships as a pack ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally), [OQ-1](#open-questions)). Two more rulings the original verdict did not anticipate: `check`'s claude-specific freshness check moves behind the `doctor_cmd` extension point that already exists, and every remaining claude *name* in core is renamed out rather than tolerated ([§3.3](#33-internalagents-already-generic-in-body), [§6](#6-internalagents--whats-actually-left)). Net effect on the categories above: category 2 is deleted, category 1 leaves core, and the "generic in name only" three are renamed — which is the whole residue.
...
- Adding agy was a `pack.json` (`packs/agy/pack.json`), not a Go change. The `agentcfg` surfaces, the mount assembly (`internal/cli/run/assemble.go`), and the host-file/mount grants (`internal/cli/run/hostclaude.go` — see [§7](#7-storage-migration-and-host-helpers-45-clarified)) all key on paths and kinds.
...
> **REVIEW — NIT, and this doc's own thesis in miniature.** `packhooks.go:6` says the three hooks are "all currently claude's". That went stale one commit *before* this doc (ab39897): `packs/agy/pack.json:98` declares `shared_credentials`. The comment is quoted approvingly here rather than corrected. One line to fix, and it is the exact seam [§3.2](#32-the-shared_credentials-hook-body-generic-name-claude-body) is about.
...
The broker exists only because Anthropic's refresh token is single-use (`docs/design/agent-credentials.md` [§2.5](./agent-credentials.md#25-the-claude-oauth-broker-loophole-claude-oauth-refresh)). It is *declared* as a loophole (`bundled_loopholes/claude-oauth-broker/manifest.jsonc`) but *implemented* in core:
...
>
> This is conditional on the same unverified premise [§5](#5-shared_credentials--generic-or-moved) leans on — that a tool ever leaves a real file where the symlink was — which is *why* the observation in the [§5](#5-shared_credentials--generic-or-moved) comment matters: one check settles both the cost of [§5](#5-shared_credentials--generic-or-moved) and whether agy can lose logins.
>
> **Second, and worth stating plainly: the agy path has never run.** The pack staged into this jail (`/ctx/packs/_official/agy/pack.json`, from the baked binary's embed.FS) carries no `shared_credentials` hook — the commit that added it (ab39897, 21:39 today) postdates this jail's boot (14:52). Consistently, `~/.gemini-shared-credentials/` does not exist and no `antigravity-oauth-token` symlink was created, though the hook `MkdirAll`s both parents unconditionally (`packhooks.go:120-125`). So "a second tool consumed the hook" is a **code-reading** result, not an observed one, and the first agy login in a jail booted after ab39897 is the moment to watch. That is not a criticism of the analysis — it is right — but it means nothing has exercised the fall-through, and [§5](#5-shared_credentials--generic-or-moved) would be rewriting a path with zero runtime evidence behind it.
...
> **DECIDED — rename them all out, and `AGENTS.md` is the only default.** Every claude mention goes unless it is genuinely generic — and "copilot implements claude's plugin format" is the shape of the exception, which should live in a pack anyway, not in core. So: rename the package ([§6](#6-internalagents--whats-actually-left)), fix `skills.go:52`, and **`DefaultBriefingFiles()` returns `["AGENTS.md"]`**. A pack whose prose lives in a differently-named file writes `from` explicitly, which is what `from` is for.
>
> **SHIPPED 2026-08-17.** `DefaultBriefingFiles()` returns `["AGENTS.md"]`. Two consequences the ruling did not spell out and the code now does: an explicit `from: "CLAUDE.md"` that is absent is no longer "conventional", so it REPORTS through `missingBriefingFromProblem` like any other declared source (`isConventionalBriefingFile` reads the same list); and the jail-notch test that pinned the fallback was INVERTED rather than deleted — `TestJailBriefingDoesNotFallBackToClaudeMd` now pins both halves, that the pack briefs nothing on the convention alone and that one `from` line is the whole remedy. One correction to the blast-radius note below: **all three** local packs name `AGENTS.md` in an explicit `from`, not two (`matt-core`, `matt-fzf`, `matt-local`, verified against `/ctx/packs/`), which makes the claim stronger rather than weaker.
>
> **ONE READER WAS MISSED, found on verification.** The blast radius was measured over PACKS and it should have been measured over READERS: `internal/cli/pack.go`'s `stagedContent` — the claimed-vs-unclaimed split behind `pack lint` — carried its own hardcoded `{"AGENTS.md", "CLAUDE.md"}` instead of calling `DefaultBriefingFiles()`, so it went on counting a root `CLAUDE.md` as content "some reader picks up" after every reader had stopped. A pack whose only content is a `CLAUDE.md` therefore linted **clean** while briefing nothing — the accepted-and-ignored shape both of lint's checks were rewritten to catch. Fixed by reading `packdecl.DefaultBriefingFiles()`, and pinned by `TestPackLintTracksTheBriefingConvention`, which asserts the BEHAVIOR rather than the list: a test on the literal would have stayed green through exactly this drift. The general lesson for [§3.3](#33-internalagents-already-generic-in-body)-shaped rulings: `DefaultBriefingFiles` documents itself as the authority, so the deletion's real blast radius is every site that re-lists it, and `rg '"AGENTS\.md"'` is the check that finds those.
>
> **Blast radius, checked before recording this: nil.** No pack in the tree or on this machine relies on the `CLAUDE.md` fallback — `packs/*` ship no briefing prose file at all (their `briefing` contributions declare a *destination*, `into`/`after`, and `.claude/CLAUDE.md` as a destination is unaffected by this), and the three local packs on this host (`matt-core`, `matt-fzf`, `matt-local`) each ship `AGENTS.md` and two of them name it in an explicit `from`. The fallback pair has therefore never had a second inhabitant. What changes is only what a *future* pack gets for free, and the argument in the doc comment — "both names are in the wild" — is an argument about the world, not about this repo, which is why it reads as reasonable and still loses: yolo picks one convention, and it is the cross-tool one.

### 3.4 Storage migration / back-compat (claude-keyed)

- `internal/storage/claudejson.go` — `SyncClaudeJSONSeed` back-propagates `oauthAccount`/`hasCompletedOnboarding` from a workspace `claude.json` into the `GLOBAL_HOME` seed. This is the one non-broker write-back-to-`GLOBAL_HOME` path ([`agent-credentials.md`](./agent-credentials.md) [§4](./agent-credentials.md#4-per-agent-matrix--where-each-agents-creds-live-and-how-they-get-there) note).
...
>
> **SHIPPED 2026-08-17, under a different name, because `hostfiles.go` was already taken.** `internal/cli/run/hostfiles.go` is the *user's* `host_files` config key — `hostUserFileArgs`, resolving user-scope entries to `/ctx/host-user/<slug>`, gated by config. This file is the PACK's grant — `hostFileArgs` + `hostMountArgs`, gated by pack ORIGIN, refused outright for a fetched pack. They are two different mechanisms with two different authorities, and collapsing them onto one filename would have replaced a stale name with an ambiguous one. It is **`packhostgrants.go`** (with `packhostgrants_test.go`), taking the `packhost*` prefix `packhostgate_test.go` and `packhostdisclosure_test.go` already established. The ruling's substance — that nothing in the file is claude-specific — is unchanged and is now stated in the file's own header.

---

## 4. The broker — the extension point, and how to ship it externally

The `loophole` contribution kind already exists (`packdecl/kinds.go:135`, `KindLoophole`) and already lets a pack ship a loophole whose daemon is **any language** — the framework owns the wire, and a third-party daemon only binds a plain Unix socket ([`loophole-packaging-overview.md`](./loophole-packaging-overview.md) [§3](./loophole-packaging-overview.md#3-the-framework-owns-the-wire)). So "ship a loophole externally" is *already* possible for the general case.
...
1. **`jail_daemon` runs inside a read-only image.** `yolo-jaild oauth-terminator` works only because `oauth-terminator` is baked into the jail image. A pack-shipped binary would have to be mounted `:ro` from the loophole module dir and be executable with a runtime the image bakes — which is the "native binary" question [`loophole-packaging-overview.md`](./loophole-packaging-overview.md) [§3.3](./loophole-packaging-overview.md#33-a-loophole-must-declare-where-it-can-run--raised-in-review) names and does not design.
...
> **DECIDED — build both.** This stops being a named-but-unbuilt extension point: **jail-daemon-as-binary** and the **relay/front folding** are the work, and the broker becomes an ordinary pack-shipped loophole on top of them. That answers [OQ-1](#open-questions) *shippable* and settles [OQ-4](#open-questions) with it (below).
>
...
> **⚠ Retracted, 2026-08-15 — the two bullets above overstate what is missing.** Designing this out produced [`broker-as-a-pack.md`](broker-as-a-pack.md), and writing it corrected the first bullet almost entirely. **Jail-daemon-as-binary is already expressible**: `{jail_loophole_dir}` is a manifest token legal in `jail_daemon.cmd`, the module dir is already bind-mounted at `/etc/yolo-jail/loopholes/<name>` **`:ro` with no `noexec`** (`loopholes/runtime.go:144-156`), `nix-ld` has been the FHS interpreter since 2026-07-22 so a non-nix binary links, and the pack-shipped subset never restricted `jail_daemon` at all. It is an **unexercised** path, not a missing mechanism — the same shape of finding as agy's never-run hook in [§3.2](#32-the-shared_credentials-hook-body-generic-name-claude-body).
>
> The second bullet survives, and shrinks: three of the relay's four jobs are already the front's. The one irreducible remainder is the **trustworthy `jail_id` stamp**, which the front is designed never to do. That single conflict is the whole design — see [`broker-as-a-pack.md`](broker-as-a-pack.md) [§5](./broker-as-a-pack.md#5-jail-identity--the-actual-design-decision), and its [OQ-BP1](./broker-as-a-pack.md#decision-ledger)/[OQ-BP2](./broker-as-a-pack.md#decision-ledger), which are what actually decide the size of this work.

~~The alternative — and my current leaning — is that **the broker stays bundled, but the seam is made explicit**: the claude-specific merge ([§3.2](#32-the-shared_credentials-hook-body-generic-name-claude-body)) moves *into* `internal/oauthbroker` (where the shared creds file is already owned), and the generic `shared_credentials` hook keeps only the symlink+copy-if-empty. "Bundled" is not the sin; "claude-specific logic living in a generically-named hook" is.~~

> **SUPERSEDED by the decision above** ([OQ-1](#open-questions) = shippable). Kept because its second half survives its first: the merge *is* being removed from the generically-named hook — it is simply being deleted outright rather than relocated ([§5](#5-shared_credentials--generic-or-moved)), so there is nothing left to move into `internal/oauthbroker`.
...
>
> **What it does not settle**, and this is the part left to check: with the broker active, the host daemon writes the *shared* path directly (`oauthbroker.WriteTokens` does tmp+rename on the shared file, never on the link), so this observation cannot isolate whether *Claude itself* ever rewrites the link. `claude-oauth-refresh-mechanics.md` [§6.4](../research/claude-oauth-refresh-mechanics.md#64-why-we-also-kept-the-on-demand-path) says Claude's own refresh path is deliberately retained. The case that matters is therefore a machine where the broker is **inactive** — its `requires.command_on_path: claude` is false, so a host without Claude Code installed runs no broker — or any refresh Claude performs itself. Watch the same `ls -l` there before deleting the harvest.
>
...
> **REVIEW — NIT.** See the [§3.3](#33-internalagents-already-generic-in-body) comment: "a handful of comments" is one comment, and it is not this. ~~My read is that the right decision is *don't* — leave the package name alone.~~ **Overruled, and the overrule is right:** a name is what the next reader searches for, so "one mention" is an argument about cost, not about whether the name misleads. It does.
>
> **DECIDED — do the rename.** `internal/agents` loses the name, `skills.go:52` loses the comment, and `DefaultBriefingFiles`'s `CLAUDE.md` ([§3.3](#33-internalagents-already-generic-in-body)) gets an explicit ruling instead of an implicit pass. The exception the maintainer named — something genuinely generic, e.g. copilot implementing claude's plugin format — belongs in a pack, not in core, so it does not license a claude name here either.
>
> **SHIPPED 2026-08-17 as `internal/jailcontent`.** The name says the OUTPUT, not the audience: content destined for a jail — the composed briefing, the staged skills tree, the loophole descriptions, the source-tree probe. It reads against `internal/hostskills`, which composes the same kind of thing into the user's real home, and the pairing is the point: a jail's destination is a staging dir yolo owns outright, a host's is the user's own tree. Two files moved with it, for the same reason the package did: `agents.go` → `jailcontent.go` and `agentsmd.go` → `briefing.go`. `skills.go`'s `~/.claude/skills` example is gone, and so is `briefing.go`'s header line calling this *"AGENTS.md / CLAUDE.md briefing generation"* — a fossil of when the destinations were a per-agent Go constant rather than a pack's `into`. Test fixtures still say `claude` (they exercise a real shipped pack) and past-tense comments still name `internal/agents` where they describe the deleted registry, which is what it was called.
...
- `internal/agents` stops being a name that implies an agent registry (**shipped**: `internal/jailcontent`).
- `hostclaude.go` stops implying a claude-specific path (**shipped**: `packhostgrants.go`, not `hostfiles.go` — see [§3.5](#35-host-side-helpers-generic-body-claude-name)).

**Costs:**

- Generalizing `shared_credentials` drops claude's boot-time merge of a stale local credential into a fresher shared one (a migration edge case; [§5](#5-shared_credentials--generic-or-moved)).
- ~~Making the broker externally shippable is a real piece of work … My leaning is to *name* the extension point and *not* build it until a second consumer appears.~~ **Superseded: it is being built** ([OQ-1](#open-questions)). The cost is real and unchanged — it touches the read-only image story and the transport — but it is now accepted rather than deferred, and it is partly repaid by deleting `internal/brokerrelay` ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)).
...
| The doc over-promises "ship externally" and the work stalls | The extension point is *named*, not designed, mirroring [`loophole-packaging-overview.md`](./loophole-packaging-overview.md) [§3.3](./loophole-packaging-overview.md#33-a-loophole-must-declare-where-it-can-run--raised-in-review)'s "design the extension point, not the implementation" |
...
>
> **New row — the doctor move regresses `check`'s output.** Moving `checkBrokerCredsFreshness` behind `doctor_cmd` ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)) trades a remaining-lifetime warning for a pass/fail grade unless `DoctorResult` grows a richer shape. Mitigation: make that shape part of the move, or keep the freshness *reporting* in `check` and take only the `claudeAiOauth` parsing out.
>
> **RESOLVED, 2026-08-17 (step 2 shipped) — neither option was needed: `DoctorResult` was already rich enough, and the loss was in the RENDERER.** `DoctorResult` carries `Output` alongside `RC`, and the self-check wire has always been a three-level line protocol (`FAIL:` / `NOTE:` / `OK:`). Core only ever parsed one third of it — `nixdiag.SplitSelfCheckProblems`, FAIL-only — and `check` discarded `Output` entirely on the rc=0 path, printing a fixed `self-check ok`. So a loophole genuinely could not report a healthy-but-informative *measurement*, and that gap, not the struct, is what forced the grading back into core.
>
> The fix is `nixdiag.SplitSelfCheckLines` (the whole protocol, `SplitSelfCheckProblems` now a FAIL-only view of it) plus `check.reportSelfCheckLines`, which renders `FAIL:`→fail, `NOTE:`→warn, `OK:`→pass on **both** the rc=0 and rc≠0 paths. The remaining-lifetime number survives verbatim as the text of an `OK:` line, and every other loophole's `NOTE:` lines — swallowed identically until now — start reaching the screen. Enriching the struct would have been the wrong half to change: a typed severity field would have to be *carried across a process boundary* the protocol already crosses in text.
>
> One behavioural consequence worth knowing: expired shared creds now make the broker's `--self-check` exit **1**, where before the grading was in-process and the self-check still exited 0. `reportBrokerDaemon` therefore moved out of the rc=0 branch — daemon liveness is the first thing you want when the grade is bad, and gating it on rc=0 would have hidden it in exactly that case.
...
4. **Leave the broker bundled**, but write the extension point down ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)) so a second consumer can pick it up. Do **not** build jail-daemon-as-binary now.

> ~~**REVIEW — DISAGREE.** … step 0 is the observation … my recommendation is mtime-newest-wins … leave the broker bundled.~~ **Superseded by the rulings; every premise it ordered around changed.** The observation is no longer a gate (the cost is accepted, [§5](#5-shared_credentials--generic-or-moved)), the mtime rule is withdrawn, and the broker is being shipped.
...
>
> 1. **Delete the harvest.** `harvestCredentialsFile` + `expiresAtMs` go; `linkThroughShared` keeps symlink → copy-if-empty → discard-local. Add the doc comment naming the accepted failure mode, and the regression test pinning `subscriptionType`/`rateLimitTier` survival across a broker refresh ([§8](#8-what-this-deletes-and-what-it-costs)). Smallest step, no dependencies, and it is the one the agy work exposed.
> 2. **Move the broker's freshness check behind its existing `doctor_cmd`** ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)), deleting `checkBrokerCredsFreshness` + `parseCredsExpiresAt` from `check`. Independent of step 1, and it settles the `DoctorResult` output shape *before* anything depends on it. — **SHIPPED 2026-08-17.** The grading is `oauthbroker.gradeSharedCreds`, behind `--self-check`; `check` renders the self-check's own graded lines instead of re-deriving them. `DoctorResult` was left alone — see the RESOLVED note in [§9](#9-risks) for why the renderer, not the struct, was the thing missing.
> 3. **The renames.** `internal/agents`, `hostclaude.go` → `hostfiles.go`, `skills.go:52`, and the `DefaultBriefingFiles` ruling ([§3.3](#33-internalagents-already-generic-in-body)). Mechanical, no behavior, so it lands between the deletions and the build rather than competing with either. Fix the stale `claude.go` line refs in [`agent-credentials.md`](agent-credentials.md) and [`jail-home.md`](jail-home.md) in this step, since step 1 invalidated them ([§7](#7-storage-migration-and-host-helpers-45-clarified) comment). — **SHIPPED 2026-08-17.** `internal/jailcontent` ([§6](#6-internalagents--whats-actually-left)), `packhostgrants.go` rather than `hostfiles.go` ([§3.5](#35-host-side-helpers-generic-body-claude-name) — the name was taken), `DefaultBriefingFiles() == ["AGENTS.md"]` ([§3.3](#33-internalagents-already-generic-in-body)). "Mechanical, no behavior" held for the first two and NOT for the third: dropping the second default is a behavior change with a test to prove it, which is why it carries an inverted regression test rather than a deleted one. The doc refs are corrected to SYMBOL names (`linkThroughShared`, `linkSharedCredential`, `isolateHistoryFile`) rather than new line numbers, since the numbers are what rotted.
> 4. **Ship the broker as a pack** — now designed in its own doc, [`broker-as-a-pack.md`](broker-as-a-pack.md), which found the work smaller and differently placed than this section assumed (see the retraction in [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)). Still last, because the first three shrink what it has to carry, and because two of its questions are open.
>
...
1. **Should the broker become externally shippable now, or stay bundled with the seam documented?**
   ...
   This is the closure question for [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally). Building jail-daemon-as-binary + relay/front folding is real work touching the read-only image and the transport; the alternative is to keep the broker bundled (it is yolo's own code, and `loophole-packaging-overview.md:87` already notes a baked client is fine for an official pack) and only relocate the claude-specific merge.
   ...
   > ~~**REVIEW — AGREE.** Sign it as written.~~ **Wrong on both halves.** The doctor surface I offered as a third blocker turned out to be an *extension point that already exists* ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)), so it was never a reason to stay bundled — and the "one consumer, wait for a second" discipline argues for deferring the work, not for the architecture being right. The maintainer's answer is the load-bearing one.

   **Answer:**
   > **Shippable.** Build jail-daemon-as-binary and fold the relay into the framework-owned front ([§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)), then move the broker onto them. It stops being an exception to *"agents are packs"* and becomes an ordinary pack-shipped loophole — and because the folding deletes `internal/brokerrelay` rather than relocating it, core gets smaller in the process. Sequenced last ([§10](#10-sequencing) step 4), after the deletions that shrink what it must carry.
   >
   > **Designed out in [`broker-as-a-pack.md`](broker-as-a-pack.md) (2026-08-15), which changed the estimate:** the first half is already expressible and merely unexercised, so the real work is the relay's `jail_id` stamp and a manifest move. Two questions there — [OQ-BP1](./broker-as-a-pack.md#decision-ledger) (may an official pack keep a baked daemon?) and [OQ-BP2](./broker-as-a-pack.md#decision-ledger) (does the front gain a declared parse, or does attribution change shape?) — decide how large this is.
...
2. **Does the `claudeAiOauth` merge move into `internal/oauthbroker`, or does it stay as a claude-specific hook?**
   ...
   **Answer:**
   > **Obviated — nothing moves, and no `internal/claudecreds` is needed.** The other two rulings dissolved it: [OQ-3](#open-questions) **deletes** the harvest's copy rather than relocating it ([§5](#5-shared_credentials--generic-or-moved)), and [§4](#4-the-broker--the-extension-point-and-how-to-ship-it-externally)'s doctor ruling **deletes** `check`'s copy by moving the grading behind the broker's existing `doctor_cmd`. What is left is one copy, in `internal/oauthbroker` — which is exactly where the original leaning wanted it, reached by deleting two things instead of building one. The broker is the only component that *must* understand the file, because it is the thing that refreshes the token.
   >
...
3. **Is the dropped boot-time merge ([§5](#5-shared_credentials--generic-or-moved)) acceptable for claude, or must the generic hook preserve a "newest-wins" merge for any schema?**
...
4. **Is "bundled loophole + baked daemon" acceptable for yolo's *own* claude-specific code, as a standing principle?**
   ...
   This is the meta-question behind [OQ-1](#open-questions). If the answer is "no, packs must be truly separate even for yolo's own code," then the broker *must* become shippable, and [OQ-1](#open-questions) is decided the other way.
