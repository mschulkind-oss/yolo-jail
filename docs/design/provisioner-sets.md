...
| 4 | **npm / go, for servers** — LSP and MCP presets | `npm install -g`, `go install` | `lsp_servers`, `mcp_presets` | **drives** — bootstrap script (`internal/entrypoint/shell.go:204`) plus the evergreen refresh (`internal/entrypoint/serverrefresh.go`) | **`lsp_servers` does NOT drive — MEASURED FALSE 2026-09-12.** The bootstrap does generate the script and the stage does exec it, but the script installs from `$YOLO_LSP_NPM_INSTALL` / `$YOLO_LSP_GO_INSTALL` and the only producer of either is the container's podman `-e` lines (`internal/cli/run/assemble.go:866-867`) — so the list is empty and the stage exits 0 having installed nothing ([handoff](../plans/handoff-macos-user-open-threads.md#1-lsp_servers-installs-nothing-on-this-backend--confirmed-and-three-published-claims-ride-on-it)). `mcp_presets` is still absent and **warned**: the wrappers are Linux-absolute and the stage installs none of the npm packages behind them. ⚠ the refresh is baked into every launcher and **silently no-ops** ([§3.1](#31-five-findings-the-table-forces), F5) | absent | `~/.yolo-installed-lsps` sentinel; receipts |
