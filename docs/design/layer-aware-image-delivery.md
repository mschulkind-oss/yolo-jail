...
| **Apple Container** | `skopeo copy nix:… oci-archive:<tmp>:<ref>`, then `container image load -i`, then the archive is removed | It has no `containers-storage` at all, and converted through `skopeo copy docker-archive:… oci:…` plus a `tar cf` — *two* full-size files, and a skopeo or a podman on `PATH` to convert between them. The `nix:` source deletes both writes and the `PATH` lookup. ✅ **MEASURED ON HARDWARE 2026-09-19** ([OQ-LI2](#91-decision-ledger)), the first time: `container image load -i` accepted a skopeo-written `oci-archive` on the self-hosted arm64 Mac, and the jail it produced started and passed all six Apple Container parity tests (42s for the load). ⚠ What was exercised is `just load`'s archive hop — the same skopeo destination and the same loader as `deliverViaArchive`, through a **different caller**. The launch path itself is still unmeasured, so this is one measurement shorter than an argument rather than none. |
...
- **Reaping frees less, and the retention policy inverts because of it.** Today each image is
  ...
  *lose* the base and pay a full re-copy, which is why
  ...
  [OQ-LS3](../reference/image-retention.md#why-its-this-way) rules that the count rises with
...
1. ✅ **[OQ-LI6](#OQ-LI6) — AUTHORIZED 2026-09-09, measurement first: build it?** Every design question above is ruled; this is the authorization, and
   ...
   - **One prerequisite in a sibling doc.** [OQ-LI4](#91-decision-ledger)'s keep-window reorder is subsumed by
     [OQ-LS3](../reference/image-retention.md#why-its-this-way), and step 1 of
   ...
   **Answer (2026-09-09): BUILD IT — and take the measurement FIRST, not alongside.**
   ...
   > [OQ-LS3](../reference/image-retention.md#why-its-this-way), which was blocked
