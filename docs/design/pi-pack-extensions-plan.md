...
**Status:** SKETCH, 2026-09-19 — incomplete, and unstable while the questions in
[`pi-pack-extensions.md`](./pi-pack-extensions.md#9-open-questions) are open.
...
activated by the owning pack's derive)**. The `target` axis is deferred
([OQ-1](./pi-pack-extensions.md#OQ-1)), flat per-file prefixing is replaced by a per-pack
subdirectory ([OQ-4](./pi-pack-extensions.md#OQ-4)), and the `claude_plugins` decision is
reopened ([OQ-2](./pi-pack-extensions.md#OQ-2)).
...
2. **`internal/packload/mergedest.go`** — extend destination borrowing to `files`, matching a
   ...
   per agent** (until [OQ-1](./pi-pack-extensions.md#OQ-1) rules otherwise): a second is a
   load error naming both contributions.
...
5. **`packs/pi/pack.json`** — declare the files alias for `pi`; migrate
   ...
   auto-discover).
6. **`packs/claude/*` + remove `HookClaudePlugins`** — if [OQ-2](./pi-pack-extensions.md#OQ-2)
   lands on seed: deliver the LSP plugin trees by file, seed
   `CLAUDE_CODE_PLUGIN_SEED_DIR`, keep the `enabledPlugins` derive, and delete the hook, the
   `packdecl.KnownHooks` entry, and the `packhook` case.
...
- **`target` axis.** Deferred, not deleted, pending [OQ-1](./pi-pack-extensions.md#OQ-1).
