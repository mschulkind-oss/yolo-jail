...
ceiling) and ships `fast`/`haiku` → `glm-5.3-flash` beside `default`/`sonnet` →
`glm-5.3` — **wire-true ids**: every agent's catalog sends them verbatim, and z.ai
rejects claude's `[1m]` spellings on both routes (measured 2026-09-04; neither pi nor
opencode strips a suffix). Only claude's derive re-spells them, appending `[1m]` from
the provider's `context_window` — claude code's own syntax for the context-1M beta,
which it strips back off before the wire. A profile stating `"model": "fast"` selects
the flash model.
