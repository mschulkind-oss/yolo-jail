...
protocols, so every agent you already have can use it, and one API key from the z.ai console
serves them all.
...
| pi, codex, opencode | a `zai` entry in their provider catalog, pointing at the openai route | the derives, reading `YOLO_PROVIDERS` (the composed table) |
...
The catalog is presence, not choice. `-p zai` puts `zai` in pi's, codex's and opencode's
catalogs; **which entry each of them uses is still that agent's own setting** (pi's default
model, codex's `model`/`model_provider`, opencode's `provider`) until
[`zai-plumbing.md`](../../docs/design/zai-plumbing.md) §5 closure rule 3 lands — one
...
Until then, a user who wants every agent on GLM writes the choice where the agent reads it —
or waits for the overlay, which is the same keys delivered by the pack instead of by hand.
...
$ ZAI_API_KEY=x yolo -p zai         # every selected CLI answers to zai; claude gets the pair
