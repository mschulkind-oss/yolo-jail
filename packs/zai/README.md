...
(`"options": {"model": "default"}`) and ships `fast` → `glm-4.7-air` beside `default` →
`glm-4.7`, so a profile stating `"model": "fast"` selects the air model.
