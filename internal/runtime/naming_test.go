...
// Values are the frozen sanitize+hash output the container name must not drift
// from.
var goldenNames = map[string]string{
	...
	"/":                                "yolo-jail-8a5edab2",
	// U+0130 (Turkish İ): full case folding expands to "i"+combining-dot, which
...
// TestPyLowerU0130 documents (and guards) the claim that U+0130 is the ONLY
// code point whose full case folding differs from what pyLower produces in a
// way that survives the [^a-z0-9-] sanitize. This test pins the special-case
// itself.
func TestPyLowerU0130(t *testing.T) {
	...
	if got := sanitize("aİb"); got != "ai-b" {
		t.Errorf("sanitize(aİb) = %q, want ai-b (full case-fold expansion)", got)
	}
