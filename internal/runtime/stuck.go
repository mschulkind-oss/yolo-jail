...
// Mirrors the set in _check_container_stuck.
var provisioningCommands = map[string]struct{}{
	"uv": {}, "mise": {}, "pip": {}, "npm": {},
}

// infraCommands are never counted as "user" processes — the container's own
// scaffolding. Mirrors the second exclusion tuple in _check_container_stuck.
...
// reason string if the container is stuck, or "" if healthy. Mirrors the pure
