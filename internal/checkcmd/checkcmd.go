...
// dry-run). The exact ANSI bytes are a NEW Go-native golden (pinned
...
// binary was absent or the process could not be started (the Python
// FileNotFoundError branch); Timeout is true when the call exceeded its
// deadline (the TimeoutExpired branch). Both degrade gracefully — callers treat
// a non-Ran/Timeout result the way the Python `except` clauses do.
...
type Options struct {
	...
	Build bool
	// Now is the clock seam for _check_broker_creds_freshness (the only
	...
	// Returns (path, ok); ok=false reproduces _resolve_repo_root's SystemExit.
	...
	// ("needs first-boot", …) matching ensure_builder. nil => real
	// implementation. onProgress receives boot status messages.
	EnsureBuilder func(onProgress func(string)) (bool, string)
	// BuildImage runs the real `nix build .#ociImage` (image._build_image_store_path)
	...
	// AccessRW tests os.access(path, R_OK|W_OK) for device-node checks (KVM /
	...
	// (grp.getgrgid), plus ok=false when the node can't be stat'd. nil => real.
	NodeGID func(string) (gid int, groupName string, ok bool)
	// InUserGroups reports whether gid is in os.getgroups(). nil => real.
...
// non-empty read is the faithful, test-injectable signal — the theoretical
...
// Timeout=true — the two graceful-degradation branches the Python subprocess
// sites rely on. dir sets the working directory (""=inherit); env entries are
