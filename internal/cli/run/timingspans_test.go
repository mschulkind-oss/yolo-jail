...
// The litter fix: podman creates `node_modules`/`.venv` in the user's own repo
// to mount the per-side shadows over, and used to leave them there — two empty
// root-owned directories in a tree the user never touched (reported and
// reproduced 2026-09-08 on a bare `git init` + one launch, no agent involved).
// Teardown removes exactly the ones this launch caused, and only while empty.
func TestShadowMountPointsArePrunedButOnlyWhenYoloMadeThem(t *testing.T) {
	ws := t.TempDir()
	home := t.TempDir()
	t.Setenv("HOME", home)
	emptyLoopholeDirs(t)
	...
	// A directory the USER already had, and one they had with content in it.
	preExisting := filepath.Join(ws, ".venv")
	if err := os.MkdirAll(preExisting, 0o755); err != nil {
		t.Fatal(err)
	}
	o.venvShadowMountArgs(newConfig(), filepath.Join(ws, ".yolo", "home"))

	// venvShadowMountArgs must have claimed node_modules (absent) and NOT .venv.
	var claimed []string
	for _, p := range o.shadowMountPoints {
		claimed = append(claimed, filepath.Base(p))
	}
	if len(claimed) != 1 || claimed[0] != "node_modules" {
		t.Fatalf("claimed %v, want only [node_modules] — a path the user already had is theirs", claimed)
	}

	// podman's side: it creates the mount point. Simulate both outcomes.
	...
	if _, err := os.Stat(nodeModules); !os.IsNotExist(err) {
		t.Error("an empty mount point yolo caused must not survive teardown")
	}
	if _, err := os.Stat(preExisting); err != nil {
		t.Error("a directory the user already had must never be removed")
	}

	// And a NON-empty one survives: rmdir fails on it by construction, which is
	// what protects a real node_modules the user populated on the host.
	if err := os.MkdirAll(nodeModules, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(nodeModules, "somepkg"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	o.pruneShadowMountPoints()
	if _, err := os.Stat(nodeModules); err != nil {
		t.Error("a non-empty node_modules must survive teardown")
	}
}
