...
func TestApplyHostReportsAnAudienceThatReachedNoDestinationOfThatKind(t *testing.T) {
	...
	// The wording is the merged one: the audience is quoted (quotedAgents) and the missing thing
	// is named as a matching `agent` rather than "that identity". Two implementations of this
	// line landed the same day; this is the surviving text, and apply.go says why.
	if n := countLines(report, "house", `addresses "alphacli"`, "declares a matching"); n != 1 {
