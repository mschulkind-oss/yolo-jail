...
func TestRewriteArgvSkipsFlagValues(t *testing.T) {
	cases := []struct {
		...
		{
			name: "--agent-profile value",
			in:   []string{"--agent-profile", "run", "--", "bash"},
			want: []string{"--agent-profile", "run", "run", "--", "bash"},
		},
		{
...
func TestValueTakingFlagsCoverRunHelpSkips(t *testing.T) {
	...
	runHelpSkips := []string{"--network", "--claude-auth", "--auth", "--agent-profile", "-p"}
