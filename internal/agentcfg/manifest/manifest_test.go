...
// YAML used to validate here without an implementation, so a codec:yaml surface passed
...
func TestAcceptedCodecsAreImplemented(t *testing.T) {
	...
	// The specific drift that shipped is now supported end-to-end.
	s := goodSurface()
	s.Codec = "yaml"
	if _, err := New(s); err != nil {
		t.Errorf("Surface{Codec: \"yaml\"} rejected: %v", err)
	}
}
