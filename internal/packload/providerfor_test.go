...
// zaiProviderPack ships the zai provider and a variant NAMED SOMETHING ELSE that requires
// it, and installs no CLI at all. The name mismatch is load-bearing: with the variant
// named "zai" the profile-name fallback would return the same string and the assertion
// could not tell a resolved requires_provider from a lucky fallback.
...
func TestProviderForNamesTheRequiredProvider(t *testing.T) {
	...
	if got := ProviderFor([]*Pack{claude, zai}, "claude", "glm"); got != "zai" {
		t.Errorf("a provider pack's requires_provider must reach the agent pack's CLI: got %q", got)
	}
	...
	if got := ProviderFor([]*Pack{claude}, "claude", "bedrock"); got != "bedrock" {
		t.Errorf("an undeclared variant names its own provider: got %q", got)
	}
...
// variant of the same name, ITS requires_provider is the more specific intent and must
// win over a provider pack's.
func TestProviderForPrefersTheBinOwnersOwnDeclaration(t *testing.T) {
	claude := &Pack{Name: "claude", Decl: declFrom(t, `{"contributes":[
	  {"kind":"program","bin":"claude","via":"npm","package":"@anthropic-ai/claude-code"},
	  {"kind":"profile","name":"glm","requires_provider":"own-zai"}]}`)}
	if got := ProviderFor([]*Pack{claude, zaiProviderPack(t)}, "claude", "glm"); got != "own-zai" {
		t.Errorf("the bin owner's requires_provider should win: got %q", got)
	}
	// Order of the pack list must not decide it.
	if got := ProviderFor([]*Pack{zaiProviderPack(t), claude}, "claude", "glm"); got != "own-zai" {
		t.Errorf("the bin owner's requires_provider should win regardless of pack order: got %q", got)
	}
}
