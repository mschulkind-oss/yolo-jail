...
// appleContainerBaseMounts ports the `if runtime == "container"` base run_cmd
...
// podmanBaseMounts ports the else-branch base run_cmd (1751-1818): the :ro
...
// podmanNestingArgs ports the `if runtime == "podman"` nesting/GPU/device+cap
...
// gitignoreMountArgs ports the global-gitignore propagation (2228-2263): read
...
// forwardHostPortsArgs ports the host-port-forwarding FLAG emission (2291-2321):
...
// hostServicesMountArgs ports the host-services sockets-dir mount + broker relay
...
// repoMountSource ports the /opt/yolo-jail source selection (2031-2036).
...
// userConfigMountArgs ports the user-config mount for nested jails (2682-2698).
...
// loopholesRuntimeArgs ports the host-side loopholes runtime args (2823-2828):
...
// resourceArgs ports the resource-limits block (2600-2649): --memory/--cpus with
