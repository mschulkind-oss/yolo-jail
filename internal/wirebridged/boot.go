...
// derives read) and serves only when claude's active profile names a provider
// whose `anthropic` endpoint is jail-local — i.e. routed AT this bridge — and
// whose `openai` endpoint supplies the upstream. Any other boot is a HEALTHY
// IDLE: bind nothing, publish nothing, sleep forever, one stderr line saying
// why. That laziness is what licenses the coarse when_bins inclusion (§3.2):
// the bridge may be staged in every launch that selects claude, and precise
// behavior is recovered here by the same selection table every agent honors.
//
...
// resolveRoute decides, from the composed provider table and the resolved
// selection, whether this boot serves and with what — or why it idles, in a
// string that names the exact absent fact (§3.4). Every idle branch is a
// HEALTHY outcome, not an error: the bridge is staged coarsely whenever claude
// is selected, and "claude rides zai today" is the common no-op.
//
...
func resolveRoute(e *entrypoint.Env) (route, string) {
	...
	profileName := ""
	if v, ok := useProfiles.Get("claude"); ok {
		profileName, _ = v.(string)
	}
	if profileName == "" {
		return route{}, "no profile is active for claude in YOLO_USE_PROFILES — nothing routes at the bridge"
	}

	providerName := packload.ProviderFor(e.LoadProfiles(), profileName)
	if providerName == "" {
		return route{}, "claude's active profile " + profileName +
			" resolves to no provider in YOLO_PROFILES"
	}

	v, ok := e.LoadProviders().Get(providerName)
	if !ok {
		return route{}, "provider " + providerName + " is not in the composed table (YOLO_PROVIDERS)"
	}
	entry, isMap := v.(*jsonx.OrderedMap)
	if !isMap {
		return route{}, "provider " + providerName + "'s table entry is malformed (YOLO_PROVIDERS)"
	}
	...
	if !ok {
		return route{}, "provider " + providerName + "'s anthropic endpoint (" + anthropicURL +
