...
**How an agent uses a host capability — a shared login, a credential broker, a device — without holding it.** A *spawned* loophole is a process that runs on the host (outside the jail) and publishes an address the jail can reach through a per-jail directory bind-mounted at `/run/yolo-services/`. The agent inside the jail can talk to the loophole without ever holding its secrets, credentials, or privileges. See [Loopholes](loopholes.md) for the broader loophole system (including intercepting loopholes like the Claude OAuth broker).
