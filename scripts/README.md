...
If you upgraded from a pre-broker install, `just deploy` automatically disables and removes the legacy `claude-token-refresher.service` / `claude-token-refresher.timer` units. See [docs/research/claude-token-logouts.md](../docs/research/claude-token-logouts.md) for the post-broker troubleshooting walkthrough.
