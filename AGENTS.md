...
- **Reaching the Host from Inside Jail**: Use `host.containers.internal` (resolves to `169.254.1.2` on podman, `host.docker.internal` also works). This hostname is automatically added by podman/docker to `/etc/hosts` and routes traffic to the host machine. Useful for agents that need to access host services (e.g., pull from host git servers, reach a development API running on the host).
